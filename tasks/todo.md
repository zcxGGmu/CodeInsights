...
- [x] 完成启动读取、`native/search` 文件列表与关键源码核对、`git status --short --branch` 与 `git log -25 --oneline`。
- [x] 确认当前实际 HEAD 为 `4cf20557`，最近历史包含 `26221722`、`c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`。
- [x] 确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，`tail_jsonl` 仍未进入最小 search sidecar 切片。
- [x] 等待只读 explorer 回传文档漂移、smoke gate 和 no-go 禁改边界复核结果。
- [x] 更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和必要 lessons，把最新已确认状态同步 / 恢复入口从 `26221722` 回填为 `4cf20557`，并继续标注 Phase 5 已完成 / 未完成边界。
...
- [x] 单独提交本轮文档状态同步；提交后最终回复给出提交 hash、验证结果和可直接复制的下次启动提示词。

### Review

- 启动恢复完成：当前分支为 `rust-go-refactor`，启动 HEAD 为 `4cf20557 docs(rust-go): 回填 26221722 Phase 5 恢复入口`，最近历史包含 `4cf20557`、`26221722`、`c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`；工作树启动时干净。
- 子代理只读复核完成：文档漂移复核确认 active Rust / Go 文档仍需从 `26221722` 回填到 `4cf20557`；smoke / no-go 复核确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，`tail_jsonl` 仍显式 out-of-scope，`apps/electron/package.json` / `bun.lock` 没有真实 `@codeinsights/native-search-*` optionalDependencies，`apps/electron/electron-builder.yml` 仍通过 `!node_modules/@codeinsights/**` 阻断 native-search allowlist，仓库未发现真实 packaged `native-search-package.json` 或 `codeinsights-native-search(.exe)` 产物。
- 文档状态已回填：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `tasks/lessons.md` 已把最新已确认状态同步 / 恢复入口推进到 `4cf20557`，并继续保留 `63e0fc2b` 为最新实现基线。`26221722`、`c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba` 与 `baa0ae3d` 仍作为历史状态同步提交记录，不再作为当前恢复入口。
- 只读 readiness 结果：`optional-package-source --native-search-package-version 0.0.3` 通过，summary 显示 `optionalPackageSourceReady=true`、4 个 planned package source ready，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`；显式 registry dry-run `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 通过，summary 显示 `optionalPackagePublishTargetChecked=true`、`optionalPackagePublishTargetReady=true`、4 个 planned package 目标版本无 collision / invalid / unavailable package、Cargo version 与 `BINARY_VERSION` 均为 `0.0.3`，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`。
- no-go evidence：默认离线 `packaged-manifest` 通过，继续显示 `optionalPackagePublicationChecked=false`、`optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`optionalDependenciesLockfileVerified=false`、`optionalDependenciesInstalledPackagesVerified=false`、`packagingConfigVerified=false`、`bundledBinaryVerified=false`、`realPackagedBinaryVerified=false`、`defaultEnableCandidate=false`，且 `release-handoff-no-go-boundary-audit` passed。显式 `packaged-manifest --check-registry` 经 `pipefail` 复核按预期 exit 1，summary 显示 `optionalPackagePublicationChecked=true`、`optionalPackagesPublished=false`，4 个 planned packages 均在 `missingPublishedOptionalPackages`：`@codeinsights/native-search-darwin-arm64`、`@codeinsights/native-search-darwin-x64`、`@codeinsights/native-search-win32-x64`、`@codeinsights/native-search-linux-x64`。
- 边界保持：本轮未执行真实 `npm publish`，未运行真实 install，未修改 `apps/electron/package.json` / `bun.lock` 以新增真实 optionalDependencies，未修改 `apps/electron/electron-builder.yml`，未创建 packaged app / native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。
- 验证通过：`git diff --check` 无输出；禁改文件 diff 无输出；`apps/electron/package.json` / 根 `package.json` / `bun.lock` 中无真实 `@codeinsights/native-search-*` optionalDependencies；`apps/electron/electron-builder.yml` 仅保留既有 `!node_modules/@codeinsights/**` 阻断规则；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；active Rust / Go 文档已把当前恢复入口推进到 `4cf20557`。

## 2026-06-08 Rust/Go Phase 5 26221722 恢复入口回填计划

范围确认：继续 `rust-go-refactor` 分支 Phase 5 “Rust search sidecar 试点”。本轮启动已按要求读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、development checklist、sidecar protocol / smoke plan 和 `native/search/`，并运行 `git status --short --branch` 与 `git log -25 --oneline`。实际 HEAD 为 `26221722 docs(rust-go): 回填 c95c5cf6 Phase 5 恢复入口`，工作树启动时干净；最新实现基线仍为 `63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest`。当前 active 文档仍有最新状态同步 / 恢复入口停在 `c95c5cf6` 的语义，因此本轮只做 `26221722` 恢复入口回填、只读 readiness/no-go 复核和状态同步提交。未获用户明确批准前，不执行真实 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。

- [x] 完成启动读取、`native/search` 文件列表与关键源码核对、`git status --short --branch` 与 `git log -25 --oneline`。
- [x] 确认当前实际 HEAD 为 `26221722`，最近历史包含 `c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`。
- [x] 确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，`tail_jsonl` 仍未进入最小 search sidecar 切片。
- [x] 等待只读 explorer 回传文档漂移、smoke gate 和 no-go 禁改边界复核结果。
- [x] 更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和必要 lessons，把最新已确认状态同步 / 恢复入口从 `c95c5cf6` 回填为 `26221722`，并继续标注 Phase 5 已完成 / 未完成边界。
- [x] 运行只读 readiness/no-go 验证：`optional-package-source --native-search-package-version 0.0.3`、显式 `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`、默认离线 `packaged-manifest`，以及预期失败的 `packaged-manifest --check-registry` evidence。
- [x] 运行禁改扫描：`git diff --check`、禁改文件 diff、真实 native optionalDependencies 扫描和 packaged native binary / package manifest 产物扫描。
- [x] 在本 Review 记录验证结果、真实未完成项和需要用户明确批准后才能执行的最小 publication gate。
- [x] 单独提交本轮文档状态同步；提交后最终回复给出提交 hash、验证结果和可直接复制的下次启动提示词。
...
- 启动恢复完成：当前分支为 `rust-go-refactor`，启动 HEAD 为 `26221722 docs(rust-go): 回填 c95c5cf6 Phase 5 恢复入口`，最近历史包含 `26221722`、`c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`；工作树启动时干净。
- 子代理只读复核完成：文档漂移复核确认 active Rust / Go 文档仍需从 `c95c5cf6` 回填到 `26221722`；no-go 复核确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，`apps/electron/package.json` / `bun.lock` 中没有真实 `@codeinsights/native-search-*` optionalDependencies，`apps/electron/electron-builder.yml` 未包含 native-search allowlist 且仍有 `!node_modules/@codeinsights/**` 排除项，仓库未发现真实 packaged `native-search-package.json` 或 `codeinsights-native-search(.exe)` 产物。
- 文档状态已回填：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `tasks/lessons.md` 已把最新已确认状态同步 / 恢复入口推进到 `26221722`，并继续保留 `63e0fc2b` 为最新实现基线。`c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba` 与 `baa0ae3d` 仍作为历史状态同步提交记录，不再作为当前恢复入口。
- 只读 readiness 结果：`optional-package-source --native-search-package-version 0.0.3` 通过，summary 显示 `optionalPackageSourceReady=true`、4 个 planned package source ready，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`；显式 registry dry-run `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 通过，summary 显示 `optionalPackagePublishTargetReady=true`、4 个 planned package 目标版本无 collision / invalid / unavailable package、Cargo version 与 `BINARY_VERSION` 均为 `0.0.3`，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`。
- no-go evidence：默认离线 `packaged-manifest` 通过，继续显示 `optionalPackagePublicationChecked=false`、`optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`optionalDependenciesLockfileVerified=false`、`optionalDependenciesInstalledPackagesVerified=false`、`packagingConfigVerified=false`、`bundledBinaryVerified=false`、`realPackagedBinaryVerified=false`、`defaultEnableCandidate=false`，且 `release-handoff-no-go-boundary-audit` passed。显式 `packaged-manifest --check-registry` 按预期 exit 1，summary 显示 `optionalPackagePublicationChecked=true`、`optionalPackagesPublished=false`，4 个 planned packages 均在 `missingPublishedOptionalPackages`：`@codeinsights/native-search-darwin-arm64`、`@codeinsights/native-search-darwin-x64`、`@codeinsights/native-search-win32-x64`、`@codeinsights/native-search-linux-x64`。
- 命令执行记录：一次同步扫描命令把包含反引号的 `rg` pattern 放进双引号，触发了 zsh command substitution 并输出 `command not found`，没有执行发布 / 安装 / builder 修改等副作用；后续固定使用单引号 pattern 复核。该 shell 包装问题已在本 Review 记录，避免后续重复。
- 边界保持：本轮未执行真实 `npm publish`，未运行真实 install，未修改 `apps/electron/package.json` / `bun.lock` 以新增真实 optionalDependencies，未修改 `apps/electron/electron-builder.yml`，未创建 packaged app / native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。
- 验证通过：`git diff --check` 无输出；禁改文件 diff 无输出；`apps/electron/package.json` / `bun.lock` 中无真实 `@codeinsights/native-search-*` optionalDependencies；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；active Rust / Go 文档已把当前恢复入口推进到 `26221722`。

## 2026-06-08 Rust/Go Phase 5 c95c5cf6 恢复入口回填计划

范围确认：继续 `rust-go-refactor` 分支 Phase 5 “Rust search sidecar 试点”。本轮启动已按要求读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、development checklist、sidecar protocol / smoke plan 和 `native/search/`，并运行 `git status --short --branch` 与 `git log -25 --oneline`。实际 HEAD 为 `c95c5cf6 docs(rust-go): 同步 5bc9ebe8 Phase 5 最新状态`，工作树启动时干净；最新实现基线仍为 `63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest`。当前 active 文档仍有最新状态同步 / 恢复入口停在 `5bc9ebe8` 的语义，因此本轮先只做 `c95c5cf6` 恢复入口自引用回填、只读 readiness/no-go 复核和状态同步提交。未获用户明确批准前，不执行真实 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。

- [x] 完成启动读取、`native/search` 文件列表核对、`git status --short --branch` 与 `git log -25 --oneline`。
- [x] 确认当前实际 HEAD 为 `c95c5cf6`，最近历史包含 `5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`。
- [x] 确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，`tail_jsonl` 仍未进入最小 search sidecar 切片。
- [x] 等待只读 explorer 回传文档漂移、smoke gate 和 no-go 禁改边界复核结果。
- [x] 更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和必要 lessons，把最新已确认状态同步 / 恢复入口从 `5bc9ebe8` 回填为 `c95c5cf6`，并继续标注 Phase 5 已完成 / 未完成边界。
- [x] 运行只读 readiness/no-go 验证：`optional-package-source --native-search-package-version 0.0.3`、显式 `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`、默认离线 `packaged-manifest`，以及预期失败的 `packaged-manifest --check-registry` evidence。
- [x] 运行禁改扫描：`git diff --check`、禁改文件 diff、真实 native optionalDependencies 扫描和 packaged native binary / package manifest 产物扫描。
- [x] 在本 Review 记录验证结果、真实未完成项和需要用户明确批准后才能执行的最小 publication gate。
- [x] 单独提交本轮文档状态同步；提交后最终回复给出提交 hash、验证结果和可直接复制的下次启动提示词。
...
- 启动恢复完成：当前分支为 `rust-go-refactor`，启动 HEAD 为 `c95c5cf6 docs(rust-go): 同步 5bc9ebe8 Phase 5 最新状态`，最近历史包含 `c95c5cf6`、`5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`；工作树启动时干净。
- 子代理只读复核完成：文档漂移复核确认 active Rust / Go 文档仍需从 `5bc9ebe8` 回填到 `c95c5cf6`；smoke gate 复核确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`；no-go 禁改复核确认当前没有真实 `@codeinsights/native-search-*` optionalDependencies、builder allowlist 修改、packaged native binary 或 `native-search-package.json` 产物。`native/search/target/release/codeinsights-native-search` 仅是 ignored Cargo build cache，不是 packaged evidence。
- 文档状态已回填：development checklist、sidecar protocol / smoke plan 和 `next-session-prompt.md` 已把最新已确认状态同步 / 恢复入口推进到 `c95c5cf6`，并继续保留 `63e0fc2b` 为最新实现基线。`5bc9ebe8`、`bcc65b3d`、`e2b620ba` 与 `baa0ae3d` 仍作为历史状态同步提交记录，不再作为当前恢复入口。
- 只读 readiness 结果：`optional-package-source --native-search-package-version 0.0.3` 通过，summary 显示 `optionalPackageSourceReady=true`、4 个 planned package source ready，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`；显式 registry dry-run `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 通过，summary 显示 `optionalPackagePublishTargetReady=true`、4 个 planned package 目标版本无 collision / invalid / unavailable package、Cargo version 与 `BINARY_VERSION` 均为 `0.0.3`，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`。
...
- 命令执行记录：首次包装 `packaged-manifest --check-registry` 时使用了 zsh 只读变量名 `status` 导致 wrapper 失败；已立即改用 `rc` 重跑同一只读 registry check，结果按预期 exit 1 并通过 wrapper 断言。
- 边界保持：本轮未执行真实 `npm publish`，未运行真实 install，未修改 `apps/electron/package.json` / `bun.lock` 以新增真实 optionalDependencies，未修改 `apps/electron/electron-builder.yml`，未创建 packaged app / native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。
- 验证通过：`git diff --check` 无输出；禁改文件 diff 无输出；`apps/electron/package.json` / `bun.lock` 中无真实 `@codeinsights/native-search-*` optionalDependencies；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；active Rust / Go 文档不再保留 “最新恢复入口 = 5bc9ebe8” 的陈旧主字段。

## 2026-06-08 Rust/Go Phase 5 5bc9ebe8 恢复与 gate 前 check-in 计划

范围确认：继续 `rust-go-refactor` 分支 Phase 5 “Rust search sidecar 试点”。本轮启动已按要求读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、development checklist、sidecar protocol / smoke plan 和 `native/search/`，并运行 `git status --short --branch` 与 `git log -25 --oneline`。当前 HEAD 为 `5bc9ebe8 docs(rust-go): 回填 bcc65b3d Phase 5 恢复入口`，工作树启动时干净；最新实现基线仍为 `63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest`。本轮已按用户要求只做状态同步：把活跃 Rust / Go 状态文档的最新已确认状态同步 / 恢复入口从 `bcc65b3d` 回填到 `5bc9ebe8`，并继续标注 Phase 5 已完成 / 未完成边界。未获用户明确批准前，不执行真实 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。

- [x] 完成启动读取、`native/search` 文件列表与关键源码核对。
- [x] 运行 `git status --short --branch`，确认当前分支 `rust-go-refactor` 且启动工作树干净。
- [x] 运行 `git log -25 --oneline`，确认最新状态同步提交为 `5bc9ebe8`，最近历史包含 `bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3` 和 `63e0fc2b`。
- [x] 确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，`tail_jsonl` 仍未进入最小 search sidecar 切片。
- [x] 记录实现前 check-in 计划与 no-go 边界。
- [x] 等待用户明确确认下一步：只做 `5bc9ebe8` 恢复入口自引用回填 / 只读复核，或批准进入最小真实 optional package gate。
- [x] 若只获准做状态回填：更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和必要 lessons，把最新已确认恢复入口从 `bcc65b3d` 回填为 `5bc9ebe8`，并运行只读 readiness/no-go 与禁改扫描。
- [ ] 若明确批准真实 optional package gate：按当前平台 / 计划包范围执行用户批准的真实 `npm publish` 命令，并在执行后用 `packaged-manifest --check-registry` 获取 registry exact-version evidence；仍不进入 install-chain / builder / packaged smoke，除非再获明确批准。
- [x] 运行只读 readiness/no-go 验证：`optional-package-source --native-search-package-version 0.0.3`、显式 `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`、默认离线 `packaged-manifest`，以及预期失败的 `packaged-manifest --check-registry` evidence。
- [x] 运行禁改扫描：`git diff --check`、禁改文件 diff、真实 native optionalDependencies 扫描和 packaged native binary / package manifest 产物扫描。
- [x] 单独提交本轮文档状态同步；提交后最终回复给出提交 hash、验证结果和可直接复制的下次启动提示词。
...
- 启动恢复完成：当前分支为 `rust-go-refactor`，启动 HEAD 为 `5bc9ebe8 docs(rust-go): 回填 bcc65b3d Phase 5 恢复入口`，最近历史包含 `5bc9ebe8`、`bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5` 和 `63e0fc2b`；工作树启动时干净。
- 文档状态已回填：development checklist、sidecar protocol / smoke plan 和 `next-session-prompt.md` 已把最新已确认状态同步 / 恢复入口推进到 `5bc9ebe8`，并继续保留 `63e0fc2b` 为最新实现基线。`bcc65b3d`、`e2b620ba` 与 `baa0ae3d` 仍作为历史状态同步提交记录，不再作为当前恢复入口。
- 习惯固化：`tasks/lessons.md` 已记录用户要求的固定习惯：每个阶段性任务完成并验证后，自动同步 development checklist、`next-session-prompt.md`、sidecar / smoke plan、`tasks/todo.md` Review 和必要 lessons，运行 no-go 验证，单独提交状态同步文档，并在最终回复给出可直接复制的下次启动提示词。
- 只读 readiness 结果：`optional-package-source --native-search-package-version 0.0.3` 通过，summary 显示 `optionalPackageSourceReady=true`、`optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`；显式 registry dry-run `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 通过，summary 显示 `optionalPackagePublishTargetReady=true`、4 个 planned package 目标版本无 collision / invalid / unavailable package，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`。
- no-go evidence：默认离线 `packaged-manifest` 通过，继续显示 `optionalPackagePublicationChecked=false`、`optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`bundledBinaryVerified=false`、`realPackagedBinaryVerified=false`、`defaultEnableCandidate=false`，且 `release-handoff-no-go-boundary-audit` passed。显式 `packaged-manifest --check-registry` 按预期 exit 1，summary 显示 `optionalPackagePublicationChecked=true`、`optionalPackagesPublished=false`，4 个 planned packages 均在 `missingPublishedPackages`，说明真实 publication 尚未完成。
- 边界保持：本轮未执行真实 `npm publish`，未运行真实 install，未修改 `apps/electron/package.json` / `bun.lock` 以新增真实 optionalDependencies，未修改 `apps/electron/electron-builder.yml`，未创建 packaged app / native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。
- 验证通过：`git diff --check` 无输出；禁改文件 diff 无输出；`apps/electron/package.json` / `bun.lock` 中无真实 `@codeinsights/native-search-*` optionalDependencies；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；只读 explorer 复核确认主要 Rust / Go 文档和 `tasks/lessons.md` 已同步到 `5bc9ebe8`，本 Review 已修正 `tasks/todo.md` 顶部执行前语境残留。

## 2026-06-08 Rust/Go Phase 5 bcc65b3d 恢复与三 gate 收口计划

范围确认：继续 `rust-go-refactor` 分支 Phase 5 “Rust search sidecar 试点”。启动已按要求读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、development checklist、sidecar protocol / smoke plan 和 `native/search/`，并运行 `git status --short --branch` 与 `git log -25 --oneline`。当前 HEAD 为 `bcc65b3d docs(rust-go): 回填 e2b620ba Phase 5 恢复入口`，工作树启动时干净，最新实现基线仍为 `63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest`。当前文档正文仍多处把最新已确认状态同步 / 恢复入口写为 `e2b620ba`，因此本轮先做恢复入口回填、只读 readiness/no-go 复核，并停在真实 gate 批准点前。未获用户明确批准前，不执行真实 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。

- [x] 完成启动读取、`native/search` 文件列表核对、`git status --short --branch` 与 `git log -25 --oneline`。
- [x] 确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`。
- [x] 等待只读 explorer 回传文档漂移与 gate 边界复核结果。
- [x] 更新 Rust / Go development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和必要 lessons，把最新已确认恢复入口从 `e2b620ba` 回填为 `bcc65b3d`，并继续标注 Phase 5 已完成 / 未完成边界。
- [x] 运行只读 readiness/no-go 验证：`optional-package-source --native-search-package-version 0.0.3`、显式 `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`、默认离线 `packaged-manifest`，以及预期失败的 `packaged-manifest --check-registry` evidence。
- [x] 运行禁改扫描：`git diff --check`、禁改文件 diff、真实 native optionalDependencies 扫描和 packaged native binary / package manifest 产物扫描。
- [x] 在本 Review 记录验证结果、真实未完成项和需要用户明确批准后才能执行的最小 publication gate。
- [x] 单独提交本轮文档状态同步；提交后最终回复给出提交 hash、验证结果和可直接复制的下次启动提示词。
...
- 启动恢复完成：当前分支为 `rust-go-refactor`，启动 HEAD 为 `bcc65b3d docs(rust-go): 回填 e2b620ba Phase 5 恢复入口`，最近历史包含 `bcc65b3d`、`e2b620ba`、`baa0ae3d`、`7c8057e3`、`885e89b5`、`63e0fc2b`、`345dcf01`、`8a0bcfd5`、`7898dae6`、`461295b0`；工作树启动时干净。
- 子代理复核：只读 explorer 确认 active Rust / Go 状态文档仍需从 `e2b620ba` 回填到 `bcc65b3d`；另一个只读 explorer 确认 `native/search/Cargo.toml` package version 与 `native/search/src/lib.rs` 的 `BINARY_VERSION` 均为 `0.0.3`，并列出 only-readiness smoke 入口与必须等待明确批准的真实 publish / install / builder / packaged smoke 边界。
- 文档状态已回填：development checklist、sidecar protocol / smoke plan 和 `next-session-prompt.md` 已把最新已确认状态同步 / 恢复入口推进到 `bcc65b3d`，并继续保留 `63e0fc2b` 为最新实现基线。`e2b620ba` 与 `baa0ae3d` 仍作为历史状态同步提交记录，不再作为当前恢复入口。
- 只读 readiness 结果：`optional-package-source --native-search-package-version 0.0.3` 通过，summary 显示 `optionalPackageSourceChecked=true`、`optionalPackageSourceReady=true`、4 个 planned package source ready，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`；显式 registry dry-run `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 通过，summary 显示 `optionalPackagePublishTargetChecked=true`、`optionalPackagePublishTargetReady=true`、4 个 planned package 目标版本无 collision / invalid / unavailable package，且仍 `optionalPackagesPublished=false`、`realPackagedBinaryVerified=false`。
- no-go evidence：默认离线 `packaged-manifest` 通过，继续显示 `optionalPackagePublicationChecked=false`、`optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`optionalDependenciesLockfileVerified=false`、`optionalDependenciesInstalledPackagesVerified=false`、`packagingConfigVerified=false`、`bundledBinaryVerified=false`、`realPackagedBinaryVerified=false`、`defaultEnableCandidate=false`，且 `release-handoff-no-go-boundary-audit` passed。显式 `packaged-manifest --check-registry` 按预期 exit 1，summary 显示 `optionalPackagePublicationChecked=true`、`optionalPackagesPublished=false`，4 个 planned packages 均在 `missingPublishedPackages`，说明真实 publication 尚未完成。
- 边界保持：本轮未执行真实 `npm publish`，未运行真实 install，未修改 `apps/electron/package.json` / `bun.lock` 以新增真实 optionalDependencies，未修改 `apps/electron/electron-builder.yml`，未创建 packaged app / native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR；native 继续 default off / 显式 opt-in，default-enable 继续暂缓。
- 验证通过：`git diff --check` 无输出；禁改文件 diff 无输出；`apps/electron/package.json` / `bun.lock` 中无真实 `@codeinsights/native-search-*` optionalDependencies；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；精确陈旧入口扫描未发现 active 文档仍把 `e2b620ba` 写成最新恢复入口。
...
- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，覆盖默认 blocked、declaration ready、install-chain ready、default-enable blocked 和手工注入越界命令 / verified flag 的失败场景。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 导出 `evaluateNativeSearchReleaseHandoffNoGoBoundary()` 和稳定 reason code，只读消费 summary，不联网、不写文件、不读取 binary path。
- [x] 保持 native default off / 显式 opt-in；blocked / future gate 不暴露 candidate / post-execution command；declaration 不混入 install；install-chain 只在当前 ready gate 批准后暴露 `bun install --frozen-lockfile`；default-enable 当前仍 blocked。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不得新增真实 `@codeinsights/native-search-*` optionalDependencies。
- [x] 运行聚焦测试、smoke、typecheck 和 no-go 边界验证。
- [x] 阶段完成后更新本 Review；development checklist、sidecar protocol / smoke plan、next-session-prompt.md 和必要 lessons 在实现提交后单独做状态同步提交。

### Review

- 已新增 `evaluateNativeSearchReleaseHandoffNoGoBoundary()` 只读审计 helper，返回 `passed`、当前 gate、default-off / explicit-opt-in 状态和稳定 violation reason code；helper 不接入 CLI summary 输出，不执行命令、不联网、不写文件、不读取 binary path。
- 审计覆盖 blocked / future gate 命令外溢、blocked execution packet 命令外溢、declaration gate 混入 install、install-chain ready 缺少 `bun install --frozen-lockfile` 或副作用标记、packaged smoke 占位命令、default-enable gate 当前必须 blocked、提前 packaged verified flag 和敏感路径 / registry 字符串泄露。
- BDD 覆盖默认离线 blocked summary、publication 后 declaration ready、install-chain ready、packaged smoke 后 default-enable blocked，以及手工注入越界命令 / `bundledBinaryVerified=true` 的失败场景。
- 版本同步：`@codeinsights/electron` 从 `0.0.175` 递增到 `0.0.176` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts`（82 pass）；`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（125 pass）；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode native-missing`；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
...
- Code review：code-reviewer 首轮指出 blocked ready command 校验、packaged smoke placeholder / post-verification command 和 candidate command array aliasing 风险；已补测试并修复。复审返回 no findings。
...
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前分支、干净工作树和最近历史。
- [x] 启动两个只读 explorer 子代理，分别复核 no-go 下的下一步代码切片和 native package metadata / source 证据缺口；第一个 explorer 建议把切片从“批准前复核命令 metadata”调整为“执行后 transition verifier”，已停止原方向并重规划。

实现计划：

- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 publication gate 执行后只推进到 `optional_dependencies_declaration`，且 optionalDependencies / lockfile / builder / packaged binary / default-enable 仍必须未验证。
- [x] 补充 declaration / packaging / packaged smoke transition 失败用例：declaration 不能提前把 lockfile / installed package 置 true；packaging config 不能提前把 bundled binary 置 true；packaged smoke 可以把 bundled binary 置 true，但 default-enable 必须保持 false。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 中导出纯函数 `evaluateNativeSearchReleaseHandoffGateTransition()`，只消费 before / after summary 的 release handoff checklist 与 summary flags，不查询 registry、不发布、不安装、不写 builder / package manifest / lockfile、不读取 binary path。
...
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native search optionalDependencies。
...
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，确认批准前复核命令不等于真实 publish。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
...
- 实现结果：新增 `evaluateNativeSearchReleaseHandoffGateTransition()`，把 release handoff checklist 中的 `expectedNextGateAfterVerification`、`expectedSummaryFlagsAfterExecution`、`mustRemainUnverifiedAfterExecution` 转成 before / after summary 的可执行 fail-closed 校验；`default_enable_risk_review` 仍在当前 smoke 中 fail-closed。
- 实现提交：`66da7eba feat(rust-go): 补齐 Phase 5 gate 迁移校验器`。
- Gate 顺序：已覆盖 publication、optionalDependencies declaration、install-chain、packaging config 和 packaged smoke 的合规迁移；同时拒绝跳 gate、提前证明 lockfile / installed package / packaging config / packaged binary，以及 blocked packaged smoke 只靠顶层布尔值绕过。
- 中间 gate readiness 修正：`optional_dependencies_declaration`、`optional_package_install_chain`、`packaging_config_allowlist` 现在在各自前置 readiness 满足时可以成为 `ready_for_approval`，但不把任何执行证据标成已完成。
- Reviewer 复核：code-reviewer 二次审查无 critical / warning；剩余建议是后续可把 declaration approval 与 install-chain dry-run candidate command 拆得更细，本轮不影响 no-go。
- 版本同步：`@codeinsights/electron` 从 `0.0.173` 递增到 `0.0.174` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（114 pass）；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`。
- 预期 no-go 验证通过：`smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，原因是 4 个计划 optional package 仍未真实发布；summary 仍保持 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false`、`defaultEnableCandidate=false`。
- 禁改边界保持：`git diff --check` 无输出；根 `README.md`、根 `AGENTS.md`、`apps/electron/electron-builder.yml` 无 diff；`apps/electron/package.json` / `bun.lock` 未新增真实 native search optionalDependencies；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 packaged native binary 或 package manifest 产物；未 push，未创建 PR。
- 状态同步：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review 已回填 `66da7eba` 最新实现基线、`aa0091a3` 当前已存在恢复入口、transition verifier 边界、未完成真实 optional / packaged gates 和下次启动提示词；本状态同步将单独提交，提交后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前分支、干净工作树和最近历史。
- [x] 启动两个只读 explorer 子代理，分别复核 no-go 下的最小代码切片和阶段完成后的验证 / 文档同步要求；收到第一个 explorer 结果后按“出现偏差立即重规划”要求，将切片从执行前 readiness 调整为执行后状态迁移期望。

实现计划：

- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 `gateExecutionEvidenceChecklist` 的状态迁移期望：publication gate 验证后只应推进到 `optional_dependencies_declaration`，且 optionalDependencies / install-chain / builder / packaged binary / default-enable 仍必须未验证；后续 gate 的 expected next gate 必须按顺序推进，不能跳 gate。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 中为 `gateExecutionEvidenceChecklist` 新增 `expectedNextGateAfterVerification`、`expectedSummaryFlagsAfterExecution`、`mustRemainUnverifiedAfterExecution` 和 `transitionFailClosedCriteria`，只读描述批准执行并验证后应出现的 summary 迁移，不查询 registry、不发布、不安装、不写 builder / package manifest / lockfile、不读取 binary path。
- [x] 保持当前仓库默认输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native search optionalDependencies。
...
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，确认 transition expectation 只描述 publication gate 验证后应推进到 optionalDependencies declaration，不把命令或期望字段当成真实发布。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`b874628b feat(rust-go): 补齐 Phase 5 gate 状态迁移期望`。
- 已在 `nativeSearchReleaseHandoffPlan.gateExecutionEvidenceChecklist` 中新增 `expectedNextGateAfterVerification`、`expectedSummaryFlagsAfterExecution`、`mustRemainUnverifiedAfterExecution` 和 `transitionFailClosedCriteria`，用于描述真实 gate 批准执行并验证后应出现的 summary 迁移。
- 迁移顺序已锁定：`optional_package_publication -> optional_dependencies_declaration -> optional_package_install_chain -> packaging_config_allowlist -> packaged_app_bundled_binary_smoke -> default_enable_risk_review -> complete`。publication / declaration gate 的 `mustRemainUnverifiedAfterExecution` 已覆盖 lockfile / installed package 子证据，避免把声明阶段误判为 install-chain 已完成。
- 边界语义保持：transition expectation 只做后续真实 gate 执行后的 fail-closed 验收输入，不驱动 `nextGate`、`verified`、candidate command、真实 publication、真实 optionalDependencies install-chain、builder allowlist、packaged binary 或 default-enable。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.173` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（104 pass）；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；显式 registry dry-run `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 预期 no-go 验证通过：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未真实发布。
- 禁改边界保持：native 继续 default off / 显式 opt-in；默认 smoke 不联网；本轮未执行 `npm publish`，未运行真实 install，未修改 `apps/electron/electron-builder.yml`，未新增真实 native search optionalDependencies，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR。
- 禁改扫描通过：`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`apps/electron/package.json` 和 `bun.lock` 中未新增真实 native search optionalDependencies；排除 `.git` / `node_modules` / `apps/electron/node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`native-search` 或 `native-search.exe` packaged 产物。
- code review：只读 code-review 子代理未发现 P0/P1/P2；剩余风险是 transition checklist 仍是描述性 metadata，未来若接入真实 executor，必须增加执行后 summary 与 expected / must-remain 字段的实际比较。
- 状态同步：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review 已回填 `b874628b` 最新开发基线、transition expectation 边界、未完成真实 optional / packaged gates 和下次启动提示词；本状态同步将单独提交。

## 2026-06-07 Rust/Go Phase 5 release handoff gate 执行证据清单计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/` 源码；当前分支为 `rust-go-refactor`，工作树起始干净，启动 HEAD 为 `bc53cc75 docs(rust-go): 回填 Phase 5 批准队列最新恢复入口`，最新开发基线为 `197569d5 feat(rust-go): 补齐 Phase 5 release handoff 批准队列`，最近历史包含 `bc53cc75`、`9d8520fa`、`197569d5`、`f78c44d6`、`bd0e61d3`、`99d76b0c`。本轮只推进真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改和真实 packaged app bundled binary smoke 之前的批准 / 证据准备：在 `nativeSearchReleaseHandoffPlan` 中新增只读 gate execution evidence checklist，明确每个真实 gate 在批准后必须产出的证据、当前 gate 可执行命令可见性、未来 gate fail-closed、以及不等于真实 verified 的边界。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前分支、干净工作树和最近历史。
- [x] 启动两个只读 explorer 子代理，分别复核 Phase 5 no-go 下的下一步切片和 smoke summary 最小扩展点；主线不等待其结果阻塞初始代码阅读。

实现计划：

- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 release handoff gate execution evidence checklist：默认 blocked 时不输出当前可执行命令；publication ready 时只给当前 publication gate 的批准后验证命令；后续 gate 必须继续无执行命令且不得被当成真实 verified。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 中为 `nativeSearchReleaseHandoffPlan` 新增只读 `gateExecutionEvidenceChecklist`，复用现有 approval queue / invocation / install-chain / builder / packaged smoke plan，不查询 registry、不发布、不安装、不写 builder / package manifest / lockfile、不读取 binary path。
- [x] 保持当前仓库默认输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 根据 code review 修正 default-enable risk review gate 的 future `networkRequired` 副作用标记，并补 full gate `networkRequired` 回归断言。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native search optionalDependencies。
...
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，确认 checklist 只让当前 publication gate 暴露批准后验证命令，不把命令当成真实发布。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`cb30b819 feat(rust-go): 补齐 Phase 5 release handoff 执行证据清单`；修正提交：`72a00b01 fix(rust-go): 标记 default enable 证据清单网络边界`。
- 已新增 `nativeSearchReleaseHandoffPlan.gateExecutionEvidenceChecklist`，从 `gateApprovalQueue` 只读派生每个 gate 的 `requiredEvidenceAfterExecution`、`postExecutionVerificationCommands`、`fileWriteTargets`、`remoteWriteRequired`、`workspaceMutationRequired`、`networkRequired`、`successCriteria`、`failClosedCriteria`、`doesNotVerify` 和 `forbiddenActions`。
- 边界语义已收紧：默认 blocked 状态下所有 gate 都不暴露 post-execution command；publication ready 时只有当前 `optional_package_publication` gate 暴露批准后的 `packaged-manifest --check-registry` 验证命令；future gate 继续 `blocked_until_prior_gate_verified` 且 `postExecutionVerificationCommands=[]`；`default_enable_risk_review` 因未来验证设计包含 `--check-registry`，现已标记 `networkRequired=true`，但仍不暴露执行命令。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.172` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（103 pass）；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；显式 registry dry-run `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 预期 no-go 验证通过：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未真实发布。
- 边界保持：native 继续 default off / 显式 opt-in；本轮未执行 `npm publish`，未运行真实 install，未修改 `apps/electron/electron-builder.yml`，未新增真实 native search optionalDependencies，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR。
- 禁改验证通过：`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`。
- code review：子代理发现 default-enable risk review 的 network side-effect 低报问题；`72a00b01` 已修复并通过复审，无剩余 findings。
- 状态同步：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review 已回填 `72a00b01` 最新开发基线、`gateExecutionEvidenceChecklist` 边界、未完成真实 optional / packaged gates，以及下次启动提示词；本状态同步将单独提交。

## 2026-06-07 Rust/Go Phase 5 最新恢复入口回填与启动提示词同步计划

范围确认：用户要求更新文档最新开发状态，清楚标注完成 / 未完成，确保下次启动可以跟踪进度继续开发，并再次要求把该习惯固定为每个阶段性任务完成后的自动动作。本轮只做 docs / tasks 状态同步：当前分支为 `rust-go-refactor`，工作树起始干净，当前 HEAD 为 `9d8520fa docs(rust-go): 同步 Phase 5 release handoff 批准队列状态`，最新开发基线为 `197569d5 feat(rust-go): 补齐 Phase 5 release handoff 批准队列`。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

- [x] 运行 `git status --short --branch` 和 `git log -8 --oneline`，确认当前分支、干净工作树、最新 docs HEAD `9d8520fa` 与最新开发基线 `197569d5`。
- [x] 更新 development checklist 顶部状态和底部可复制提示词，把最新已确认恢复入口推进到 `9d8520fa`，并保持真实 optional / packaged / default-enable gates 未完成。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md`，标注 Phase 0-4 已完成、Phase 5 已完成到 release handoff 全 gate 批准队列状态同步，真实 optional package 发布 / install-chain / builder / packaged smoke / default enable 仍未完成。
- [x] 更新 `docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md`，把最新状态同步推进到 `9d8520fa`，继续说明 approval packet / approval queue 不等于真实 publication / install-chain / builder / packaged evidence。
- [x] 更新 `tasks/lessons.md`，把阶段完成后自动状态同步与给出可复制下次启动提示词的习惯继续固化。
- [x] 更新本 Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改文件 diff 检查、真实 native optionalDependencies 扫描、packaged native binary 产物扫描和 `git status --short --branch`。
- [x] 单独提交本轮状态同步文档，并在最终回复给出实际 docs HEAD 与可复制下次启动提示词。

### Review

- 文档校正：已将 Rust / Go development checklist、sidecar protocol / smoke plan 和 `next-session-prompt.md` 的最新已确认恢复入口推进到 `9d8520fa docs(rust-go): 同步 Phase 5 release handoff 批准队列状态`，并保留“本轮再次提交后以下次 `git log -5 --oneline` 最新 Rust / Go docs 提交为准”的恢复规则。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 release handoff 全 gate 批准队列和对应状态同步。真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 习惯固化：已补 `tasks/lessons.md`，再次记录每个阶段完成并验证通过后，自动更新 development checklist、`next-session-prompt.md`、`tasks/todo.md` Review 和必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档；最终回复给出实际 docs HEAD 和可复制下次启动提示词。
...
- 验证通过：`git diff --check` 无输出；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`。
- 提交状态：本轮状态同步文档将单独提交；提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。

## 2026-06-07 Rust/Go Phase 5 release handoff 全 gate 批准队列计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、development checklist、sidecar protocol / smoke plan 和 `native/search/`；当前分支为 `rust-go-refactor`，工作树起始干净，当前 HEAD 为 `f78c44d6 docs(rust-go): 回填 Phase 5 最新恢复入口`，最新开发基线保持 `99d76b0c feat(rust-go): 补齐 Phase 5 release handoff 批准包`，最近历史包含 `f78c44d6`、`bd0e61d3`、`99d76b0c`、`3089b768`、`f02ceae0`、`ddc818ae`。本轮只推进真实 optional package 发布、optionalDependencies 实际声明 / 安装执行、builder allowlist 实际修改和真实 packaged app bundled binary smoke 之前的批准准备：在 `nativeSearchReleaseHandoffPlan` 中新增全 gate approval queue，明确每个后续真实 gate 的顺序位置、批准项、候选命令可见性、未到达 gate 的 fail-closed 状态和禁止动作。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前分支、干净工作树和最近历史。
- [x] 确认本轮不使用子代理工具：当前可用子代理工具要求只有用户明确要求并行 agent work 时才能 spawn；本轮改由主线本地完成。

实现计划：

- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 release handoff 全 gate approval queue：当前 gate 才能暴露候选命令；后续 gate 必须 `blocked_until_prior_gate_verified` 且无候选命令；已完成 gate 只能标为 `verified`，不得授权重复执行。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 中为 `nativeSearchReleaseHandoffPlan` 新增 `gateApprovalQueue`，复用现有 execution / invocation / handoff plan，不查询 registry、不发布、不安装、不写 builder / package manifest / lockfile、不读取 binary path。
- [x] 保持当前仓库默认输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
...
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，确认 queue 只让当前 publication gate ready，不把候选命令当成真实发布。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`197569d5 feat(rust-go): 补齐 Phase 5 release handoff 批准队列`。
- 已新增 `nativeSearchReleaseHandoffPlan.gateApprovalQueue`，按 `optional_package_publication -> optional_dependencies_declaration -> optional_package_install_chain -> packaging_config_allowlist -> packaged_app_bundled_binary_smoke -> default_enable_risk_review` 输出全 gate approval matrix。
- 队列语义已收紧：只有当前且 ready 的 gate 会输出 `status="ready_for_approval"`、`approvalRequired=true` 和候选命令；后续 gate 必须保持 `blocked_until_prior_gate_verified` 且 `candidateCommands=[]`；已完成 gate 只能标为 `verified`，不得授权重复执行。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.170` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（103 pass）；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；显式 registry dry-run `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 预期 no-go 验证通过：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未真实发布。
- 边界保持：native 继续 default off / 显式 opt-in；本轮未执行 `npm publish`，未运行真实 install，未修改 `apps/electron/electron-builder.yml`，未新增真实 native search optionalDependencies，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR。
- 禁改验证通过：`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`。
- 状态同步：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review 已回填 `197569d5` 最新开发基线、`gateApprovalQueue` 边界、未完成真实 optional / packaged gates，以及下次启动提示词；本状态同步将单独提交。

## 2026-06-07 Rust/Go Phase 5 最新状态回填与启动提示词同步计划

范围确认：用户要求更新文档最新开发状态，清楚标注完成 / 未完成，确保下次启动能继续开发，并要求把该习惯固定为每个阶段性任务完成后的自动动作。本轮只做 docs / tasks 状态回填：当前分支为 `rust-go-refactor`，工作树起始干净，当前 HEAD 为 `bd0e61d3 docs(rust-go): 同步 Phase 5 release handoff 批准包状态`，最新开发基线为 `99d76b0c feat(rust-go): 补齐 Phase 5 release handoff 批准包`。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`，并运行 `git status --short --branch` 与 `git log -25 --oneline`。
- [x] 确认本轮不使用子代理工具：当前工具约束要求只有用户显式要求并行 agent work 时才能 spawn；本轮改由主线本地完成。
- [x] 更新 development checklist 顶部状态、当前完成 / 未完成清单和底部可复制提示词，把最新已确认恢复入口推进到 `bd0e61d3`，并明确真实 optional / packaged / default-enable gates 仍未完成。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md`，标注 Phase 0-4 已完成、Phase 5 已完成到 release handoff 批准包状态同步，真实 optional package 发布 / install-chain / builder / packaged smoke / default enable 仍未完成。
- [x] 更新 `docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md`，把最新状态同步推进到 `bd0e61d3`，并继续说明 approval packet 不等于真实 publication / install-chain / builder / packaged evidence。
- [x] 更新 `tasks/lessons.md`，固化阶段完成后自动状态同步和最终回复给出可复制下次启动提示词的习惯。
- [x] 更新本 Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改文件 diff 检查、真实 native optionalDependencies 扫描、packaged native binary 产物扫描和 `git status --short --branch`。
- [x] 单独提交本轮状态同步文档。
...
- 文档校正：已将 Rust / Go development checklist、sidecar protocol / smoke plan 和 `next-session-prompt.md` 的最新已确认恢复入口推进到 `bd0e61d3 docs(rust-go): 同步 Phase 5 release handoff 批准包状态`，并保留“本轮再次提交后以下次 `git log -5 --oneline` 最新 Rust / Go docs 提交为准”的恢复规则。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 release handoff 批准包和对应状态同步。真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 习惯固化：已补 `tasks/lessons.md`，把“每个阶段性任务完成并验证通过后，自动更新 development checklist、`next-session-prompt.md`、`tasks/todo.md` Review 和必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档；最终回复给出实际 docs HEAD 和可复制下次启动提示词”作为默认动作。
- 边界保持：native 继续 default off / 显式 opt-in；本轮只做文档 / 任务状态同步，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未新增真实 `@codeinsights/native-search-*` optionalDependencies，未 push，未创建 PR。
- 验证通过：`git diff --check` 无输出；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；`git status --short --branch` 仅显示本轮 5 个 docs / tasks 文件。
- 提交状态：本轮状态同步文档已单独提交；提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。

## 2026-06-06 Rust/Go Phase 5 release handoff 批准包前置计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，当前 HEAD 为 `3089b768 docs(rust-go): 回填 Phase 5 最新状态和启动提示词`，最新开发基线为 `ddc818ae feat(rust-go): 补齐 Phase 5 builder allowlist 修改意图`，最新实际恢复入口为 `3089b768`。本轮只推进真实 optional package 发布、optionalDependencies 声明 / 安装、builder allowlist 实际修改和 packaged app bundled binary smoke 之前的批准准备：在 `nativeSearchReleaseHandoffPlan` 中新增机器可读 current gate approval packet，明确当前 next gate 的批准项、前置证据、候选命令、授权边界和禁止动作。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认最近历史包含 `3089b768`、`f02ceae0`、`ddc818ae`、`c1890a04`、`8ed8c1c5` 和 `3f5a0783`。
- [x] 确认本轮不使用子代理工具：当前工具约束要求只有用户显式要求并行 agent work 时才能 spawn；本轮改由主线本地完成。

实现计划：

- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 release handoff approval packet：当前 `optional_package_publication` gate ready 时只能授权 publish 调用审核，不授权 package.json / bun.lock / builder / packaged smoke / default enable；blocked gate 时不得输出可执行批准。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 中为 `nativeSearchReleaseHandoffPlan` 新增 `currentGateApprovalPacket`，只消费现有 execution / invocation / handoff plan，不查询 registry、不发布、不安装、不写 builder / package manifest / lockfile、不读取 binary path。
- [x] 保持当前仓库默认输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native optionalDependencies。
...
- [x] 运行 `bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 与 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`，确认 handoff 只进入批准准备状态，不把 ready 当成真实发布。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`99d76b0c feat(rust-go): 补齐 Phase 5 release handoff 批准包`。
- 已在 `nativeSearchReleaseHandoffPlan` 中新增 `currentGateApprovalPacket`，按 `optionalPackageExecutionPlan.nextStage` 输出当前 gate 的批准状态、所需 approval、前置证据、候选命令、授权边界和禁止动作。
- blocked gate 现在明确 `approvalRequired=false` 且 `candidateCommands=[]`；显式 publish-target registry dry-run 和 source preflight 都 ready 时，当前 gate 进入 `optional_package_publication` / `ready_for_approval`，但只授权 release approval 后的候选 `npm publish <native-search-package-source:...> --access public` 调用审核，不授权 package.json / bun.lock / builder / packaged app / default enable。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.169` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（103 pass）；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；显式 registry dry-run `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 预期 no-go 验证通过：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未真实发布。
- 边界保持：native 继续 default off / 显式 opt-in；本轮未执行 `npm publish`，未运行真实 install，未修改 `apps/electron/electron-builder.yml`，未新增真实 native search optionalDependencies，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR。
- 禁改验证通过：`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`。
- 状态同步：development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review 已回填 `99d76b0c` 最新开发基线、`currentGateApprovalPacket` 边界、未完成真实 optional / packaged gates，以及下次启动提示词；本状态同步将单独提交。

## 2026-06-06 Rust/Go Phase 5 最新状态回填与启动提示词同步计划

范围确认：用户要求更新文档最新开发状态，清楚标注完成 / 未完成，确保下次启动能继续跟踪进度，并给出可直接复制的下次启动提示词；同时要求把该习惯固定为每个阶段性任务完成后的自动动作。本轮只做 docs / tasks 状态回填：当前分支为 `rust-go-refactor`，当前 HEAD 为 `f02ceae0 docs(rust-go): 同步 Phase 5 builder allowlist 修改意图状态`，最新开发基线为 `ddc818ae feat(rust-go): 补齐 Phase 5 builder allowlist 修改意图`。继续保持 native default off / 显式 opt-in；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

- [x] 运行 `git status --short --branch` 和 `git log -12 --oneline`，确认当前分支、HEAD 与最近 Rust / Go 提交历史。
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、development checklist、sidecar protocol / smoke plan 和 `next-session-prompt.md`，确认仍需要把恢复入口从过渡说明推进到 `f02ceae0`。
- [x] 更新 development checklist 顶部状态、当前阶段完成状态和底部可复制提示词，明确 `ddc818ae` 是最新开发基线、`f02ceae0` 是当前已确认恢复入口。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md`，标注 Phase 0-4 已完成、Phase 5 已完成到 builder allowlist 修改意图状态同步，真实 optional / packaged / default-enable gates 未完成。
- [x] 更新 `docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md`，补充最新状态同步提交不等于真实发布、安装、builder 修改或 packaged smoke。
- [x] 更新 `tasks/lessons.md`，固化阶段完成后自动状态同步、no-go 验证、单独提交和最终回复给出可复制提示词的习惯。
- [x] 更新本 Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改文件 diff 检查、真实 native optionalDependencies 扫描、packaged native binary 产物扫描和 `git status --short --branch`。
- [x] 单独提交本轮状态同步文档。
...
- 文档校正：已将 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `c1890a04 docs(rust-go): 回填 Phase 5 fail-closed 最新恢复入口` 推进到 `f02ceae0 docs(rust-go): 同步 Phase 5 builder allowlist 修改意图状态`，并保留“本轮再次提交后以下次 `git log -5 --oneline` 最新 Rust / Go docs 提交为准”的恢复规则。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 builder allowlist 修改意图状态同步。真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 习惯固化：已补 `tasks/lessons.md`，把“每个阶段性任务完成并验证通过后，自动更新 development checklist、`next-session-prompt.md`、`tasks/todo.md` Review 和必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档”作为默认动作。
- 边界保持：native 继续 default off / 显式 opt-in；本轮只做文档 / 任务状态同步，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未新增真实 `@codeinsights/native-search-*` optionalDependencies，未 push，未创建 PR。
- 验证通过：`git diff --check` 无输出；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`。
- 本轮状态同步提交完成后，下次启动以 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口；最终回复会给出本轮实际 HEAD 和可复制提示词。

## 2026-06-06 Rust/Go Phase 5 builder allowlist 实际修改前置证据计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。本轮实现前启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，当前 HEAD 为 `c1890a04 docs(rust-go): 回填 Phase 5 fail-closed 最新恢复入口`，最新开发基线保持 `3f5a0783 feat(rust-go): 收紧 Phase 5 release handoff fail-closed 证据`；实现提交后当前开发基线推进到 `ddc818ae feat(rust-go): 补齐 Phase 5 builder allowlist 修改意图`。本轮只推进 builder allowlist 实际修改前置工作：在不修改 `apps/electron/electron-builder.yml` 的前提下，把后续真实 builder allowlist 修改的精确 YAML edit intent、操作顺序、验收证据和禁止动作机器可读化，避免后续把 dry-run plan 或宽泛 include 当成 packaging verified。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认最近历史包含 `c1890a04`、`8ed8c1c5`、`3f5a0783`、`6bde0340`、`5bb2cebd` 和 `4fec7fab`。
- [x] 确认本轮不使用子代理工具：当前工具约束要求只有用户显式要求并行 agent work 时才能 spawn，本轮改由主线本地完成。

实现计划：

- [x] 测试先行扩展 `apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts` 与 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 builder allowlist 实际修改前置证据：精确 per-package include、阻断 exclude removal intent、禁止宽泛 include、验收命令和“不修改 builder / 不证明 packaged verified”语义。
- [x] 在 `apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.ts` 的 `packagingConfigAllowlistChangePlan` 中新增只读 candidate edit intent 字段；只消费当前 validator 结果，不写 `electron-builder.yml`，不发布、不安装、不读取 binary。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` summary 中透出该字段，当前仓库仍保持 `packagingConfigVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native optionalDependencies。

验证计划：

- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
...
- 实现提交：`ddc818ae feat(rust-go): 补齐 Phase 5 builder allowlist 修改意图`。
- 已新增 `NativeSearchPackagingConfigAllowlistCandidateYamlEditPlan` 和 `packagingConfigAllowlistChangePlan.candidateYamlEditPlan`，只读输出后续真实 `apps/electron/electron-builder.yml` 修改的目标文件、`modifiesFile=false`、操作顺序、待添加精确 per-package include、需要移除的阻断规则、禁止宽泛 include 和验收证据。
- 当前 `candidateYamlEditPlan.addIncludes` 仍为 4 个精确 package include，`removeRules` 仍包含 `!node_modules/@codeinsights/**`；这只说明 builder allowlist 实际修改的审查输入已机器可读化，不代表 `electron-builder.yml` 已修改，也不代表 `packagingConfigVerified=true`。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.168` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts`（99 pass）；`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`；预期 no-go `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，4 个计划 optional package 仍在 `missingPublishedOptionalPackages`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 边界保持：native 继续 default off / 显式 opt-in；本轮未修改 `apps/electron/electron-builder.yml`，未创建 packaged native binary，未执行 `npm publish`，未运行真实 install，未新增真实 native search optionalDependencies，未修改根 `README.md` / 根 `AGENTS.md`，未 push，未创建 PR。
- 禁改验证通过：`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock` 无匹配；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`。
- 本轮状态同步已更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 lessons；提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `6bde0340`，最近历史包含 `4fec7fab` 和 `5bb2cebd`。
- [x] 启动只读子代理复核 Phase 5 gate / no-go 现状；结论是本轮只能做不触碰真实发布、安装、builder 修改或 packaged binary 的 evidence tightening。

实现计划：

- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 release handoff 必须按 `optionalPackageExecutionPlan.nextStage` fail-closed，且 packaged smoke ready / verified 布尔值不能跳过 publish-target、source、publication、optionalDependencies 和 builder allowlist gate。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 中补充最小机器可读 evidence / design 字段或收紧现有字段：只消费现有 summary / gate，不查询 registry、不发布、不安装、不写 builder / package manifest / lockfile、不读取 binary path。
- [x] 保持当前仓库默认输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native optionalDependencies。
...
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`3f5a0783 feat(rust-go): 收紧 Phase 5 release handoff fail-closed 证据`。
- 已新增 `nativeSearchReleaseHandoffPlan.gateBinding`，把 release handoff 的 authoritative next gate 绑定到 `optionalPackageExecutionPlan.nextStage`，并要求 verified 绑定 `optionalPackageExecutionPlan.verified`；后置 publication / packaged smoke evidence 不能乱序跳过上游 gate。
- 已新增 `packagedBundledBinarySmokeInvocationPlan.executionDesign`，机器可读记录真实 packaged app smoke 的 preferred / legacy input mode、registry check、real packaged app requirement、临时 fixture 禁止当作真实 evidence，以及不创建 packaged app、不发布、不安装、不修改 builder 的边界。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.167` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts`（60 pass）；`bun test apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（103 pass）；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；显式 registry dry-run `smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 预期 no-go 验证通过：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，summary 包含 `optional_packages_not_published`、`bundledBinaryVerified=false` 和 `defaultEnableCandidate=false`。
- 禁改边界保持：根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml` 无 diff；未创建 packaged native binary；`apps/electron/package.json` / `bun.lock` 未出现真实 native search optionalDependencies；未 push，未创建 PR。
- 代码审查子代理结论：未发现阻断问题；剩余风险是仍需要真实 optional package 发布、optionalDependencies 实际声明 / 安装、builder allowlist 实际修改和真实 packaged app bundled binary smoke 后，才可进入最终 default-enable 风险决策。
- 本轮状态同步会单独提交文档；提交后最终回复给出实际 docs HEAD。

## 2026-06-06 Rust/Go Phase 5 release handoff 最新恢复入口回填计划

范围确认：用户要求更新 Rust / Go 文档最新开发状态，清楚标注完成 / 未完成，确保下次启动可以继续跟踪进度，并提供可直接复制的下次启动提示词；同时要求把“每个阶段性任务完成后自动同步文档状态”的习惯固化。本轮只做状态文档回填：当前分支为 `rust-go-refactor`，最新开发基线保持 `4fec7fab feat(rust-go): 补齐 Phase 5 release handoff 证据计划`，把当前 `git log` 已确认的最新 Rust / Go docs 状态同步提交 `5bb2cebd docs(rust-go): 同步 Phase 5 release handoff 状态` 写入 development checklist 和 `next-session-prompt.md`。继续保持 native default off / 显式 opt-in；不创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。

- [x] 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认当前分支为 `rust-go-refactor`，最近历史包含 `5bb2cebd`、`4fec7fab`、`6525f6fb`、`95305503` 和 `77b266e8`。
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `native/search/`，确认本轮只需做状态同步。
- [x] 更新 `tasks/lessons.md`，固化 `5bb2cebd` 是 `4fec7fab` 后当前已确认恢复入口，并再次记录阶段完成后自动同步 development checklist、next-session prompt、`tasks/todo.md` Review 和必要 lessons 的习惯。
- [x] 更新 development checklist 顶部状态、下次启动入口和底部可复制提示词，把最新已确认恢复入口从 `6525f6fb` 推进到 `5bb2cebd`。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md` 顶部状态和可复制提示词，把最新已确认恢复入口推进到 `5bb2cebd`。
- [x] 更新本 Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改文件 diff 检查、真实 native optionalDependencies 扫描、packaged native binary 产物扫描、恢复入口一致性扫描和 `git status --short --branch`。
- [x] 单独提交本轮状态同步文档。
...
- 文档校正：已将 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `6525f6fb docs(rust-go): 回填 Phase 5 publication invocation 最新恢复入口` 推进到 `5bb2cebd docs(rust-go): 同步 Phase 5 release handoff 状态`；最新开发基线保持 `4fec7fab feat(rust-go): 补齐 Phase 5 release handoff 证据计划`。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 native search sidecar、sidecar manager、fallback gate、benchmark gate、Agent nested parity、smoke/cache、optional package manifest / resolver / layout / evidence / identity、optionalDependencies declaration / install-chain preflight、default-enable readiness、packaging config allowlist preflight、publication preflight、packaged smoke plan、publish-target dry-run、source `BINARY_VERSION=0.0.3`、optional package source preflight、optional / packaged execution plan、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change / invocation plan、publication invocation 发布证据收紧和 release handoff 证据计划。真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 语义边界继续明确：native 必须继续 default off / 显式 opt-in；默认 smoke 不联网；`optionalPackageSourceReady=true`、`optionalPackagePublishTargetReady=true`、`ready_for_release_handoff`、`ready_for_invocation` 或 `ready_for_execution` 都不等于真实发布、安装、builder 修改、packaged verified 或 default-enable 可行；`publishedPackages` 只表示真实 registry publication evidence，collision / invalid / unavailable / duplicate metadata 只能进入 blocker / exclusion。真实 verified 仍必须同时满足 `bundledBinaryVerified=true` 且无 blocker。
- 已补 `tasks/lessons.md`：用户要求“更新文档最新开发状态、标注完成 / 未完成、给下次启动提示词”时，必须重新核对 `git status --short --branch` 与 `git log -25 --oneline` 并回填当前真实 docs HEAD；每个阶段性任务完成并通过验证后，默认更新 development checklist、`next-session-prompt.md`、`tasks/todo.md` Review 和必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档。
- 边界保持：本轮只做文档 / 任务状态同步，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未新增真实 `@codeinsights/native-search-*` optionalDependencies，未 push，未创建 PR。
- 验证通过：恢复入口一致性扫描显示 development checklist 与 `next-session-prompt.md` 的当前状态入口均指向 `5bb2cebd`；`git diff --check` 无输出；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`git diff -- apps/electron/package.json bun.lock | rg -n '@codeinsights/native-search|optionalDependencies|native-search-' || true` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock || true` 无输出；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；`git status --short --branch` 仅显示本轮 4 个 docs / tasks 文件。
- 只读审查结果：子代理复核本轮 4 个文档 / 任务文件后未发现阻断问题。
- 本轮状态同步提交完成后，下次启动以 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。

## 2026-06-06 Rust/Go Phase 5 release handoff evidence plan 计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，当前 HEAD 为 `6525f6fb docs(rust-go): 回填 Phase 5 publication invocation 最新恢复入口`，最新开发基线为 `77b266e8 fix(rust-go): 收紧 Phase 5 publication invocation 发布证据`。本轮只推进真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改和真实 packaged app bundled binary smoke 之前的交接证据前置：新增只读 `nativeSearchReleaseHandoffPlan`，聚合现有 publication invocation、optionalDependencies install-chain change plan、builder allowlist change plan、packaged smoke invocation plan 和 default-enable blocker，输出各真实 gate 的 required approval、missing evidence、candidate commands、forbidden actions 和 no-go 语义。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` / `bun.lock` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。实现提交已完成：`4fec7fab feat(rust-go): 补齐 Phase 5 release handoff 证据计划`。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `6525f6fb`，最新开发基线 `77b266e8` 在最近历史中。
- [x] 并行启动两个只读子代理分别复核 optional publication / install-chain 与 packaged smoke / builder allowlist no-go；主线收敛到 release handoff evidence plan，不触碰 no-go 文件。

实现计划：

- [x] 测试先行扩展 `native-runtime-smoke.test.ts`，锁住 `nativeSearchReleaseHandoffPlan` 的 blocked / ready 语义、证据缺失、候选命令脱敏、forbidden actions 和“不证明 published / installed / packaged verified / default-enable”的边界。
- [x] 在 `native-runtime-smoke.ts` 新增只读 `nativeSearchReleaseHandoffPlan` summary helper：只消费现有 plan / gate 结果，不查询 registry、不运行 `npm publish` / `npm pack`、不安装、不写 package manifest / lockfile / builder、不读取 binary。
- [x] 在 `smoke:native-runtime` summary 接入 `nativeSearchReleaseHandoffPlan`，当前仓库默认继续输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本到 `0.0.166` 并同步 `bun.lock`；未新增真实 native optionalDependencies。
...
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，确认 handoff 只进入 ready / blocked 交接状态，不把 target 404 当作 published。
- [x] 运行预期 no-go：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认 4 个计划 optional package 未发布时仍返回非零。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`4fec7fab feat(rust-go): 补齐 Phase 5 release handoff 证据计划`。`@codeinsights/electron` 已递增到 `0.0.166` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 已新增 smoke summary 字段 `nativeSearchReleaseHandoffPlan`，聚合 `optionalPackagePublicationInvocationPlan`、`optionalDependenciesInstallChainChangePlan`、`packagingConfigAllowlistChangePlan`、`packagedBundledBinarySmokeInvocationPlan` 和 `optionalPackageExecutionPlan`，输出 `status`、`nextGate`、required approvals、missing evidence、acceptance evidence、candidate commands、doesNotVerify 和 forbidden actions。
- 语义边界已收紧：`ready_for_release_handoff` 只表示当前顺序 gate 的下一步可交接给人工审核 / 执行，绝不等于真实 publish / install / builder 修改 / packaged smoke / default enable 已完成。publication / packaged smoke ready 字段已绑定 `optionalPackageExecutionPlan.nextStage`，后置 packaged 布尔值不能跳过 publish-target / source / publication 顺序 gate。
- 当前 no-go 保持：native 继续 default off / 显式 opt-in；默认 smoke 不联网；真实 optional package 发布、真实 optionalDependencies 声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts`（60 pass）；`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（103 pass）；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3`；显式 registry dry-run `smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 预期 no-go 验证通过：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未真实发布。
- 禁改边界验证通过：根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml` 无 diff；`apps/electron/package.json` / `bun.lock` diff 未出现 native search optionalDependencies；全量声明扫描未发现真实 `@codeinsights/native-search-*` optional package；排除 `.git` / `node_modules` / `native/search/target` 后未发现 packaged native binary 产物；未 push，未创建 PR。
- 代码审查子代理结论：未发现阻断问题，确认 release handoff plan 没有把 ready / verified 外推为真实发布、安装、packaged verified 或 default enable，默认 smoke 不联网，candidate commands 只是字符串。

## 2026-06-06 Rust/Go Phase 5 publication invocation 最新恢复入口回填计划

范围确认：用户要求更新文档最新开发状态，清楚标注完成 / 未完成，确保下次启动可以跟踪进度继续开发，并提供可直接复制的下次启动提示词；同时要求把“每个阶段性任务完成后自动同步文档状态”的习惯固化。本轮只做状态文档回填：当前 HEAD 为 `95305503 docs(rust-go): 同步 Phase 5 publication invocation 证据收紧状态`，最新开发基线保持为 `77b266e8 fix(rust-go): 收紧 Phase 5 publication invocation 发布证据`，最新已确认恢复入口推进到 `95305503`。继续保持 native default off / 显式 opt-in；不创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。

- [x] 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认当前分支为 `rust-go-refactor`、工作树起始干净，且最近历史包含 `95305503`、`77b266e8`、`f949fc13`、`084546cc`、`076f25c6` 和 `edeba827`。
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `native/search/`，确认本轮只需做状态同步。
- [x] 更新 `tasks/lessons.md`，固化 `95305503` 是 `77b266e8` 后当前已确认恢复入口，并再次记录阶段完成后自动同步 development checklist、next-session prompt、`tasks/todo.md` Review 和必要 lessons 的习惯。
- [x] 更新 development checklist 顶部状态、下次启动入口和底部可复制提示词，把最新已确认恢复入口从 `084546cc` 推进到 `95305503`。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md` 顶部状态和可复制提示词，把最新已确认恢复入口推进到 `95305503`。
- [x] 更新本 Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改文件 diff 检查、真实 native optionalDependencies 扫描、packaged native binary 产物扫描、旧主动恢复入口残留扫描和 `git status --short --branch`。
- [x] 单独提交本轮状态同步文档。

### Review

- 文档校正：已将 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `084546cc docs(rust-go): 回填 Phase 5 packaged smoke invocation 最新恢复入口` 推进到 `95305503 docs(rust-go): 同步 Phase 5 publication invocation 证据收紧状态`；最新开发基线保持 `77b266e8 fix(rust-go): 收紧 Phase 5 publication invocation 发布证据`。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 optional package publication invocation plan 和 publication invocation 发布证据收紧。真实 optional package 发布、真实 optionalDependencies 声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 语义边界继续明确：`optionalPackagePublicationInvocationPlan` 只表示真实 `npm publish` 调用输入、候选命令、验收证据和 forbidden actions 已机器可读化；`ready_for_invocation` 不等于 package 已发布。`publishedPackages` 只能来自真实 registry exact-version publication evidence；collision、invalid、unavailable、duplicate 或矛盾 metadata 只能进入 blocker / exclusion。真实 verified 仍必须同时满足 `bundledBinaryVerified=true` 且无 blocker。
...
- 边界保持：native 继续 default off / 显式 opt-in；本轮只做文档 / 任务状态同步，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未新增真实 `@codeinsights/native-search-*` optionalDependencies，未 push，未创建 PR。
- 验证通过：`git diff --check`；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`git diff -- apps/electron/package.json bun.lock | rg -n '@codeinsights/native-search|optionalDependencies|native-search-' || true` 无输出；`rg -n '"@codeinsights/native-search' apps/electron/package.json bun.lock || true` 无输出；排除 `.git` / `node_modules` / `native/search/target` 后未发现 `native-search-package.json`、`codeinsights-native-search` 或 `codeinsights-native-search.exe`；旧主动恢复入口扫描无输出；`git status --short --branch` 仅显示本轮 4 个 docs / tasks 文件。本轮状态同步提交完成后，下次启动以 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。

## 2026-06-06 Rust/Go Phase 5 optional package publication invocation plan 计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，当前 HEAD 为 `084546cc docs(rust-go): 回填 Phase 5 packaged smoke invocation 最新恢复入口`，最新开发基线为 `edeba827 feat(rust-go): 补齐 Phase 5 packaged smoke invocation plan`。本轮只推进真实 optional package 发布前的调用设计前置：在既有 `optionalPackagePublicationChangePlan` 之上新增只读 `optionalPackagePublicationInvocationPlan`，把真实 `npm publish` 前的 required inputs、candidate commands、per-package invocation blockers、acceptance evidence 和 forbidden actions 机器可读化。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行 `npm pack`，不安装依赖；除按项目规则递增 `@codeinsights/electron` patch 版本并同步 `bun.lock` 外，不写真实 optionalDependencies 声明 / 安装链路，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `084546cc`，最新开发基线 `edeba827` 在最近历史中。
- [x] 启动两个只读子代理分别复核 optional publication / install-chain 和 packaged smoke / builder allowlist no-go；主线先推进 publication invocation plan，不触碰 no-go 文件。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts` 与 `native-runtime-smoke.test.ts`，锁住 `optionalPackagePublicationInvocationPlan` 的 blocked / ready_for_invocation、默认离线边界、per-package blocker、候选命令脱敏和“不证明 published / installed / packaged verified”语义。
- [x] 在 `native-runtime-package-manifest.ts` 新增只读 optional package publication invocation plan helper：只消费 `optionalPackagePublicationChangePlan` 和 publication evidence，不查询 registry、不运行 `npm publish` / `npm pack`、不安装、不写 package manifest / lockfile / builder、不读取 binary。
- [x] 在 `smoke:native-runtime` summary 接入 `optionalPackagePublicationInvocationPlan`，当前仓库默认必须继续输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native optionalDependencies。

验证计划：

- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3`。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，确认 invocation plan 只进入 ready / review 状态，不把 target 404 当作 published。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行 no-go smoke：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest` 与预期 no-go `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 实现提交：`f949fc13 feat(rust-go): 补齐 Phase 5 optional package publication invocation plan`。
- 证据收紧提交：`77b266e8 fix(rust-go): 收紧 Phase 5 publication invocation 发布证据`。当前最新开发基线推进到 `77b266e8`，`@codeinsights/electron` 已递增到 `0.0.165` 并同步 `bun.lock`。
- 已新增 `buildNativeSearchOptionalPackagePublicationInvocationPlan()` 和 smoke summary 字段 `optionalPackagePublicationInvocationPlan`，只读输出真实 `npm publish` 前的 required inputs、candidate commands、per-package invocation status / blocker、acceptance evidence 和 forbidden actions。`ready_for_invocation` 只表示可进入人工审核 / 调用，不运行 `npm publish` / `npm pack`，不安装、不写 package manifest / lockfile / builder、不读取 binary、不证明 packaged app bundled binary。
- 已收紧 publication invocation 证据：顶层 `publishedPackages` 只从 package-level `status="published"` 汇总；完整 verified 必须要求 planned package 集合与 published package 集合唯一且严格匹配。invalid、unavailable、duplicate、collision 或矛盾 registry evidence 只能进入 blocker / exclusion，不能当作已发布；`not_required_already_published` 只能在 4 个 planned exact package 全部真实 registry verified 且无 blocker 时成立。
- 当前 no-go 保持：native 继续 default off / 显式 opt-in；真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 验证通过：`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts`（39 pass）；`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（101 pass）；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode packaged-manifest`；预期 no-go `smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未发布；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 状态同步已更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review。最新已确认恢复入口在本轮状态同步提交前仍记录为 `084546cc docs(rust-go): 回填 Phase 5 packaged smoke invocation 最新恢复入口`；本轮 docs 提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。
- 状态同步验证通过：`git diff --check`；禁改文件检查确认根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml` 无 diff；`apps/electron/package.json` / `bun.lock` diff 未出现 native search optionalDependencies；全量声明扫描未发现真实 `@codeinsights/native-search-*` optional package；排除 `native/search/target` 后未发现 packaged native binary 产物；旧 `edeba827` / `076f25c6` 恢复口径扫描无残留。
...
- [x] 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认当前工作树起始干净且最新 docs 提交为 `c4000bc9`。
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `native/search/`。
- [x] 更新 development checklist 顶部状态、编号启动步骤和底部可复制提示词，把最新已确认恢复入口从 `235ea492` 推进到 `c4000bc9`。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md` 顶部状态和可复制提示词，把最新已确认恢复入口推进到 `c4000bc9`。
- [x] 更新 `tasks/lessons.md`，再次固化阶段完成后自动同步 development checklist、next-session prompt、`tasks/todo.md` Review、必要 lessons，并单独提交状态同步的习惯。
- [x] 更新本 `tasks/todo.md` Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改边界检查、真实 native optionalDependencies 扫描、packaged native binary 产物扫描和 `git status --short --branch`。
- [x] 单独提交本轮状态同步文档。
...
- 文档校正：已将 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `235ea492 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口` 推进到 `c4000bc9 docs(rust-go): 同步 Phase 5 publication plan 证据拆分状态`；最新开发基线保持 `d679eabb fix(rust-go): 拆分 Phase 5 publication plan 发布证据`。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 optional package publication change plan 证据拆分。真实 optional package 发布、真实 optionalDependencies 声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 语义边界继续明确：`publishedPackages` 只表示真实 registry publication 证据；`publishTargetCollisionPackages` 与 `existingInvalidPublishedPackages` 只进入 exclusion / blocker，不能当作已发布。`optionalPackagePublicationChangePlan.ready_for_review`、`optionalDependenciesInstallChainChangePlan` 和 `packagingConfigAllowlistChangePlan` 都只是实际发布 / 修改 / 安装前 review 输入，不证明 packaged verified。
- 已补 `tasks/lessons.md`：用户再次强调阶段完成后自动同步时，要默认更新 development checklist、next-session prompt、`tasks/todo.md` Review、必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档。
- 边界保持：native 继续 default off / 显式 opt-in；本轮只做文档 / 任务状态同步，未创建 packaged native binary，未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未新增真实 `@codeinsights/native-search-*` optionalDependencies，未 push，未创建 PR。
- 验证通过：`git diff --check`；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；真实 native optionalDependencies 扫描无匹配；packaged native binary 产物扫描无输出；恢复入口残留扫描无匹配；`git status --short --branch` 仅显示本轮 docs / tasks 文件。
- 本轮状态同步提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口，最终回复给出本轮实际 HEAD 和可复制提示词。

## 2026-06-06 Rust/Go Phase 5 optional package publication change plan 计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，当前 HEAD 为 `235ea492 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`，最新开发基线为 `7726a008 feat(rust-go): 补齐 Phase 5 optionalDependencies install-chain dry-run plan`，最新已确认恢复入口为 `235ea492`。本轮只推进真实 optional package 发布前的人工 review / 发布执行计划收口：在既有 publish-target dry-run、optional package source preflight、publication gate 和 execution plan 之上，新增机器可读 `optionalPackagePublicationChangePlan`，输出 4 个计划发布包、目标版本、publish-target / source / registry publication blockers、候选发布前验证命令、候选发布命令形状和禁止动作。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行 `npm pack`，不安装依赖，不写 `apps/electron/package.json` / `bun.lock`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不修改 `apps/electron/electron-builder.yml`，不创建 packaged native binary，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前分支、工作树和最近提交历史。
- [x] 尝试通过子代理复核 Phase 5 gate / no-go 与 `native/search` 现状；因 thread agent limit 不可用，改为本地只读复核，本轮主线收敛到 optional package publication change plan，不触碰 no-go 文件。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts` 和 `native-runtime-smoke.test.ts`，锁住 publication change plan 的 blocked / ready_for_review、planned packages、registry / source / target blockers、默认离线边界、候选命令脱敏和“不证明 published / installed / packaged verified”语义。
- [x] 在 `native-runtime-package-manifest.ts` 新增只读 optional package publication change plan helper：只消费 publish-target / source / publication preflight 结果，不查询 registry、不运行 pack / publish、不安装、不写 package manifest / lockfile / builder、不读取 binary。
- [x] 在 `smoke:native-runtime` summary 接入 `optionalPackagePublicationChangePlan`，当前仓库默认必须继续输出 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native optionalDependencies。

验证计划：

- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3` 和显式 `--check-registry` dry-run，确认 publication change plan 不把 publish-target ready 外推为 published。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
...
- 实现提交：`d679eabb fix(rust-go): 拆分 Phase 5 publication plan 发布证据`，基于 `8097ed4c` 的 publication change plan 与 `2188e276` 的 collision 过滤修正继续收口。
- 已新增并收紧 `optionalPackagePublicationChangePlan`：输出 4 个 planned package、目标版本、publish-target / source / publication blocker、candidate preflight commands、candidate publication commands 和 forbidden actions。`ready_for_review` 只代表真实 publication 前的人工 review 输入，不运行 `npm publish` / `npm pack`，不安装、不写 `apps/electron/package.json` / `bun.lock` / builder，不读取 binary。
- 证据语义已拆分：`publishedPackages` 只表示 registry 中 exact expected version 且 metadata 有效的真实 publication 证据；`publishTargetCollisionPackages` 与 `existingInvalidPublishedPackages` 只会进入 `commandExcludedPackages` 并排除候选发布命令，不能被当作已发布证据。
- 当前 no-go 仍成立：默认 smoke 不联网；显式 `--check-registry` 的 publish-target dry-run 可进入 `ready_for_review`，但 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false` 和 `nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false` 均保持 false。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.162` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- smoke 验证通过：`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode packaged-app-layout`。预期 no-go：`smoke:native-runtime -- --mode packaged-manifest --check-registry` 返回 exit 1，因为 4 个计划 optional package 尚未发布。
- 禁改边界保持：未创建 packaged native binary；未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；未新增真实 native search optionalDependencies；未 push，未创建 PR。代码审查子代理因 thread agent limit 不可用，已完成本地复核。
- 状态同步已更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review：最新开发基线推进到 `d679eabb`，最新已确认恢复入口先记录为 `235ea492 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`，本轮 docs 提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。
- 状态同步验证通过：`git diff --check`；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；真实 native optionalDependencies 扫描无匹配；packaged native binary 产物扫描无输出；`git status --short --branch` 仅显示本轮 docs / tasks 文件。

## 2026-06-06 Rust/Go Phase 5 optionalDependencies install-chain 最新恢复入口回填计划

范围确认：用户要求更新 Rust / Go Phase 5 最新开发状态、清楚标注完成 / 未完成，并给出下次启动可直接复制的提示词，同时再次强调每个阶段性任务完成后自动同步。本轮只做状态文档回填：最新开发基线保持为 `7726a008 feat(rust-go): 补齐 Phase 5 optionalDependencies install-chain dry-run plan`，把当前 `git log` 已确认的最新 Rust / Go docs 状态同步提交 `bf98779f docs(rust-go): 同步 Phase 5 optionalDependencies install-chain 后续状态` 写入 development checklist 和 next-session prompt。继续保持 native default off / 显式 opt-in；不创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。

- [x] 运行 `git status --short --branch` 和 `git log -10 --oneline`，确认当前工作树起始干净且最新 docs 提交为 `bf98779f`。
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go checklist 和 `next-session-prompt.md`。
- [x] 更新 development checklist 顶部状态和底部可复制提示词，把最新已确认恢复入口从 `e8ffc59f` 推进到 `bf98779f`。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md` 顶部状态和可复制提示词，把最新已确认恢复入口推进到 `bf98779f`。
...
- 文档校正：已将 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `e8ffc59f docs(rust-go): 回填 Phase 5 builder allowlist 最新恢复入口` 推进到 `bf98779f docs(rust-go): 同步 Phase 5 optionalDependencies install-chain 后续状态`；最新开发基线保持 `7726a008 feat(rust-go): 补齐 Phase 5 optionalDependencies install-chain dry-run plan`。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 optionalDependencies install-chain dry-run plan。真实 optional package 发布、真实 optionalDependencies 声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
...
- 验证通过：`git diff --check`；`git diff --name-only -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；真实 native optionalDependencies 扫描无输出；`git status --short --branch` 仅显示本轮 docs / tasks 文件。
- 本轮状态同步提交完成后，下次启动以 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口，最终回复给出本轮实际 HEAD 和可复制提示词。

## 2026-06-06 Rust/Go Phase 5 optionalDependencies install-chain dry-run plan 计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，最新开发基线为 `a3f98673 feat(rust-go): 补齐 Phase 5 builder allowlist dry-run plan`，最新已确认恢复入口为 `e8ffc59f docs(rust-go): 回填 Phase 5 builder allowlist 最新恢复入口`。本轮只推进真实 optionalDependencies 声明与安装执行前置设计：在现有 optional package publication / optionalDependencies declaration / install-chain gate 之上，新增一个机器可读 dry-run change plan，输出 4 个 planned exact optionalDependency spec、当前缺失声明 / lockfile / installed package blocker、候选验证命令和禁止动作。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；不执行 `npm publish`，不运行真实 install，不修改 `apps/electron/package.json` 以新增真实 `@codeinsights/native-search-*` optionalDependencies，不创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `e8ffc59f`，最新开发基线 `a3f98673` 在最近历史中。
- [x] 通过只读子代理并行复核 optional package / install-chain 与 packaged smoke / builder allowlist 两个方向；本轮主线收敛到 optionalDependencies install-chain dry-run plan，不触碰 no-go 文件。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `c43b64f2`，最新开发基线 `ce89f490` 在最近历史中。
- [x] 只读复核当前 optional package / packaged smoke helper，确认本轮不具备真实发布、真实安装或 builder allowlist 实际修改权限。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts` 和 `native-runtime-smoke.test.ts`，锁住 builder allowlist dry-run plan 的 required include、remove / replace candidates、approval required、默认 blocked 和不证明 packaged binary 边界。
- [x] 在 `native-runtime-package-manifest.ts` 新增只读 builder allowlist change plan helper：只消费当前 `electron-builder.yml` 文本和既有 validator 结果，不写 YAML、不解析 binary、不安装 package、不发布 package。
- [x] 在 `smoke:native-runtime` summary 接入 builder allowlist change plan，保持当前仓库 `packagingConfigVerified=false`、`packagedBundledBinarySmokePlan.status="blocked"`、`nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 native optionalDependencies。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `10faa4a3`，最新开发基线 `05c393bb` 在最近历史中。
- [x] 通过只读子代理并行复核 optional package / install-chain 与 packaged smoke / builder allowlist 两个方向；本轮主线先处理真实执行前的顺序 gate，不触碰 no-go 文件。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts` 和 `native-runtime-smoke.test.ts`，锁住 optional / packaged execution plan 的 blocked / ready 阶段、顺序 blocker、默认不联网和 ready 不等于 verified 的边界。
- [x] 在 `native-runtime-package-manifest.ts` 新增只读执行顺序预检 helper：只消费既有 gate 布尔值和 blocker，不读取 binary、不查询 registry、不安装、不发布、不写 builder。
- [x] 在 `smoke:native-runtime` summary 接入 `optionalPackageExecutionPlan`，输出当前阶段、blockedBy、completedPrerequisites、nextAllowedActions、forbiddenActions 和 candidateCommands；默认 `packaged-manifest` / `packaged-app-layout` 必须仍 blocked。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 `@codeinsights/native-search-*` optionalDependencies。

验证计划：

- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3` 和显式 `--check-registry` dry-run，确认执行顺序预检不改变 publish-target 语义。
- [x] 运行 no-go smoke：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest` 与 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-app-layout`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 已新增 `buildNativeSearchOptionalPackageExecutionPlan()` 和 `optionalPackageExecutionPlan` smoke summary。该 plan 只消费已有 gate 布尔值，不读取 binary、不查询 registry、不安装、不发布、不写 builder；`completedPrerequisites` 是严格顺序前缀，`observedEvidenceStages` 只表示独立 evidence。
- `optional-package-publish-target --check-registry --native-search-package-version 0.0.3` 现在同时运行 in-memory source preflight；当 publish target 与 package source 都 ready 时，execution plan 只推进到 `nextStage="optional_package_publication"`、`readyForPublication=true`，仍保持 `optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false` 和 `bundledBinaryVerified=false`。
- 代码审查子代理指出 summary 顶层 ready 字段必须和 checked 绑定；已修复为 `checked && ready`，并补测试覆盖 `checked=false, ready=true` 的矛盾输入，避免顶层 summary 与 execution plan 分叉。
- 验证通过：`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`（67 pass）；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode packaged-app-layout`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 边界保持：native 继续 default off / 显式 opt-in；未创建 packaged native binary；未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；未新增真实 `@codeinsights/native-search-*` optionalDependencies；未 push，未创建 PR。
- 状态同步将更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `tasks/lessons.md`：最新开发基线推进到 `ce89f490`，最新已确认恢复入口先记录为 `10faa4a3 docs(rust-go): 回填 Phase 5 optional package source 最新恢复入口`，本轮 docs 提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。

## 2026-06-05 Rust/Go Phase 5 optional package source 最新恢复入口回填计划

范围确认：用户要求再次更新 Rust / Go Phase 5 最新开发状态、标清已完成 / 未完成，并给出下次启动可直接复制的提示词，同时再次强调阶段性任务完成后自动同步。本轮只做状态文档回填：最新开发基线保持为 `05c393bb feat(rust-go): 补齐 Phase 5 optional package source 预检`，把当前 `git log` 已确认的最新 Rust / Go docs 状态同步提交 `cf17e7da docs(rust-go): 同步 Phase 5 optional package source 后续状态` 写入 development checklist 和 next-session prompt。继续保持 native default off / 显式 opt-in；不创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。

- [x] 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认当前工作树干净且最新 docs 提交为 `cf17e7da`。
- [x] 更新 development checklist 顶部状态和底部可复制提示词，把最新已确认恢复入口从 `e11f4476` 推进到 `cf17e7da`。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md` 顶部状态和可复制提示词，把最新已确认恢复入口从 `e11f4476` 推进到 `cf17e7da`。
- [x] 更新 `tasks/lessons.md`，再次固化阶段完成后自动同步文档并回填真实 docs 提交的习惯。
- [x] 更新本 `tasks/todo.md` Review，记录完成项、未完成项、验证命令和禁改边界。
- [x] 运行 `git diff --check`、禁改边界检查、真实 native optionalDependencies 扫描和 `git status --short --branch`。
- [ ] 单独提交本轮状态同步文档。
...
- 已将 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `e11f4476 docs(rust-go): 回填 Phase 5 release source 最新恢复入口` 回填为 `cf17e7da docs(rust-go): 同步 Phase 5 optional package source 后续状态`；最新开发基线继续保持 `05c393bb feat(rust-go): 补齐 Phase 5 optional package source 预检`。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 optional package source preflight，且 `optionalPackageSourceReady=true` 只表示 planned source metadata 形状 ready；真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 已更新可复制提示词：下次启动先读取 lessons / todo / Rust-Go 文档 / native/search，并运行 `git status --short --branch` 与 `git log -25 --oneline`，确认最近历史包含 `cf17e7da`、`05c393bb`、`e11f4476`、`1b496638` 和 `dc75f380`。
- 已补 lessons：阶段完成后如果再次要求更新最新状态 / 下次启动提示词，必须回填当前 `git log` 可确认的最新 Rust / Go docs 提交，不能停在上一轮恢复入口。
- 验证通过：`git diff --check`；`git diff -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；真实 native optionalDependencies 扫描无输出；`git status --short --branch` 仅显示本轮 4 个 docs / tasks 文件。
...
## 2026-06-05 Rust/Go Phase 5 optional package source preflight 计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，最新开发基线为 `dc75f380 feat(rust-go): 对齐 Phase 5 native search release source version`，最新已确认恢复入口为 `e11f4476 docs(rust-go): 回填 Phase 5 release source 最新恢复入口`。本轮只推进真实 optional package 发布前置中的“本地 package source / npm pack metadata 形状”只读 gate：验证 4 个 planned `@codeinsights/native-search-*` package source manifest 是否具备 exact package name/version、private=false、license、os/cpu、bin、files allowlist 和闭合 `native-search-package.json` 计划，但不创建 packaged native binary、不执行 `npm publish`、不运行安装链路、不修改 `apps/electron/package.json` optionalDependencies、不修改 `apps/electron/electron-builder.yml`、不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。native 继续 default off / 显式 opt-in；ready 仍不等于 verified。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前 HEAD 为 `e11f4476`，最新开发基线 `dc75f380` 在最近历史中。
- [x] 只读复核当前 optional package / packaged smoke helper，确认本轮不具备真实发布、真实安装或 builder allowlist 实际修改权限。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts` 和 `native-runtime-smoke.test.ts`，锁住 optional package source preflight 的 passed / failed / 默认不联网 / 不证明 publication 或 packaged binary 边界。
- [x] 在 `native-runtime-package-manifest.ts` 新增只读 `validateNativeSearchOptionalPackageSources()` helper：只消费 planned package source manifest 对象和 package version，校验 package.json metadata、files allowlist、`native-search-package.json` manifest plan，不读取 binary、不读取路径、不发布、不安装。
- [x] 在 `smoke:native-runtime` 新增 `--mode optional-package-source`，输出 `optionalPackageSourceChecked`、`optionalPackageSourceReady`、`optionalPackageSourceBlockers`、`optionalPackageSourceReadyPackages`、`invalidOptionalPackageSourcePackages` 等 summary 字段；ready 不改变 `optionalPackagesPublished`、`optionalDependenciesDeclared`、`optionalDependenciesInstallChainVerified`、`packagingConfigVerified`、`realPackagedBinaryVerified`、`bundledBinaryVerified` 或 default enable。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增真实 `@codeinsights/native-search-*` optionalDependencies。
...
- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3` 和显式 `--check-registry` dry-run，确认 source preflight 不改变 publish-target / publication 语义。
- [x] 运行 no-go smoke：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest` 与 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-app-layout`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 已完成 optional package source preflight：新增 `validateNativeSearchOptionalPackageSources()` 和 `smoke:native-runtime -- --mode optional-package-source`，只读验证 planned package source manifest 的 metadata 形状、`files` allowlist、`native-search-package.json` 计划与 exact package version；ready 仅表示 source blueprint 准备好，不等于 package 已发布、依赖已声明/安装、builder allowlist 已修改或 packaged binary 已验证。
- 已补齐边界：source preflight 明确拒绝 lifecycle scripts、运行时 dependency 字段和 path-like 字段，不会运行 `npm pack`、`npm publish`、安装链路或 packaged smoke。
- 验证通过：`bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`；`smoke:native-runtime -- --mode optional-package-source --native-search-package-version 0.0.3`；`smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3` 与 `--check-registry`；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode packaged-app-layout`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 边界仍保持：native 继续 default off / 显式 opt-in；未创建 packaged native binary；未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；未新增真实 `@codeinsights/native-search-*` optionalDependencies；未 push，未创建 PR。
- 状态同步已更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/lessons.md` 和本 Review；最新开发基线推进到 `05c393bb`，最新已确认恢复入口先记录为 `e11f4476`，本轮 docs 提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。
- 禁改边界验证通过：`git diff -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；真实 native optionalDependencies 扫描无输出；`git status --short --branch` 仅显示本轮 5 个 docs / tasks 文件。
...
- [x] 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认当前分支、工作树和最新 docs / feature 基线。
- [x] 更新 development checklist 顶部状态、完成 / 未完成清单和下一轮启动入口，把最新已确认恢复入口从 `a05084cd` 推进到 `92a6f1a6`。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md` 顶部状态和可复制提示词，确保下次启动能从 `92a6f1a6` 或本轮后续 docs 提交继续。
- [x] 更新 `tasks/lessons.md`，把“阶段完成后自动同步文档、Review、lessons 和可复制提示词”作为默认动作再次固化。
- [x] 运行 `git diff --check`、禁改文件扫描和 native optionalDependencies 扫描。
- [x] 单独提交本轮状态同步。

### Review

- 已把 Rust / Go development checklist 和 `next-session-prompt.md` 的最新已确认恢复入口从 `a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口` 回填为 `92a6f1a6 docs(rust-go): 同步 Phase 5 publish target 后续状态`；最新开发基线继续保持 `9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`。
- 完成 / 未完成状态保持清晰：Phase 0-4 已完成；Phase 5 已完成到 optional package publish-target dry-run 预检；真实 optional package 发布、真实 optionalDependencies 声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策和 Phase 6-9 仍未完成。
- 已补充 `tasks/lessons.md`：以后阶段完成或用户要求最新进度 / 下次启动提示词时，默认先核对 `git log`、回填最新 docs 恢复入口、更新 Review / lessons、运行禁改边界验证并单独提交。
- 验证通过：`git diff --check`；`git diff -- README.md AGENTS.md apps/electron/electron-builder.yml` 无输出；`rg -n '"@codeinsights/native-search-|@codeinsights/native-search-' apps/electron/package.json bun.lock apps/electron/electron-builder.yml || true` 无输出；`git status --short --branch` 仅显示本轮 4 个文档 / 任务文件。
- 边界保持：native 继续 default off / 显式 opt-in；未创建 packaged native binary；未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；未新增真实 native search optionalDependencies；未 push，未创建 PR。

## 2026-06-05 Rust/Go Phase 5 optional package publish-target dry-run 预检计划

范围确认：继续 Phase 5 “Rust search sidecar 试点”。启动检查已确认当前分支为 `rust-go-refactor`、工作树起始干净，最新开发基线为 `dedbfed1 feat(rust-go): 补齐 Phase 5 packaged smoke 执行计划预检`，最新已确认恢复入口为 `a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口`。本轮只推进真实 optional package 发布前置工作：新增 publish-target dry-run / preflight gate，只读检查计划发布版本、registry 目标版本冲突、planned optional package manifest 元数据和 native source version 一致性。继续保持 native default off / 显式 opt-in；默认 smoke 不联网；显式 `--check-registry` 只读查询 npm；不发布 npm package，不创建 packaged native binary，不执行安装链路，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。

启动基线：

- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`、`docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认 HEAD 为 `a05084cd`，最新开发基线 `dedbfed1` 在最近历史中。
- [x] 通过只读子代理复核下一步切片；结论是 publish-target dry-run gate 比继续扩展 packaged binary fixture 更能推进真实 optional package 发布前置工作。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts` 和 `native-runtime-smoke.test.ts`，锁住 publish-target 版本、registry 目标版本冲突、404 可发布目标、registry unavailable、planned manifest metadata 和 native source version gate。
- [x] 在 `native-runtime-package-manifest.ts` 新增 `validateNativeSearchOptionalPackagePublishTarget()` 只读 helper：输入计划版本、registry packument metadata、Cargo version 与 binary version；输出 `optionalPackagePublishTargetReady`、blockers、available / collision / invalid / unavailable package 列表，不读取 binary、不安装、不发布、不输出路径。
- [x] 在 `smoke:native-runtime` 新增 `--mode optional-package-publish-target`、`--native-search-package-version <version>` 和 summary 字段；默认不联网时必须因 `registry_check_required` 保持 not ready，显式 registry check 才做只读目标版本检查。
- [x] 当前仓库应被 `native_search_binary_version_not_release_ready` 阻断，因为 `native/search/Cargo.toml` 为 `0.0.3`，`native/search/src/lib.rs` 为 `0.0.3-dev`；该 blocker 不等于真实 publication no-go 的 `optional_packages_not_published`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增 optionalDependencies。

验证计划：

- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-default-enable-readiness.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3`，确认默认不联网且 target not ready。
- [x] 运行显式 registry dry-run：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version 0.0.3 --check-registry`，预期当前 target 仍因 `native_search_binary_version_not_release_ready` no-go；若 registry 仍 404，package target availability 可成立但不代表 published。
- [x] 运行既有 no-go smoke：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest` 与 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-app-layout`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 已新增 `validateNativeSearchOptionalPackagePublishTarget()` 和 `smoke:native-runtime -- --mode optional-package-publish-target`。该 dry-run gate 只读检查计划发布版本、registry 目标版本冲突、planned optional package manifest 元数据、Cargo version 与 source `BINARY_VERSION` 一致性；不发布 npm package、不安装 package、不写 package manifest、不读取 binary、不输出路径。
...
- 当前默认离线 dry-run 输出 `optionalPackagePublishTargetChecked=false`、`optionalPackagePublishTargetReady=false`，blockers 包含 `registry_check_required` 和 `native_search_binary_version_not_release_ready`，exit 0；显式 `--check-registry` 只读查询 npm 后，4 个 planned packages 目标版本当前可发布，但仍因 `native/search/src/lib.rs` 为 `0.0.3-dev` 输出 `native_search_binary_version_not_release_ready`，exit 1。
- 既有真实 packaged gate 保持 no-go：`optionalPackagesPublished=false`、`optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`packagingConfigVerified=false`、`realPackagedBinaryVerified=false`、`bundledBinaryVerified=false`、`packagedBundledBinarySmokePlan.status="blocked"`、`nativeSearchDefaultEnableReadiness.defaultEnableCandidate=false`。
...
- 代码审查子代理未发现 Critical / High 问题；只建议回填本 Review 与 checklist。边界保持：native 继续 default off / 显式 opt-in；未创建 packaged native binary；未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；未新增真实 native search optionalDependencies；未 push，未创建 PR。
- 状态同步已更新 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `tasks/lessons.md`：最新开发基线推进到 `9e3e3dff`，最新已确认恢复入口先保留 `a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口`，本轮 docs 提交完成后以下次 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。
- 当前 no-go 保持：真实 optional package 尚未发布，真实 optionalDependencies 未声明 / 未安装，builder allowlist 未实际修改，真实 packaged app bundled binary smoke 未执行，publish-target dry-run 仍被 `native_search_binary_version_not_release_ready` 阻断，native 继续 default off / 显式 opt-in。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、`docs/improve/rust-go/next-session-prompt.md`、`docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md` 和 `native/search/`。
- [x] 运行 `git status --short --branch` 与 `git log -25 --oneline`，确认当前分支为 `rust-go-refactor`、工作树起始干净、最新开发基线 `348e003d` 和最新恢复入口 `60bdd3a3` 均在最近历史中。
- [x] 通过子代理只读复核 optional package / install-chain 与 packaged smoke / default-enable 两个方向；结论是当前不能声明真实 optionalDependencies 或修改 builder，合理下一步是补只读 publication / registry evidence gate。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts`、`native-runtime-default-enable-readiness.test.ts` 和 `native-runtime-smoke.test.ts`，锁住 publication 未验证 / 未发布时 native 继续 explicit opt-in，且不能绕过 install-chain、packaging config 和 real packaged binary gate。
- [x] 在 `native-runtime-package-manifest.ts` 新增只读 optional package publication metadata 校验 helper：只接收 registry 元数据对象，不读取 binary、不安装、不输出路径；要求 4 个计划包都存在、版本为 exact semver、manifest name 匹配、platform / arch / os / cpu / binary metadata 与计划一致。
- [x] 在 `native-runtime-default-enable-readiness.ts` 增加 `optionalPackagesPublished` gate 和 `optional_packages_not_published` blocker，让 default-enable 风险聚合显式区分“包已发布”和“声明 / lockfile / installed package 已验证”。
- [x] 在 `smoke:native-runtime -- --mode packaged-manifest` 接入 publication preflight summary 字段；默认不联网时 case skipped 且 `optionalPackagesPublished=false`，显式 `--check-registry` 时只读 npm registry 元数据并报告 missing / invalid packages。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增 optionalDependencies。
...
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`，确认默认无 registry 查询但 summary 输出 `optionalPackagesPublished=false`。
- [x] 运行显式只读 registry preflight（不安装）：`bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest --check-registry`，确认当前若 registry 仍 E404 则输出 missing packages、保持 no-go；如果网络不可用，则记录无法验证而不把 gate 置 true。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-app-layout`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck`、`bun run --filter='@codeinsights/electron' build:main`、`bun install --frozen-lockfile --dry-run`、`git diff --check`。
- [x] 禁改边界检查：确认未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`，未创建 packaged native binary，未新增真实 native search optionalDependencies，未 push，未创建 PR。
- [x] 阶段完成后更新 development checklist、sidecar protocol / smoke plan、next-session-prompt.md、必要 lessons 和本 `tasks/todo.md` Review，并单独提交实现与状态同步。
...
- 已新增 `validateNativeSearchOptionalPackagePublication()` 和 `getNativeSearchOptionalDependencyExpectedVersions()`。publication helper 只消费 registry packument 元数据和可选 expected exact version map，不读取 binary、不安装 package、不输出路径；无 expected version 时验证 `dist-tags.latest`，存在 expected version 时验证 `versions[expectedVersion]`，避免 publication 和 install-chain 各自证明不同版本。
- `smoke:native-runtime` 新增 `--check-registry` 显式只读入口。默认 `packaged-manifest` / `packaged-app-layout` 不联网，publication case 为 skipped 且 `optionalPackagesPublished=false`；显式 `--check-registry` 时只读查询 npm registry，404 进入 `missingPublishedOptionalPackages`，非 OK / fetch error 进入 `unavailablePublishedOptionalPackages`，metadata 不匹配进入 `invalidPublishedOptionalPackages`。
- 已把 `optionalPackagesPublished` 接入 `nativeSearchDefaultEnableReadiness` 和 smoke summary，新增 blocker `optional_packages_not_published`；`realPackagedBinaryVerified` / `bundledBinaryVerified` 现在必须同时满足非临时 fixture、optional package 已发布、optionalDependencies install-chain、packaging config、packaged evidence、packaged identity 和真实 resolver verification。
- 代码审查子代理先指出 temporary fixture 可绕过 summary、`packaged-app-layout --check-registry` 不会失败、publication 只看 latest 三个风险；已补红灯测试并修复。复审未发现 Critical / High 阻断。
- 当前真实 no-go：默认 `packaged-manifest` 仍为 exit 0 的预检视角，但 summary 为 `optionalPackagePublicationChecked=false`、`optionalPackagesPublished=false`、4 个 `missingOptionalDependencies`、4 个 lockfile / installed missing packages、`packagingConfigVerified=false`、4 个 `missingPackagingConfigPackages`、`blockingPackagingConfigExcludes=["!node_modules/@codeinsights/**"]`、`realPackagedBinaryVerified=false`。显式 registry `packaged-manifest --check-registry` 和 `packaged-app-layout --check-registry` 当前均 exit 1，4 个计划包都在 `missingPublishedOptionalPackages`，`optional_packages_not_published` blocker 生效。
- 版本同步：`@codeinsights/electron` 已递增到 `0.0.152` 并同步 `bun.lock`；未新增真实 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：新增测试先红灯；实现后目标测试矩阵 `50 pass`；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode packaged-manifest --check-registry` 预期 exit 1 no-go；`smoke:native-runtime -- --mode packaged-app-layout`；`smoke:native-runtime -- --mode packaged-app-layout --check-registry` 预期 exit 1 no-go；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 边界保持：native 继续 default off / 显式 opt-in；未创建 packaged native binary；未修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；未新增真实 native search optionalDependencies；未 push，未创建 PR。真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke 和最终 default-enable 风险决策仍未完成。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go 优化方案、development checklist、Phase 5 dependency decision record、sidecar protocol / smoke plan、next-session prompt 和 `native/search/`。
- [x] 运行 `git status --short --branch` 和 `git log -12 --oneline`，确认当前分支为 `rust-go-refactor`，工作树干净，最近历史包含 `eb715c46`、`05df1687`、`563b804c`、`16d5af21`、`31e37cbb`、`800dc885`、`52613780`、`24d1b657`、`e95cd284`、`ce104a59`、`8226c992`。
- [x] 确认当前文档恢复入口仍停在 `05df1687`，本轮状态同步需要回填实际 HEAD `eb715c46` 或本轮状态文档提交后的最新 Rust / Go docs 提交。
- [x] 只读检查 `native-runtime-package-manifest.ts`、`native-runtime-package-resolver.ts`、`native-runtime-smoke.ts`、`apps/electron/package.json` 和 `native/search/`，确认当前没有真实 native search optionalDependencies，也没有 packaged native binary。

实现计划：

- [x] 测试先行扩展 `native-runtime-package-manifest.test.ts`，锁住 optionalDependencies 支持 exact semver 与 `npm:@codeinsights/native-search-*@x.y.z` 同名 alias，拒绝 alias 指向非 native search 包、跨平台包、空 alias、range、`latest`、`file:` / `workspace:` / git / http / path-like spec。
- [x] 新增只读 installed package / lockfile consistency helper 测试：完整声明 + lockfile/installed manifest 一致时通过；当前仓库缺少 native search 声明、lockfile 或 installed package 时必须返回 `installedPackagesVerified=false` / `lockfileVerified=false`，且不读取 binary、不输出路径。
- [x] 扩展 `packaged-manifest` smoke：新增 optional package install-chain preflight case 与 summary 字段，当前仓库继续输出 `optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`realPackagedBinaryVerified=false`。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不新增 `@codeinsights/native-search-*` optionalDependencies。
- [x] 更新 Phase 5 sidecar protocol / smoke plan、development checklist、next-session-prompt.md 和必要 lessons，明确本轮只完成 install-chain preflight 设计，不等于真实 optional package 发布 / 真实 packaged bundled binary smoke。
...
- 实现提交：`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检`。
- 已新增 `validateNativeSearchOptionalPackageInstallChain()`，在 declaration gate 之外只读校验 native search optionalDependencies、`bun.lock` resolved package entry 和 installed package manifest 一致性；该 helper 不读取 binary、不输出路径、不证明真实 packaged bundled binary。
- optionalDependencies 版本声明已收紧：只接受 exact semver 或 `npm:<同名 native search package>@exact-semver`，拒绝跨包 alias、跨平台 alias、`latest` / range、空值、非字符串、`file:`、`workspace:`、git / http URL 和 path-like spec。
- `packaged-manifest` smoke 已新增 `packaged-optional-dependencies-install-chain-preflight` case 与 summary 字段：`optionalDependenciesInstallChainVerified`、`optionalDependenciesLockfileVerified`、`optionalDependenciesInstalledPackagesVerified`、`missingOptionalDependencyLockfilePackages`、`missingInstalledOptionalDependencies`、`invalidInstalledOptionalDependencies`。当前仓库未声明真实 `@codeinsights/native-search-*` optionalDependencies，因此该 case 保持 skipped，summary 继续明确 `optionalDependenciesDeclared=false`、`optionalDependenciesInstallChainVerified=false`、`realPackagedBinaryVerified=false`。
- 代码审查风险已修正：`realPackagedBinaryVerified=true` 现在必须满足 install-chain verified；lockfile 检查要求 resolved package entry，不能只因 importer 文本包含包名通过；声明存在但 lockfile / installed package 不完整时 install-chain preflight 会失败。
- 已递增 `@codeinsights/electron` patch 版本到 `0.0.146` 并同步 `bun.lock`；没有添加 `@codeinsights/native-search-*` optionalDependencies。
- 验证通过：目标 `bun test` 三个文件；`smoke:native-runtime -- --mode packaged-manifest`；`smoke:native-runtime -- --mode packaged-app-layout`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`；禁改文件 / native binary / 真实 optionalDependencies 检查无命中。
- 边界保持：native search 继续显式 opt-in / default off；未创建 packaged native binary；未修改 `apps/electron/electron-builder.yml`、根 `README.md`、根 `AGENTS.md`；未 push，未创建 PR。真实 optional package 发布 / 实际 optionalDependencies 声明与安装执行、真实 packaged app bundled binary smoke、Agent event-loop default-enable 风险评估和 Phase 6-9 仍未完成。
- 状态同步提交：由本轮文档提交生成，最终以 `git log -5 --oneline` 中最新 Rust / Go docs 提交为准。
...
- 已完成 / 未完成状态保持：Phase 0-4 已完成；Phase 5 当时已完成 search sidecar、sidecar manager、fallback gate、contract parity、benchmark、fake sidecar / cache corruption smoke、native-cache schema、optional package manifest schema、packaged-manifest preflight、bundled package resolver fixture、packaged app layout preflight smoke、packaged app evidence classifier、`packagedAppIdentityVerified` gate 和 optionalDependencies declaration preflight gate；真实 optional package 发布 / 安装执行、真实 packaged app bundled binary smoke、default-enable 风险评估、Phase 6-9 当时仍未完成。后续 `1114233b` 已补 optional package install-chain preflight。
...
- `packaged-app-layout` 的真实验证 gate 已收紧：即使 resolver、packaged evidence、identity 都通过，`realPackagedBinaryVerified=true` 当时已要求 `optionalDependenciesDeclared=true` 且非临时 fixture；后续 `1114233b` 已进一步要求 `optionalDependenciesInstallChainVerified=true`。当前仓库缺少 native search optionalDependencies，所以仍不能证明真实 packaged binary。
...
- [x] 读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go 优化方案、development checklist、Phase 5 dependency decision record、sidecar protocol / smoke plan、next-session prompt 和 `native/search/`。
- [x] 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含 `8226c992`、`e5b92fa7`、`9fe91b03`、`fb7e2d73`、`2cc95b1b`。
- [x] 运行 `git log -8 --oneline`，确认更早 native smoke / benchmark 文档提交 `9c102c0a`、`c6104eee`、`4f9c4d28` 仍在历史中。
- [x] 确认当前结论：`packaged-manifest` 只证明 manifest schema / 平台矩阵和 TS fallback 可用，summary 为 `bundledBinaryVerified=false`，不是真实 bundled binary packaged smoke；native 继续显式 opt-in / default off。
...
- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-package-resolver.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-package-manifest.test.ts apps/electron/scripts/native-runtime-smoke.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode packaged-manifest`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck` 和 `bun run --filter='@codeinsights/electron' build:main`。
- [x] 运行 `bun install --frozen-lockfile --dry-run`。
...
- 已新增 `apps/electron/src/main/lib/native-runtime/native-runtime-package-resolver.ts` 和测试。resolver 只通过 optional package `package.json` 的 module resolution 得到包根，再读取包内固定 `native-search-package.json` 和 `bin/{binaryName}`；不扫描 `node_modules`，不读取系统 `PATH`，不接受 manifest 里的路径字段。
- resolver 校验范围：optional package 平台矩阵、manifest exact schema、package / platform / arch / binaryName 匹配、app `node_modules` realpath allowlist、binary 存在与可执行、SHA-256 fingerprint 匹配；失败只返回 `unsupported_platform` / `package_missing` / `manifest_missing` / `manifest_invalid` / `binary_missing` / `binary_not_executable` / `checksum_mismatch` 这类 reason，不把真实路径写入错误结构。
- 代码审查子代理发现一个阻断问题：初版 resolver 虽然不查系统 `PATH`，但默认 `createRequire().resolve()` 可能沿 Node 规则解析到上级 / 用户 `node_modules` 的同名包。已修复为必须传过 app `node_modules` realpath allowlist，package.json 和 binary 都必须位于 allowlist 内；测试新增“拒绝 allowlist 外同名 package”回归。
- `TypeScriptNativeRuntimeService` 的 native search 创建路径已接入 resolver，但仍受 `CODEINSIGHTS_NATIVE_RUNTIME=1` 与 `CODEINSIGHTS_NATIVE_SEARCH=1` 保护；显式 `CODEINSIGHTS_NATIVE_SEARCH_BINARY` 优先用于本地开发 / benchmark，没有显式 binary 时只有 Electron packaged 环境才尝试 bundled optional package，解析失败保持 TypeScript fallback，不走 PATH。已新增非 packaged 环境 feature flags 不自动解析 bundled package 的 service 级测试。
...
- [x] 测试先行扩展 `apps/electron/scripts/native-runtime-smoke.test.ts`，锁住 `protocol-mismatch`、`crash`、`timeout`、`cache-corruption` 不再 skipped，且 summary 不泄露 fake binary path、home path、token 或真实 config/cache 路径。
- [x] 在 `apps/electron/scripts/native-runtime-smoke.ts` 内新增测试专用 fake sidecar fixture：只用 `process.execPath + 临时 JS 脚本` 模拟 protocol mismatch、search crash、search timeout；不从系统 `PATH` 查找，不创建 packaged binary。
- [x] 为 `cache-corruption` 使用隔离 `CODEINSIGHTS_CONFIG_DIR` 写入损坏 manifest，验证 `readNativeRuntimeCacheManifest()` 返回 `cache_corrupted`，同时证明 TypeScript fallback 搜索仍可用。
- [x] 保持 `native-missing` / `native-available` 既有行为兼容；native available 仍只接受显式 binary，未提供时 skipped。
- [x] 递增 `@codeinsights/electron` patch 版本并同步 `bun.lock`；不改 optionalDependencies。

验证计划：

- [x] 运行 `bun test apps/electron/scripts/native-runtime-smoke.test.ts`。
- [x] 运行 `bun test apps/electron/src/main/lib/native-runtime/native-runtime-cache-schema.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-sidecar-manager.test.ts apps/electron/scripts/native-runtime-smoke.test.ts`。
- [x] 运行 `bun run --filter='@codeinsights/electron' smoke:native-runtime -- --mode protocol-mismatch`、`--mode crash`、`--mode timeout`、`--mode cache-corruption`。
- [x] 运行 `bun run --filter='@codeinsights/electron' typecheck` 和 `bun run --filter='@codeinsights/electron' build:main`。
- [x] 运行 `git diff --check`，确认未修改 `electron-builder.yml`、根 `README.md`、根 `AGENTS.md`，未创建 packaged native binary。
- [x] 更新 development checklist、next-session-prompt.md、`tasks/todo.md` Review 和必要 lessons；阶段完成后单独提交实现与状态同步。
...
- 阶段实现已提交：`2cc95b1b feat(rust-go): 补齐 Phase 5 fake sidecar smoke 失败路径`。
- 已把 `smoke:native-runtime` 的 `protocol-mismatch`、`crash`、`timeout`、`cache-corruption` 从 skipped 计划入口改成可执行 smoke。`protocol-mismatch` / `crash` / `timeout` 使用 `process.execPath + 临时 JS fake sidecar`，分别验证 `version_mismatch`、`crashed`、`timeout` fallback；`cache-corruption` 使用隔离 `CODEINSIGHTS_CONFIG_DIR/native-cache/manifest.json` 写入损坏 manifest 并验证 `cache_corrupted`。
- 每个新增 smoke mode 都先运行 TypeScript fallback 搜索并断言可用；summary 只暴露 `nativeSearchBinaryProvided` boolean，case detail 经 `redactNativeRuntimeText()` 脱敏，不打印 fake sidecar path、home path、token 或真实 config/cache 路径。
- 已保持 `native-available` 行为：未显式传 `--native-search-binary` 或 `CODEINSIGHTS_NATIVE_SEARCH_BINARY` 时仍 skipped，不从系统 `PATH` 查找；本轮未创建 packaged native binary，未修改 `optionalDependencies`，未修改 `electron-builder.yml`。
- 已递增 `@codeinsights/electron` patch 版本到 `0.0.139` 并同步 `bun.lock`。
- 验证通过：`bun test apps/electron/scripts/native-runtime-smoke.test.ts`（9 pass）；`bun test apps/electron/src/main/lib/native-runtime/native-runtime-cache-schema.test.ts apps/electron/src/main/lib/native-runtime/native-runtime-sidecar-manager.test.ts apps/electron/scripts/native-runtime-smoke.test.ts`（27 pass）；四个 CLI smoke mode `protocol-mismatch` / `crash` / `timeout` / `cache-corruption` 均输出 `passed`；`bun run --filter='@codeinsights/electron' typecheck`；`bun run --filter='@codeinsights/electron' build:main`；`bun install --frozen-lockfile --dry-run`；`git diff --check`。
- 代码审查子代理未发现阻断问题；重点确认进程清理、`CODEINSIGHTS_CONFIG_DIR` 恢复、PATH 边界、脱敏和 default-enable / packaged 边界均保持。
- 边界保持：native search 继续显式 opt-in / default off；未 push，未创建 PR；未修改根 `README.md` / 根 `AGENTS.md`；未修改 `electron-builder.yml`；未创建 packaged native binary。下一步应优先推进 optional package manifest 与 packaged smoke 设计，或继续分析 Agent native work-delay 小幅回退。
...
- [x] 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认当前分支 `rust-go-refactor`，最近历史包含 `65c67c52`、`aaede459`、`0eb350ff`、`e4bcd123`、`fdb6997b`。
- [x] 确认本轮禁止事项：不生成 packaged native binary，不修改打包配置，不默认启用 native，不触碰根 `README.md` / 根 `AGENTS.md`。

验证计划：

- [x] 复核 benchmark runner 输出字段和 gate 规则：记录 Chat / Agent TS 与 Rust native 的 P50 / P95 / P99、event loop delay、memory delta、数据规模和样本数。
- [x] 运行 Rust release binary 可用性检查；若 release binary 不存在，只允许 `cargo build --release --manifest-path native/search/Cargo.toml` 生成本地 ignored 产物，不创建 packaged binary。
- [x] 至少复跑 2 轮 100MB native benchmark：`--records 100000 --payload-bytes 900 --workspace-files 100000 --log-bytes 524288000 --iterations 20 --native-search-binary /Users/zq/Desktop/ai-projs/posp/RV-Insights/native/search/target/release/codeinsights-native-search`。
- [x] 对照 Phase 5 gate：Chat native P95 至少比 TS 快 3 倍且 event loop delay 降低 70%；Agent native P95 至少比 TS 快 2 倍，并重点判断 event loop delay 是否仍回退。
- [x] 若 Agent event loop gate 不稳定或回退，Review 中明确 native 仍保持显式 opt-in / default off，并规划下一步为 benchmark runner event-loop 复核或 sidecar manager overhead 分析，而不是 default enable。
- [x] 运行必要验证：`cargo test` / `cargo fmt --check` / `cargo clippy`、native runtime 相关 Bun tests、`bun run --filter='@codeinsights/electron' build:main`、`git diff --check`。
- [x] 根据结论更新 development checklist、next-session-prompt.md、`tasks/todo.md` Review 和必要 lessons，然后单独提交状态同步。

### Review

- 启动检查已确认：当前分支为 `rust-go-refactor`，最近历史包含 `65c67c52 docs(rust-go): 回填 Phase 5 search 性能优化最新恢复入口`、`aaede459 docs(rust-go): 同步 Phase 5 search 性能优化后续状态`、`0eb350ff feat(rust-go): 优化 Phase 5 Rust search sidecar 早停性能`、`e4bcd123 docs(rust-go): 回填 Phase 5 sidecar manager 最新恢复入口`、`fdb6997b docs(rust-go): 同步 Phase 5 sidecar manager 后续开发状态`。
- Benchmark runner 复核结论：脚本只输出 summary，不内置自动 pass / fail gate；P95 使用排序后 `ceil(percentile * length) - 1`，event loop delay 是每个 case 20 次迭代中的最大 `setTimeout(0)` 延迟，而不是 P95。
- 本轮先发现本地 release binary 仍报告 `0.0.1-dev`，与源码 `0.0.2-dev` 不一致；已运行 `cargo build --release --manifest-path native/search/Cargo.toml` 重建本地 ignored release binary，并用 `status` / `shutdown` smoke 确认 `0.0.2-dev` 后再跑 benchmark。本轮未创建 packaged native binary。
- 稳定 benchmark 第 1 轮命令：`bun run --filter='@codeinsights/electron' native-runtime:benchmark --records 100000 --payload-bytes 900 --workspace-files 100000 --log-bytes 524288000 --iterations 20 --native-search-binary /Users/zq/Desktop/ai-projs/posp/RV-Insights/native/search/target/release/codeinsights-native-search`。结果：TS Chat 99,527,780 bytes，P95 277.973ms，event loop 62.511ms；Rust native Chat P95 3.516ms，event loop 1.651ms。TS Agent 102,397,780 bytes，P95 227.239ms，event loop 1.269ms；Rust native Agent P95 2.203ms，event loop 1.732ms。
- 稳定 benchmark 第 2 轮命令同上。结果：TS Chat 99,527,780 bytes，P95 267.874ms，event loop 9.793ms；Rust native Chat P95 3.429ms，event loop 1.478ms。TS Agent 102,397,780 bytes，P95 220.111ms，event loop 1.127ms；Rust native Agent P95 2.201ms，event loop 1.416ms。
- Gate 结论：Chat native P95 和 event loop gate 两轮均达标；Agent native P95 两轮均远超 2x gate，但 Agent native event loop delay 两轮均小幅高于同轮 TS fallback。虽然绝对值只有约 1.4-1.7ms，但方向仍是回退，且 optional package、native-cache schema、smoke script 和 packaged smoke 尚未完成，因此 native 继续保持显式 opt-in / default off，不进入 default enable。
- 下一步建议：优先复核 benchmark runner 的 event-loop 测量口径和 sidecar manager overhead，或在 default off 前提下设计 smoke script、optional package 与 native-cache schema；在 P95 / event loop gate 与 packaged smoke 同时满足前，不默认启用 native。
...
- [x] 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认当前分支 `rust-go-refactor` 且最近历史包含 `fdb6997b`、`319f30e8`、`c76f3b33`、`e39682f1`。
- [x] 复核 `docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md` 与 `docs/improve/rust-go/next-session-prompt.md` 已标清 Phase 0-4 完成、Phase 5 已完成项、未完成 default enable / optional package / packaged smoke / Phase 6-9。
- [x] 将 latest recovery entry 从“`319f30e8` 或其后的 docs 提交”回填为真实状态同步提交 `fdb6997b docs(rust-go): 同步 Phase 5 sidecar manager 后续开发状态`。
- [x] 更新 `tasks/lessons.md`，记录用户再次要求状态同步时必须回填真实恢复入口，而不是保留需要推断的占位表达。
- [x] 验证文档 diff、Markdown fence、禁止触碰文件和 git status。
- [x] 单独提交本轮状态同步复核，提交信息使用详细中文；最终回复给出最新 HEAD 和可复制提示词。

### Review

- 最新开发基线保持为 `319f30e8 feat(rust-go): 接入 Phase 5 Rust search sidecar manager 与 fallback gate`；最新恢复入口已回填为 `fdb6997b docs(rust-go): 同步 Phase 5 sidecar manager 后续开发状态`。
- 已完成项明确：Phase 0-4；Phase 5 前置计划、依赖 decision record、sidecar protocol / fallback / packaged smoke 计划、100MB TS fallback benchmark gate、`native/search/` search-only Rust sidecar、Electron main process sidecar manager、Chat fallback gate、Rust vs TS contract parity 和 100MB native benchmark。
- 未完成项明确：Rust search 性能优化 / gate 复核、default enable、optional package、native-cache schema、smoke script、packaged smoke、Phase 6 大文件 / 日志 chunk preview、Phase 7 PathSafety / GitOutputParser、Phase 8 打包 / CI / 发布收口、Phase 9 Go supervisor 可选 spike。
- 下次入口明确：先确认 `fdb6997b` 和 `319f30e8` 在 `git log -5 --oneline` 中，再基于 benchmark 结果优化 Rust search 性能或重新规划 default-enable gate；未过 P95 / event loop gate 前不得默认启用 native。
- 边界保持：本轮不改业务代码，不创建 packaged native binary，不修改 `electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。
...
- [x] 检查 `git status --short --branch` 和 `git log -5 --oneline`，确认最新已提交恢复入口仍为 `f2f90e31 docs(rust-go): 同步 Phase 4 后续开发状态与下次启动入口`，当前存在 Phase 5 前置文档改动。
- [x] 更新 `docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`：标清 Phase 0-4 已完成、Phase 5 前置依赖决策 / protocol / smoke 计划已完成、Rust sidecar 实现尚未开始、Phase 6-9 未完成。
- [x] 更新 `docs/improve/rust-go/next-session-prompt.md`：写清 Phase 5 前置文档已完成，下一步先跑 TS fallback benchmark，再进入最小 Rust sidecar 实现。
- [x] 更新 `tasks/todo.md`：保留 Phase 5 前置计划 Review，并新增本状态同步计划。
- [x] 更新 `tasks/lessons.md`：记录用户再次强调的阶段收尾默认动作，避免以后只口头给提示词。
- [x] 验证：`git diff --check`、行尾空白扫描、Markdown code fence 检查、`git status --short --branch`。
- [x] 单独提交本轮 Phase 5 前置状态同步文档，提交信息使用详细中文；实际提交号以最终回复和 `git log` 为准。
...
## 2026-06-03 Rust/Go Phase 5 Rust search sidecar 前置计划

范围确认：Phase 0、Phase 1、Phase 2、Phase 3 和 Phase 4 已完成。最新开发基线为 `16cbb3e1 feat(rust-go): 完成 Phase 4 Native Runtime diagnostics 前端体验`，最新状态同步恢复入口为 `f2f90e31 docs(rust-go): 同步 Phase 4 后续开发状态与下次启动入口`。启动检查已确认当前分支为 `rust-go-refactor`。本阶段从 Phase 5 “Rust search sidecar 试点”开始，但先只做依赖搜索、decision record、sidecar protocol、fallback、contract parity、packaged smoke 和性能门槛计划；计划与决策写清后无需等待确认。当前步骤不直接写 Rust / Go 实现代码，不安装依赖，不创建 native binary，不修改根 `README.md` / 根 `AGENTS.md`。

启动基线：

- [x] 已读取 `tasks/lessons.md`、`tasks/todo.md`、Rust / Go 优化方案、开发跟踪清单和下次启动提示词。
- [x] 已运行 `git status --short --branch`：当前分支为 `rust-go-refactor`，最新状态以当前工作树和后续 `git status` 为准。
- [x] 已运行 `git log -5 --oneline`：最近历史包含 `f2f90e31 docs(rust-go): 同步 Phase 4 后续开发状态与下次启动入口` 和 `16cbb3e1 feat(rust-go): 完成 Phase 4 Native Runtime diagnostics 前端体验`。
- [x] 已确认 Phase 5 入口仍缺少前置门禁：Rust 依赖 decision record、sidecar protocol、fallback / packaged smoke 计划和性能收益门槛。

Phase 5 当前范围：

- [x] 先做 Rust / Go / TypeScript 路线与依赖搜索：比较继续 TS、Rust sidecar crate、Go package / supervisor 三类路线，不安装任何依赖。
- [x] 新增 Phase 5 dependency decision record：记录候选依赖当前版本、维护状态、license、跨平台、体积 / transitive deps、CVE / 安全关注点、替代路线和最终建议。
- [x] 明确 sidecar protocol 计划：优先 stdin/stdout JSON-RPC line-delimited protocol；定义 handshake、request / response envelope、requestId、protocolVersion、capabilities、timeout、shutdown、stderr 脱敏和 contract violation 行为。
- [x] 明确 fallback 计划：native disabled / missing / version_mismatch / contract_violation / timeout / crashed / cache_corrupted 时，本进程禁用 native 或降级 TS fallback；renderer 只看脱敏状态，不看 binary path。
- [x] 明确 packaged smoke 计划：覆盖 native available / missing、bundled binary path、不使用系统 `PATH`、unpacked app、cache corruption、protocol mismatch；实际打包和 binary 创建留到实现阶段或 Phase 8。
- [x] 明确性能收益门槛：若 Rust sidecar 未达到门槛，不默认启用 native。
- [x] 明确最小实现进入条件：前置 decision record、protocol plan、fallback plan、packaged smoke plan 和 benchmark 门槛写入文档后，才允许进入最小 Rust search sidecar 实现。

性能收益门槛：

- [x] `chat-search-large-history` / 100MB JSONL 内容搜索：Phase 5 native P95 至少比 Phase 1 / 当前 TS fallback 快 3 倍；event loop delay 至少降低 70%。
- [x] `agent-runtime-search` / 100MB 级 Agent JSONL：P95 至少快 2 倍；native crash / timeout 不影响 Agent UI 和会话事实源。
- [x] `pipeline-tail-large-records` / 50000 records：tail P95 与历史总量弱相关，较 Phase 2 TS cursor tail 至少不退化；如果收益不足，不把 tail 纳入首个 Rust sidecar 默认能力。
- [x] `workspace-file-name-search` / 100000 files：首次索引后热查询 P95 小于 150ms；Rust 只在证明能明显降低主进程阻塞后接手。
- [x] native 冷启动不让应用可交互时间增加超过 500ms；默认懒启动，启动探测只能轻量执行。
- [x] binary 体积、构建时间、optional package 数量和 packaged smoke 成本必须在 Review 中记录；收益不足时默认保留 TS fallback。

拟触达文件：

- [x] `tasks/todo.md`
- [x] `docs/improve/rust-go/2026-06-03-phase-5-dependency-decision-record.md`
- [x] `docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md`
- [x] `docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md`（已同步 Phase 5 前置状态）
- [x] `docs/improve/rust-go/next-session-prompt.md`（已同步 Phase 5 前置恢复入口）
- [ ] `tasks/lessons.md`（仅发现新的依赖 / protocol / smoke 纪律时）
...
依赖 decision record 必须覆盖：

- [x] 继续 TypeScript：复用当前 `TypeScriptEventSearchService`、`TypeScriptPipelineTailService`、`TypeScriptWorkspaceIndexService` 的成本和收益。
- [x] Rust 标准库优先方案：先评估是否可用标准库完成 line-delimited JSON protocol、JSONL tail 和简单 substring search。
- [x] Rust crate 候选：`serde` / `serde_json`、`memchr`、`regex`、`walkdir`、`ignore`、`memmap2`、`tantivy`；每个候选记录用途、版本、license、维护状态、跨平台和是否进入首个 sidecar。
- [x] Go package / supervisor 路线：标准库、`fsnotify` 等只作为 Phase 9 条件 spike，不进入 Phase 5 默认实现。
- [x] 安全与供应链：license、CVE、release 活跃度、维护者 / issue 活跃度、transitive dependencies、binary 体积和 release 签名影响。

sidecar protocol / fallback 计划必须覆盖：

- [x] handshake：`status` 返回 protocolVersion、cacheSchemaVersion、capabilities、binaryVersion、implementation。
- [x] request envelope：每行一个 JSON object，包含 `requestId`、`method`、`params`、`deadlineMs`；response 包含 `ok`、`result` 或 typed `error`。
- [x] 初始 method：`status`、`search`、`tail_jsonl`、`shutdown`；`index_jsonl` 只在 decision record 确认 cache schema 后进入实现。
- [x] stdout 只输出 protocol JSON，stderr 只允许脱敏 diagnostics；不得输出 prompt、token、credentialed URL、完整 home path 或原始 JSONL 内容。
- [x] timeout / crash / bad JSON / protocol mismatch / contract violation：main process 记录 typed diagnostics，本进程禁用 native 或降级 TS fallback。
- [x] 所有 native 结果进入业务流程前必须在 main process 做 schema 校验、二次脱敏、路径白名单和事实源边界检查。
...
- [x] smoke 输出必须脱敏，不打印 binary path 给 renderer，不打印 token / Authorization / credentialed URL。
...
实现阶段验证命令草案（本轮不执行，除非后续开始 Rust sidecar 实现）：
...
- [x] 不直接写 Rust / Go 代码。
- [x] 不安装依赖，不运行 `bun add`、`cargo add`、`go get` 或修改 lockfile。
- [x] 不创建 native binary、Rust crate、Go module、optionalDependencies 或打包配置。
- [x] 不修改根 `README.md` / 根 `AGENTS.md`。
- [x] 不把 native cache 当作 JSON / JSONL 事实源，不写业务事实源。
- [x] 不让 renderer 知道 sidecar endpoint、binary path、cache path 或任意 native command。
- [x] 不从系统 `PATH` 隐式寻找 sidecar，不把 smoke 建立在系统工具存在上。
- [x] 不展示 token、Authorization、credentialed remote URL、完整 home path、原始 stderr 或用户真实 JSONL 内容。
- [x] 不把 Go supervisor 纳入 Phase 5 默认主线；仅保留 Phase 9 条件 spike。
- [x] 不 push、不创建 PR、不执行真实远端写。

### 实现前 Check-in

- [x] 已将 Phase 5 前置计划写入本节；用户已明确无需确认，继续进行依赖搜索 / decision record 与 protocol / fallback / packaged smoke 计划。

### Review

- 前置计划已完成：新增 `docs/improve/rust-go/2026-06-03-phase-5-dependency-decision-record.md` 和 `docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md`，明确 Phase 5 先做依赖决策、protocol、fallback、contract parity、packaged smoke 和性能门槛。
- 依赖决策结论：Phase 5 首个 Rust sidecar 只建议进入实现阶段时评估 `serde`、`serde_json` 和可选 `memchr`；暂缓 `regex`、`walkdir`、`ignore`、`memmap2`、`tantivy`；Go / `fsnotify` 不进入 Phase 5 默认实现，只保留 Phase 9 条件 spike。
- 安全查询结论：OSV 查询中 `regex` 存在旧版本历史 advisory，修复版本为 `1.5.5`，当前候选 `1.12.3` 不在受影响范围内；因首版只做 literal search，仍不引入 `regex`。
