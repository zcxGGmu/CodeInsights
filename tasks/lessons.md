...
## 2026-06-08 Rust / Go Phase 5 4cf20557 恢复入口回填与 no-go 复核

- `4cf20557 docs(rust-go): 回填 26221722 Phase 5 恢复入口` 是本轮启动时最新已确认状态同步 / 恢复入口；`63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest` 仍是最新实现基线。
- 当最新 docs HEAD 已经推进到 `4cf20557`，但 active 文档仍把 `26221722` 写成“最新状态同步 / 恢复入口”时，必须继续执行自引用回填：更新 development checklist、`next-session-prompt.md`、sidecar / smoke plan、`tasks/todo.md` Review 和必要 lessons，把最新入口推进到 `4cf20557`，同时保留 `26221722` 作为历史状态同步提交。
- 当前真实完成边界不变：Phase 0-4 完成；Phase 5 已完成到 optional package readiness、no-go audit summary / smoke case 外显、install-chain installed manifest metadata 加固、3 个核心 gate 计划收缩、`5bc9ebe8` / `c95c5cf6` / `26221722` / `4cf20557` 恢复入口回填。readiness、dry-run、handoff、approval packet、execution checklist、transition verifier、no-go audit 和状态同步都不能写成真实 package published / installed / packaged verified。
...
- 未获明确批准前，仍不得执行真实 `npm publish`、真实 install、`apps/electron/electron-builder.yml` 修改、真实 `@codeinsights/native-search-*` optionalDependencies 新增、packaged binary 创建、push 或 PR；只读 readiness / registry evidence 必须继续标注为 no-go / readiness，不能外推为 publication 完成。

## 2026-06-08 Rust / Go Phase 5 26221722 恢复入口回填与 no-go 复核

- `26221722 docs(rust-go): 回填 c95c5cf6 Phase 5 恢复入口` 是本轮启动时最新已确认状态同步 / 恢复入口；`63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest` 仍是最新实现基线。
- 当最新 docs HEAD 已经推进到 `26221722`，但 active 文档仍把 `c95c5cf6` 写成“最新状态同步 / 恢复入口”时，必须继续执行自引用回填：更新 development checklist、`next-session-prompt.md`、sidecar / smoke plan、`tasks/todo.md` Review 和必要 lessons，把最新入口推进到 `26221722`，同时保留 `c95c5cf6` 作为历史状态同步提交。
- 当前真实完成边界不变：Phase 0-4 完成；Phase 5 已完成到 optional package readiness、no-go audit summary / smoke case 外显、install-chain installed manifest metadata 加固、3 个核心 gate 计划收缩、`5bc9ebe8` / `c95c5cf6` / `26221722` 恢复入口回填。readiness、dry-run、handoff、approval packet、execution checklist、transition verifier、no-go audit 和状态同步都不能写成真实 package published / installed / packaged verified。
- 当前真实未完成边界不变：未执行真实 `npm publish`，未声明真实 `@codeinsights/native-search-*` optionalDependencies，未运行真实 install，未获得 `bun.lock` / installed package manifest metadata 的真实 package evidence，未修改 builder allowlist，未创建真实 packaged app / native binary，未完成 packaged bundled binary smoke，default-enable 继续暂缓。
- 未获明确批准前，仍不得执行真实 `npm publish`、真实 install、`apps/electron/electron-builder.yml` 修改、真实 `@codeinsights/native-search-*` optionalDependencies 新增、packaged binary 创建、push 或 PR；只读 readiness / registry evidence 必须继续标注为 no-go / readiness，不能外推为 publication 完成。
...
- no-go audit 通过不等于 package published、optionalDependencies declared / installed、builder allowlist modified、real packaged binary verified、bundledBinaryVerified 或 default-enable candidate。真实 verified 仍必须等真实 optional / packaged gate 可执行并通过。
- 本轮 no-go 不变：native default off / 显式 opt-in；默认 smoke 不联网；不执行真实发布 / 安装；不修改 `apps/electron/electron-builder.yml`、根 `README.md`、根 `AGENTS.md`；不新增真实 `@codeinsights/native-search-*` optionalDependencies；不创建 packaged native binary；不 push / PR。
...
- 用户要求“更新文档最新开发状态、标注完成 / 未完成、给下次启动提示词”时，即使当前 HEAD 已经是 docs 状态同步提交，也要再次用 `git status --short --branch` 与 `git log -25 --oneline` 回填当前真实 docs HEAD，不能让 checklist 或 next-session prompt 停在上一轮恢复入口。
- 每个阶段性任务完成并验证通过后，默认自动执行状态同步：更新 development checklist、`next-session-prompt.md`、`tasks/todo.md` Review 和必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档；最终回复必须给出本轮实际 docs HEAD 和可复制启动提示词。
- 状态同步不改变 Phase 5 no-go：native 继续 default off / 显式 opt-in；默认 smoke 不联网；不创建 packaged native binary；不修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；不新增真实 `@codeinsights/native-search-*` optionalDependencies；不 push，不创建 PR。
...
- 用户要求“更新文档最新开发状态、标注完成 / 未完成、给下次启动提示词”时，即使上一轮刚完成状态同步，也要再次用 `git status --short --branch` 与 `git log -25 --oneline` 回填当前真实 docs HEAD，不能让 checklist 或 next-session prompt 停在上一轮恢复入口。
- 每个阶段性任务完成并验证通过后，默认执行状态同步：更新 development checklist、`next-session-prompt.md`、`tasks/todo.md` Review 和必要 lessons，运行 no-go 边界验证，并单独提交状态同步文档；最终回复给出本轮实际 docs HEAD 和可复制提示词。
- 本轮只做恢复入口与完成 / 未完成状态回填，不改变 Phase 5 no-go：native 继续 default off / 显式 opt-in；默认 smoke 不联网；不创建 packaged native binary；不修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；不新增真实 `@codeinsights/native-search-*` optionalDependencies；不 push，不创建 PR。

## 2026-06-06 Rust / Go Phase 5 release handoff 证据计划边界

- `4fec7fab feat(rust-go): 补齐 Phase 5 release handoff 证据计划` 已成为当前最新开发基线；后续 Rust / Go 下次启动提示词的最新开发基线应指向 `4fec7fab`，最新已确认恢复入口在本轮状态同步提交前仍是 `6525f6fb docs(rust-go): 回填 Phase 5 publication invocation 最新恢复入口`。
- `nativeSearchReleaseHandoffPlan` 只能作为真实 release / install / builder / packaged smoke / default-enable 前的交接证据聚合器。`ready_for_release_handoff` 只说明当前顺序 gate 的下一步可交给人工审核或执行，不等于真实 package 已发布、optionalDependencies 已声明或安装、builder allowlist 已修改、packaged binary 已验证或 native 可默认启用。
- release handoff 的 ready 字段必须绑定 `optionalPackageExecutionPlan.nextStage`。后置 packaged smoke 布尔值、`ready_for_execution` 或手工构造的 `bundledBinaryVerified` 测试输入不能跳过 publish-target / source / publication 顺序 gate；如果上游 gate 未完成，handoff 必须 fail-closed 到上游 `nextGate`。
- 本轮仍不改变 Phase 5 no-go：native 继续 default off / 显式 opt-in；默认 smoke 不联网；不创建 packaged native binary；不修改根 `README.md` / 根 `AGENTS.md` / `apps/electron/electron-builder.yml`；不新增真实 `@codeinsights/native-search-*` optionalDependencies；不 push，不创建 PR。
...
- `d679eabb fix(rust-go): 拆分 Phase 5 publication plan 发布证据` 曾是 publication change plan 证据拆分后的最新开发基线；本条已被上方 `edeba827` lesson 覆盖，后续下次启动提示词的最新开发基线不应继续指向 `d679eabb`。
