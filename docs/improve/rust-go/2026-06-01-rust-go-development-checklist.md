...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、fake sidecar / cache-corruption smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture 和 `packaged-app-layout` 只读 preflight smoke；native 未达默认启用门槛，packaged native binary / 真实 optional package / optionalDependencies / 真实 packaged app bundled binary smoke 尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含本轮 `packaged-app-layout` preflight 提交（提交后以实际 `git log` 为准）、`24d1b657 docs(rust-go): 回填 Phase 5 bundled resolver 最新恢复入口`、`e95cd284 docs(rust-go): 同步 Phase 5 bundled resolver 后续状态`、`ce104a59 feat(rust-go): 补齐 Phase 5 bundled package resolver fixture`。如需确认更早 optional manifest / fake sidecar / native smoke / benchmark 文档，再用 `git log -12 --oneline` 或更深日志确认 `8226c992`、`e5b92fa7`、`9fe91b03`、`fb7e2d73`、`2cc95b1b`、`9c102c0a`、`c6104eee`、`4f9c4d28`。
...
- [x] 阶段开始
- [x] 依赖评估完成
- [~] 测试先行完成（Rust search-only sidecar、sidecar manager、contract parity、benchmark、smoke script、native-cache schema、optional package manifest 预检和 bundled package resolver fixture 测试已完成；真实 packaged app bundled binary smoke 测试尚未完成）
- [~] 实现完成（Rust search-only sidecar、Electron main process sidecar manager、Chat opt-in native search、benchmark 增强、基础 smoke script、native-cache schema helper、optional package manifest 预检和 bundled package resolver fixture 已完成；真实 optional package、optionalDependencies、packaged binary 和 default enable 尚未完成）
- [~] 验证完成（Rust / native runtime tests / typecheck / build:main / benchmark / packaged-manifest preflight / resolver fixture smoke 已通过；真实 packaged app bundled binary smoke 尚未完成）
- [~] 阶段 Review 完成（已记录前置、search-only、sidecar manager、性能优化、benchmark gate、smoke/cache schema、fake sidecar smoke、optional package manifest 预检和 bundled package resolver fixture Review；完整 Phase 5 Review 尚未完成）
- [ ] 阶段提交完成
