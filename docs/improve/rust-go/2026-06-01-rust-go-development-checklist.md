...
> 最新已确认恢复入口：`3d82125e docs(rust-go): 同步 Phase 5 optional packaged 执行顺序后续状态`；若本轮状态同步提交后，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
...
3. 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `3d82125e docs(rust-go): 同步 Phase 5 optional packaged 执行顺序后续状态`、`ce89f490 feat(rust-go): 补齐 Phase 5 optional packaged 执行顺序预检`、`10faa4a3 docs(rust-go): 回填 Phase 5 optional package source 最新恢复入口`、`cf17e7da docs(rust-go): 同步 Phase 5 optional package source 后续状态`、`05c393bb feat(rust-go): 补齐 Phase 5 optional package source 预检`、`e11f4476 docs(rust-go): 回填 Phase 5 release source 最新恢复入口`、`1b496638 docs(rust-go): 同步 Phase 5 release source version 后续状态`、`dc75f380 feat(rust-go): 对齐 Phase 5 native search release source version`、`63507795 docs(rust-go): 回填 Phase 5 publish target 最新恢复入口`、`92a6f1a6 docs(rust-go): 同步 Phase 5 publish target 后续状态`、`9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`、`a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口` 和 `33d0990d docs(rust-go): 同步 Phase 5 packaged smoke 执行计划后续状态`。
