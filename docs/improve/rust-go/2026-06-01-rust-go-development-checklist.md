...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、fake sidecar / cache-corruption smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` gate 和 optionalDependencies declaration preflight gate；native 未达默认启用门槛，packaged native binary / 真实 optional package 发布 / optionalDependencies 实际声明与安装链路 / 真实 packaged app bundled binary smoke 尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含 `563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate`、`16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`、`31e37cbb fix(rust-go): 收紧 Phase 5 packaged app evidence 真实验证`、`800dc885 feat(rust-go): 补齐 Phase 5 packaged app evidence smoke` 和 `52613780 feat(rust-go): 补齐 Phase 5 packaged app layout 预检 smoke`；再用 `git log -8 --oneline` 确认 `24d1b657 docs(rust-go): 回填 Phase 5 bundled resolver 最新恢复入口`、`e95cd284 docs(rust-go): 同步 Phase 5 bundled resolver 后续状态`、`ce104a59 feat(rust-go): 补齐 Phase 5 bundled package resolver fixture` 和 `8226c992` 仍在历史中，如需确认更早 optional manifest / fake sidecar / native smoke / benchmark 文档，再用更深日志确认 `e5b92fa7`、`9fe91b03`、`fb7e2d73`、`2cc95b1b`、`9c102c0a`、`c6104eee`、`4f9c4d28`。
...
5. 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进真实 optional package / 真实 packaged app bundled binary smoke 设计，或继续分析 Agent native work-delay 小幅回退。只有 P95 / event loop gate 同时达标且真实 packaged smoke 进入可执行阶段后，才允许默认启用 native。
...
- [x] 阶段开始
- [x] 依赖评估完成
- [~] 测试先行完成（Rust search-only sidecar、sidecar manager、contract parity、benchmark、smoke script、native-cache schema、optional package manifest 预检、bundled package resolver fixture、packaged app layout、packaged app evidence classifier 和 optionalDependencies declaration preflight gate 测试已完成；真实 packaged app bundled binary smoke 测试尚未完成）
- [~] 实现完成（Rust search-only sidecar、Electron main process sidecar manager、Chat opt-in native search、benchmark 增强、基础 smoke script、native-cache schema helper、optional package manifest 预检、bundled package resolver fixture、packaged app layout preflight、packaged app evidence classifier 和 optionalDependencies declaration preflight gate 已完成；真实 optional package 发布 / optionalDependencies 实际声明与安装链路、packaged binary 和 default enable 尚未完成）
- [~] 验证完成（Rust / native runtime tests / typecheck / build:main / benchmark / packaged-manifest preflight / resolver fixture smoke / packaged app layout smoke / optionalDependencies declaration preflight 已通过；真实 packaged app bundled binary smoke 尚未完成）
- [~] 阶段 Review 完成（已记录前置、search-only、sidecar manager、性能优化、benchmark gate、smoke/cache schema、fake sidecar smoke、optional package manifest 预检、bundled package resolver fixture、packaged app layout preflight、packaged app evidence classifier 和 optionalDependencies declaration preflight Review；完整 Phase 5 Review 尚未完成）
- [ ] 阶段提交完成
...
启动后先运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含最新 Rust / Go docs 同步提交（如存在）、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate`、`16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`、`31e37cbb fix(rust-go): 收紧 Phase 5 packaged app evidence 真实验证`、`800dc885 feat(rust-go): 补齐 Phase 5 packaged app evidence smoke`；再用 `git log -8 --oneline` 确认 `52613780 feat(rust-go): 补齐 Phase 5 packaged app layout 预检 smoke`、`24d1b657 docs(rust-go): 回填 Phase 5 bundled resolver 最新恢复入口`、`e95cd284 docs(rust-go): 同步 Phase 5 bundled resolver 后续状态`、`ce104a59 feat(rust-go): 补齐 Phase 5 bundled package resolver fixture` 和 `8226c992`，如需确认更早 optional manifest / fake sidecar / native smoke / benchmark 文档，再用更深日志确认 `e5b92fa7`、`9fe91b03`、`fb7e2d73`、`2cc95b1b`、`9c102c0a`、`c6104eee`、`4f9c4d28`。下一步继续 Phase 5“Rust search sidecar 试点”：在 default off 前提下优先推进真实 optional package / `optionalDependencies` 实际声明与安装链路 / 真实 packaged app bundled binary smoke，或继续分析 Agent native work-delay 小幅回退；未达到 P95 / event loop gate 且真实 packaged smoke 未完成前不得默认启用 native。不要创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。
