...
> 本轮新增：`ddc818ae` 把 builder allowlist 实际修改前的 YAML edit intent 收敛为闭合的 `candidateYamlEditPlan`：当前仓库仍需要添加 4 个 `node_modules/@codeinsights/native-search-*/**/*` 的精确 per-package include，并移除阻断规则 `!node_modules/@codeinsights/**`；同时继续禁止 `node_modules/**`、`node_modules/@codeinsights/**` 和 `node_modules/@codeinsights/native-search-*` 这类宽泛 include。该 plan 的 `modifiesFile=false` 明确说明本轮没有改 `apps/electron/electron-builder.yml`，且不得把 plan 当成 `packagingConfigVerified` 或 packaged binary verified。`3f5a0783` 的 release handoff fail-closed 绑定仍要求 authoritative next gate 来自 `optionalPackageExecutionPlan.nextStage`，publication / packaged smoke evidence 不能乱序跳过上游 gate。
...
- [x] Phase 5 builder allowlist 修改意图状态同步：`f02ceae0` 已回填 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 lessons；该提交只同步状态，不改变真实 publication / install-chain / builder / packaged smoke gate。
