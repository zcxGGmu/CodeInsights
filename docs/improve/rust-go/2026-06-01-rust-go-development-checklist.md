...
5. 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进真实 optional package 发布 / 实际 optionalDependencies 声明与安装执行、真实 packaged app bundled binary smoke 设计，或 default-enable 风险评估前置清单。只有 P95 / event loop gate、Agent facade parity 和真实 packaged smoke 同时进入可执行通过阶段后，才允许默认启用 native。
...
启动后先运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `665c5db7 feat(rust-go): 补齐 Phase 5 Agent nested native parity`、`c7726a56 docs(rust-go): 回填 Phase 5 Agent facade 最新恢复入口`、`a6f79e14 docs(rust-go): 同步 Phase 5 Agent facade benchmark 后续状态`、`9a06908a feat(rust-go): 补齐 Phase 5 Agent facade benchmark 分析`、`468e5395 docs(rust-go): 同步 Phase 5 native benchmark gate 后续状态`、`f86553ee feat(rust-go): 补齐 Phase 5 native benchmark gate 汇总`、`7891cfc6 docs(rust-go): 回填 Phase 5 optional package 安装链路恢复入口`、`f32d409f docs(rust-go): 同步 Phase 5 optional package 安装链路后续状态`、`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检`、`eb715c46 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`、`05df1687 docs(rust-go): 同步 Phase 5 optionalDependencies 预检后续状态`、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate`、`16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`、`31e37cbb fix(rust-go): 收紧 Phase 5 packaged app evidence 真实验证`、`800dc885 feat(rust-go): 补齐 Phase 5 packaged app evidence smoke`、`52613780 feat(rust-go): 补齐 Phase 5 packaged app layout 预检 smoke`、`24d1b657 docs(rust-go): 回填 Phase 5 bundled resolver 最新恢复入口`、`e95cd284 docs(rust-go): 同步 Phase 5 bundled resolver 后续状态`、`ce104a59 feat(rust-go): 补齐 Phase 5 bundled package resolver fixture` 和 `8226c992 docs(rust-go): 同步 Phase 5 optional package manifest 预检状态`。下一步继续 Phase 5“Rust search sidecar 试点”：在 default off 前提下优先推进真实 optional package 发布 / `optionalDependencies` 实际声明与安装执行、真实 packaged app bundled binary smoke 设计，或 default-enable 风险评估前置清单；真实 packaged smoke 与 install-chain 未完成前不得默认启用 native。不要创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。
