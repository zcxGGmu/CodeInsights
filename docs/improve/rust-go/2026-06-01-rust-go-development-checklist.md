...
- [ ] 真实 Rust optional package 发布 / optionalDependencies 实际声明与安装执行、真实 packaged app bundled binary smoke 和 default-enable 判断尚未实现；当前 npm registry 查询显示 4 个计划 `@codeinsights/native-search-*` 包为 `E404`，且当前 packaging config allowlist 预检明确报告 builder files 未允许这些 package，因此不能在不发布真实包 / 不获准调整 builder allowlist 的前提下安全声明并安装真实 optionalDependencies。
