...
> 本轮新增：`edeba827` 新增 `--packaged-app-root` packaged smoke 调用入口和 `packagedBundledBinarySmokeInvocationPlan`，把真实 packaged app root 解析、候选命令、legacy `--app-node-modules-root` 兼容命令、验收证据、required inputs、blocked reason 和 forbidden actions 机器可读化。`.app/Contents/Resources/app/node_modules`、`.app/Contents/Resources/app.asar.unpacked/node_modules`、直接 `Resources`、直接 `Resources/app` 与直接 `app.asar.unpacked` 均只输出 evidence code，不输出 packaged root、node_modules root 或 binary path；legacy node_modules 参数仍优先，但缺失路径只会进入 `app_node_modules_root_unresolved`，不能被标成 resolved。该 invocation plan 不创建 packaged app、不发布、不安装、不改 builder、不证明 `bundledBinaryVerified`。`8097ed4c` / `2188e276` / `d679eabb` 的 publication plan 证据边界仍成立：`publishedPackages` 只表示 registry exact expected version 且 metadata 有效的真实 publication 证据；collision / invalid metadata 只进入 exclusion / blocker。
