...
- [ ] 真实 Rust optional package / optionalDependencies、bundled package resolver、packaged smoke 和 default-enable 判断尚未实现。
...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、fake sidecar / cache-corruption smoke、optional package manifest schema helper 和 `packaged-manifest` 预检；native 未达默认启用门槛，packaged native binary / 真实 optional package / bundled package resolver / packaged smoke 尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含 `e5b92fa7 feat(rust-go): 补齐 Phase 5 optional package manifest 预检`、`9fe91b03 docs(rust-go): 回填 Phase 5 fake sidecar smoke 最新恢复入口`、`fb7e2d73 docs(rust-go): 同步 Phase 5 fake sidecar smoke 后续状态`、`2cc95b1b feat(rust-go): 补齐 Phase 5 fake sidecar smoke 失败路径`。如需确认更早 native smoke / benchmark 文档，再用 `git log -8 --oneline` 确认 `9c102c0a`、`c6104eee`、`4f9c4d28`。
...
5. 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进 bundled package resolver / 真实 packaged smoke 设计，或继续分析 Agent native work-delay 小幅回退。只有 P95 / event loop gate 同时达标且真实 packaged smoke 进入可执行阶段后，才允许默认启用 native。
...
- [x] 阶段开始
- [x] 依赖评估完成
- [~] 测试先行完成（Rust search-only sidecar、sidecar manager、contract parity、benchmark、smoke script、native-cache schema 和 optional package manifest 预检测试已完成；真实 packaged smoke 测试尚未完成）
- [~] 实现完成（Rust search-only sidecar、Electron main process sidecar manager、Chat opt-in native search、benchmark 增强、基础 smoke script、native-cache schema helper 和 optional package manifest 预检已完成；真实 optional package、packaged binary 和 default enable 尚未完成）
- [~] 验证完成（Rust / native runtime tests / typecheck / build:main / benchmark / packaged-manifest preflight 已通过；真实 packaged smoke 尚未完成）
- [~] 阶段 Review 完成（已记录前置、search-only、sidecar manager、性能优化、benchmark gate、smoke/cache schema、fake sidecar smoke 和 optional package manifest 预检 Review；完整 Phase 5 Review 尚未完成）
- [ ] 阶段提交完成
