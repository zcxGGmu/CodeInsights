...
> 最新已确认恢复入口：`eb715c46 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`；若本轮再次产生状态同步提交，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
