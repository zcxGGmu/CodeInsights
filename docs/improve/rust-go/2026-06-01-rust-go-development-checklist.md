...
> 更新时间：2026-06-04
> 最新开发基线：`9a06908a feat(rust-go): 补齐 Phase 5 Agent facade benchmark 分析`。
> 最新已确认恢复入口：`468e5395 docs(rust-go): 同步 Phase 5 native benchmark gate 后续状态`；若本轮再次产生状态同步提交，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
> 当前结论：Rust / Go 优化重构 Phase 0、Phase 1、Phase 2、Phase 3 和 Phase 4 已完成；已建立 shared NativeRuntime DTO / fixtures / benchmark，在 TypeScript fallback 内收敛 Chat / Agent / Pipeline 搜索 facade，完成 Pipeline records cursor tail 与 SearchDialog Pipeline 内容搜索接入，完成 TypeScript workspace 文件索引 cache、watcher invalidation、SearchDialog Workspace 文件分组和安全路径白名单，并补齐 Native Runtime diagnostics 的 IPC / preload / Jotai / Settings UI / SearchDialog 状态可见体验。Phase 5 已完成前置依赖 decision record、sidecar protocol / fallback / packaged smoke 计划、100MB TS fallback benchmark gate、`native/search/` 最小 Rust search-only sidecar 源码切片、Electron main process sidecar manager、Chat search opt-in fallback 边界、Rust vs TS contract parity、100MB native benchmark 对比、search `limit + 1` 早停性能优化、benchmark event-loop 口径增强、native benchmark gate 机器可读汇总、Agent production facade benchmark 分析、基础 `smoke:native-runtime` 脚本、native-cache manifest schema helper、`protocol-mismatch` / `crash` / `timeout` / `cache-corruption` fake sidecar / isolated cache smoke、optional package manifest schema helper / `packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke 入口、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` 真实 packaged app identity gate、optionalDependencies declaration preflight gate，以及 optional package install-chain preflight gate；尚未实现 packaged native binary、真实 optional package 发布 / optionalDependencies 实际声明与安装执行、真实 packaged app bundled binary smoke、Agent facade native parity 或 Go supervisor。
> 当前策略：Phase 5 native search 仍不能默认启用。本轮已用新增 event-loop samples / baseline / work delay 字段复跑 2 轮 100MB / 20 iterations 稳定 benchmark；Rust native Chat / Agent P95 仍远超性能门槛，Chat work delay gate 达标，但 Agent native `eventLoopWorkDelayP95Ms` 两轮仍高于 TS fallback（0.065ms vs 0ms；0.164ms vs 0.047ms），未扣 baseline 的 Agent `eventLoopDelayP95Ms` 也两轮小幅回退（1.199ms vs 1.121ms；1.191ms vs 1.114ms）。这些差异绝对值很小，但方向仍不支持 default enable；`f86553ee` 已让 benchmark summary 输出 `nativeSearchGate`，机器可读记录 Chat / Agent native vs TS 的 P95、event-loop delay P95 和 work-delay P95 delta，并固定 `optional_package_install_chain_not_evaluated`、`packaged_app_bundled_binary_not_evaluated` 为默认启用阻塞项。`9a06908a` 进一步新增 `agentFacadeSearch` 与 `agent-runtime-production-facade-search`，证明当前生产 Agent 搜索支持 nested SDK message text，但未声明 `nativeTextFields`，即使 sidecar 可用仍是 TypeScript facade / fallback，且该 case 不进入 direct native `nativeSearchGate`。即使 fake sidecar / cache-corruption smoke、`packaged-manifest` 预检、resolver fixture、optionalDependencies declaration preflight gate 和 install-chain preflight gate 已补齐，在真实 packaged app bundled binary smoke、真实 optional package 发布 / optionalDependencies 实际声明与安装执行、Agent facade native parity 和 default-enable 风险评估完成前，native 必须继续保持显式 opt-in / default off。Go supervisor 仅作为 Phase 9 有条件 spike，不进入默认主线。
...
- [~] Phase 5：Rust search sidecar 试点（前置依赖决策、protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar 源码切片、Electron main process sidecar manager、fallback gate、contract parity、native benchmark 对比、search 早停性能优化、2 轮稳定 benchmark 复跑、benchmark event-loop 口径增强、新字段稳定 benchmark 复跑、native benchmark gate 机器可读汇总、Agent production facade benchmark 分析、基础 smoke script、native-cache schema helper、`protocol-mismatch` / `crash` / `timeout` / `cache-corruption` fake sidecar / isolated cache smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke 入口、packaged app evidence classifier、`packagedAppIdentityVerified` gate、optionalDependencies declaration preflight gate 和 optional package install-chain preflight gate 已完成；Agent direct native benchmark 与生产 Agent facade 口径仍不等价，真实 optional package 发布 / optionalDependencies 实际声明与安装执行、真实 packaged app bundled binary smoke、Agent facade native parity、default-enable 风险评估未完成，default enable 与真实 packaged smoke 尚未完成）。
...
- [ ] 真实 Rust optional package 发布 / optionalDependencies 实际声明与安装执行、真实 packaged app bundled binary smoke 和 default-enable 判断尚未实现；当前 npm registry 查询显示 4 个计划 `@codeinsights/native-search-*` 包为 `E404`，且 `apps/electron/electron-builder.yml` 当前排除 `node_modules/@codeinsights/**`，因此不能在不发布真实包 / 不调整 builder allowlist 的前提下安全声明并安装真实 optionalDependencies。
...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、native benchmark gate 汇总、Agent production facade benchmark 分析、fake sidecar / cache-corruption smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` gate、optionalDependencies declaration preflight gate 和 optional package install-chain preflight gate；native 未达默认启用门槛，packaged native binary / 真实 optional package 发布 / optionalDependencies 实际声明与安装执行 / 真实 packaged app bundled binary smoke / Agent facade native parity 尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -20 --oneline`，确认最近历史包含 `9a06908a feat(rust-go): 补齐 Phase 5 Agent facade benchmark 分析`、`468e5395 docs(rust-go): 同步 Phase 5 native benchmark gate 后续状态`、`f86553ee feat(rust-go): 补齐 Phase 5 native benchmark gate 汇总`、`7891cfc6 docs(rust-go): 回填 Phase 5 optional package 安装链路恢复入口`、`f32d409f docs(rust-go): 同步 Phase 5 optional package 安装链路后续状态`、`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检`、`eb715c46 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`、`05df1687 docs(rust-go): 同步 Phase 5 optionalDependencies 预检后续状态`、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate`、`16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`、`31e37cbb fix(rust-go): 收紧 Phase 5 packaged app evidence 真实验证`、`800dc885 feat(rust-go): 补齐 Phase 5 packaged app evidence smoke`、`52613780 feat(rust-go): 补齐 Phase 5 packaged app layout 预检 smoke`、`24d1b657 docs(rust-go): 回填 Phase 5 bundled resolver 最新恢复入口`、`e95cd284 docs(rust-go): 同步 Phase 5 bundled resolver 后续状态`、`ce104a59 feat(rust-go): 补齐 Phase 5 bundled package resolver fixture` 和 `8226c992 docs(rust-go): 同步 Phase 5 optional package manifest 预检状态`。
...
5. 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进真实 optional package 发布 / 实际 optionalDependencies 声明与安装执行、真实 packaged app bundled binary smoke 设计，或设计 Agent facade nested content native parity。只有 P95 / event loop gate、Agent facade parity 和真实 packaged smoke 同时进入可执行通过阶段后，才允许默认启用 native。
...
- [x] 阶段开始
- [x] 依赖评估完成
- [~] 测试先行完成（Rust search-only sidecar、sidecar manager、contract parity、benchmark、native benchmark gate 汇总、Agent production facade benchmark、smoke script、native-cache schema、optional package manifest 预检、bundled package resolver fixture、packaged app layout、packaged app evidence classifier、optionalDependencies declaration preflight gate 和 optional package install-chain preflight gate 测试已完成；真实 packaged app bundled binary smoke 测试尚未完成）
- [~] 实现完成（Rust search-only sidecar、Electron main process sidecar manager、Chat opt-in native search、benchmark 增强、native benchmark gate 汇总、Agent production facade benchmark 分析、基础 smoke script、native-cache schema helper、optional package manifest 预检、bundled package resolver fixture、packaged app layout preflight、packaged app evidence classifier、optionalDependencies declaration preflight gate 和 optional package install-chain preflight gate 已完成；真实 optional package 发布 / optionalDependencies 实际声明与安装执行、packaged binary、Agent facade native parity 和 default enable 尚未完成）
- [~] 验证完成（Rust / native runtime tests / typecheck / build:main / benchmark / native benchmark gate 汇总 / Agent production facade benchmark / packaged-manifest preflight / resolver fixture smoke / packaged app layout smoke / optionalDependencies declaration preflight / optional package install-chain preflight 已通过；真实 packaged app bundled binary smoke 尚未完成）
- [~] 阶段 Review 完成（已记录前置、search-only、sidecar manager、性能优化、benchmark gate、native benchmark gate 汇总、Agent production facade benchmark、smoke/cache schema、fake sidecar smoke、optional package manifest 预检、bundled package resolver fixture、packaged app layout preflight、packaged app evidence classifier、optionalDependencies declaration preflight 和 optional package install-chain preflight Review；完整 Phase 5 Review 尚未完成）
- [ ] 阶段提交完成
...
- 子代理复核结论：Agent native work-delay 小幅回退更可能来自 benchmark 口径、sidecar hop 固定成本和短调用噪声；生产 Agent 搜索当前不应直接加 `nativeTextFields`，因为 Agent message 可包含嵌套 SDK content，而 Rust sidecar 目前只抽 top-level string fields。
