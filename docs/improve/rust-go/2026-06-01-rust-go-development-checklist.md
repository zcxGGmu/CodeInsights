...
- [x] Phase 5 optional package publish-target dry-run 预检已完成：`9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检` 新增 `validateNativeSearchOptionalPackagePublishTarget()` 和 `smoke:native-runtime -- --mode optional-package-publish-target --native-search-package-version <version>`。默认不联网时保持 `optionalPackagePublishTargetChecked=false` 和 `optionalPackagePublishTargetReady=false`；显式 `--check-registry` 只读检查目标版本是否已碰撞，并验证 planned manifest metadata、Cargo version 与 source `BINARY_VERSION` 一致性。当前 `0.0.3` 目标版本下 4 个 package 名均可发布、无 registry collision，但 `native/search/src/lib.rs` 仍为 `0.0.3-dev`，因此 `optionalPackagePublishTargetReady=false` 且 blocker 为 `native_search_binary_version_not_release_ready`。该 dry-run 不发布 package、不安装、不声明 optionalDependencies、不改变 `optionalPackagesPublished`、不证明真实 packaged binary。
- [ ] 真实 Rust optional package 发布 / optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke 和 default-enable 判断尚未实现；当前显式 publication registry smoke 仍显示 4 个计划 `@codeinsights/native-search-*` 包均在 `missingPublishedOptionalPackages`，publish-target dry-run 仅证明目标版本当前可用但被 `native_search_binary_version_not_release_ready` 阻断，且当前 packaging config allowlist 预检明确报告 builder files 未允许这些 package，因此不能在不发布真实包 / 不获准调整 builder allowlist 的前提下安全声明并安装真实 optionalDependencies。
...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、native benchmark gate 汇总、Agent production facade benchmark 分析、Agent nested native parity、fake sidecar / cache-corruption smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` gate、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检和 optional package publish-target dry-run 预检；native 未达默认启用门槛，packaged native binary / 真实 optional package 发布 / optionalDependencies 实际声明与安装执行 / 真实 packaged app bundled binary smoke / builder allowlist 实际修改 / 最终 default-enable 风险决策尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`、`a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口`、`33d0990d docs(rust-go): 同步 Phase 5 packaged smoke 执行计划后续状态`、`dedbfed1 feat(rust-go): 补齐 Phase 5 packaged smoke 执行计划预检`、`d2009562 docs(rust-go): 同步 Phase 5 optional package 发布预检后续状态`、`6ae1c896 feat(rust-go): 补齐 Phase 5 optional package 发布状态预检`、`60bdd3a3 docs(rust-go): 回填 Phase 5 packaging config 最新恢复入口`、`f359dadc docs(rust-go): 同步 Phase 5 packaging config allowlist 后续状态`、`348e003d feat(rust-go): 补齐 Phase 5 packaging config allowlist 预检`、`e4d38aff docs(rust-go): 同步 Phase 5 default-enable readiness 后续状态`、`2bf88a5a feat(rust-go): 补齐 Phase 5 default-enable readiness 预检`、`97db0052 docs(rust-go): 同步 Phase 5 Agent nested parity 后续状态`、`665c5db7 feat(rust-go): 补齐 Phase 5 Agent nested native parity`、`c7726a56 docs(rust-go): 回填 Phase 5 Agent facade 最新恢复入口`、`a6f79e14 docs(rust-go): 同步 Phase 5 Agent facade benchmark 后续状态`、`9a06908a feat(rust-go): 补齐 Phase 5 Agent facade benchmark 分析`、`468e5395 docs(rust-go): 同步 Phase 5 native benchmark gate 后续状态`、`f86553ee feat(rust-go): 补齐 Phase 5 native benchmark gate 汇总`、`7891cfc6 docs(rust-go): 回填 Phase 5 optional package 安装链路恢复入口`、`f32d409f docs(rust-go): 同步 Phase 5 optional package 安装链路后续状态`、`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检`、`eb715c46 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`、`05df1687 docs(rust-go): 同步 Phase 5 optionalDependencies 预检后续状态`、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate` 和 `16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`。
...
5. 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进真实 optional package 发布 / 实际 optionalDependencies 声明与安装执行、真实 packaged app bundled binary smoke 设计，或仅在真实 optional / packaged gate 可执行通过后再做最终 default-enable 风险决策。只有 P95 / event loop gate、Agent facade parity、真实 optional install-chain 和真实 packaged smoke 同时进入可执行通过阶段后，才允许默认启用 native。
...
- Rust sidecar `0.0.3` / `0.0.3-dev` 现在支持搜索顶层 legacy `content` 或 `message.content[]` 中 `type="text"` 的字符串 block；不会搜索 raw JSON、`tool_use.input`、`tool_use.name` 或非 text block。直接 sidecar snippet 仍做 Bearer / credentialed URL 脱敏。
...
- 当前默认离线 dry-run 输出 `optionalPackagePublishTargetChecked=false`、`optionalPackagePublishTargetReady=false`，blockers 包含 `registry_check_required` 和 `native_search_binary_version_not_release_ready`，exit 0；显式 `--check-registry` 只读查询 npm 后，4 个 planned packages 目标版本当前可发布、无 registry collision，但仍因 `native/search/src/lib.rs` 为 `0.0.3-dev` 输出 `native_search_binary_version_not_release_ready`，exit 1。
...
当前进度：Rust / Go 优化方案、开发跟踪清单、阶段提交纪律、Phase 0“基线、契约与 benchmark”、Phase 1“TypeScript fallback 与 EventSearchService 重构”、Phase 2“Pipeline records cursor / tail 与全局搜索接入”、Phase 3“Workspace 文件索引 TS cache 与 watcher invalidation”和 Phase 4“前端可见体验、Jotai 状态和 diagnostics”已经完成。Phase 5 已完成前置计划、依赖 decision record、sidecar protocol / fallback / packaged smoke 计划、100MB 级 TS fallback benchmark gate、`native/search/` 最小 Rust search-only sidecar 源码切片、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark 对比、Rust search 早停性能优化、2 轮稳定 benchmark 复跑、benchmark event-loop 口径增强、新字段稳定 benchmark 复跑、native benchmark gate 汇总、Agent production facade benchmark 分析、Agent nested native parity、基础 `smoke:native-runtime` 脚本、native-cache manifest schema helper、`protocol-mismatch` / `crash` / `timeout` / `cache-corruption` fake sidecar / isolated cache smoke、optional package manifest schema helper / `packaged-manifest` 预检、bundled package resolver fixture、packaged app layout preflight smoke、packaged app evidence classifier、`packagedAppIdentityVerified` gate、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate`、`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检`、`f86553ee feat(rust-go): 补齐 Phase 5 native benchmark gate 汇总`、`9a06908a feat(rust-go): 补齐 Phase 5 Agent facade benchmark 分析`、`665c5db7 feat(rust-go): 补齐 Phase 5 Agent nested native parity`、`2bf88a5a feat(rust-go): 补齐 Phase 5 default-enable readiness 预检`、`348e003d feat(rust-go): 补齐 Phase 5 packaging config allowlist 预检`、`6ae1c896 feat(rust-go): 补齐 Phase 5 optional package 发布状态预检`、`dedbfed1 feat(rust-go): 补齐 Phase 5 packaged smoke 执行计划预检`，以及 `9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`。最新开发基线是 `9e3e3dff`，最新已确认恢复入口是 `a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口`；若本轮再次产生状态同步提交，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。

已完成的 Phase 5 sidecar manager 切片：Rust crate 直接依赖固定为 `serde = 1.0.228`、`serde_json = 1.0.150`；实现 stdin / stdout line-delimited JSON protocol、`status`、literal `search`、`shutdown` 和 explicit out-of-scope `tail_jsonl` typed error；Electron main process 通过 `native-runtime-sidecar-manager.ts` 显式 binary path opt-in，不从系统 PATH 查找；覆盖 missing binary、version mismatch、timeout、crash、shutdown、bad JSON stdout、stdout buffer 上限、contract violation、source boundary violation、range violation、拒绝类错误不 fallback、Chat legacy snippet parity、真实 Rust vs TS parity。`0eb350ff` 已把 Rust search 升级到 `0.0.2` / `0.0.2-dev`，在第 `limit + 1` 个命中后早停并返回 `hasMore=true`；`665c5db7` 已把 Rust search 升级到 `0.0.3` / `0.0.3-dev`，新增受限白名单 extractor `agent_message_search_text`，只搜索顶层 legacy `content` 或 `message.content[]` 中 `type=\"text\"` 的字符串 block，不搜索 raw JSON / `tool_use` / 任意 nested path。最新 2 轮 100MB / 20 iterations 新字段 benchmark 显示 native P95 继续稳定达标，Chat work delay gate 达标，但 Agent native event-loop work delay 两轮仍小幅高于 TS；`f86553ee` 已新增 `nativeSearchGate`，机器可读输出 Chat / Agent TS vs native 的 P95、event-loop delay P95、work-delay P95 delta、benchmark blockers 和 default-enable blockers，无 native binary 或 benchmark 全部通过时都不会把 `defaultEnableCandidate` 误置为 true。`9a06908a` 已新增 `agentFacadeSearch` 和 `agent-runtime-production-facade-search`；`665c5db7` 进一步让生产 Agent source 可声明 `nativeTextExtractor=\"agent_message_search_text\"`，并新增 `native-agent-runtime-production-facade-search` parity case。native 命中仍只作为 cursor anchor，legacy snippet / messageId 继续由 TypeScript 回读 JSONL 后按生产规则重建；direct native Agent benchmark 仍是 synthetic top-level case，不能外推为 default enable 依据。基础 smoke/cache 切片：`smoke:native-runtime` 支持 native-missing / native-available / protocol-mismatch / crash / timeout / cache-corruption / packaged-manifest / packaged-app-layout / optional-package-publish-target 入口；native available 无显式 binary 时 skipped，不从 PATH 查找；native-cache manifest root 固定为 `getConfigDir()/native-cache`，记录 schema / protocol / package plan，不记录 binary path，损坏 manifest 返回 `cache_corrupted`。`e5b92fa7` 已补齐 optional package manifest schema helper 和 `packaged-manifest` manifest 预检；`ce104a59` 已补齐 bundled package resolver fixture，resolver 使用 app `node_modules` realpath allowlist、固定 manifest / binary name、可执行权限和 SHA-256 fingerprint 校验，且只在 Electron packaged 环境自动尝试。`800dc885` 已补齐 packaged app evidence classifier，`packaged-app-layout` 可识别 `asar-unpacked` 和当前 `asar:false` 的 `unpacked-app` 布局；`31e37cbb` 已收紧真实验证。`563b804c` 新增 optionalDependencies declaration preflight，`1114233b` 进一步新增 optional package install-chain preflight：`packaged-manifest` summary 明确 `optionalDependenciesDeclared=false`、4 个 `missingOptionalDependencies`、`invalidOptionalDependencies=[]`、`optionalDependenciesInstallChainVerified=false`、`optionalDependenciesLockfileVerified=false`、`optionalDependenciesInstalledPackagesVerified=false`、`bundledBinaryVerified=false`、`fixtureBundledPackageVerified=true`、`usesTemporaryFixture=true`、`realPackagedBinaryVerified=false`；`realPackagedBinaryVerified=true` 必须同时满足 resolver 成功、非临时 fixture、`packagedAppEvidenceVerified=true`、`packagedAppIdentityVerified=true`、`optionalDependenciesDeclared=true`、`optionalDependenciesInstallChainVerified=true` 和真实 optional package / binary 存在。optionalDependencies 版本声明只接受 exact semver 或 `npm:<同名 native search package>@exact-semver`，拒绝 `latest` / range、跨包 alias、`file:`、`workspace:`、git / http URL 和 path-like spec；`bun.lock` 预检必须匹配 resolved package entry，声明存在但 lockfile / installed package manifest 不完整时 install-chain smoke 会 failed。`2bf88a5a` 已新增 `nativeSearchDefaultEnableReadiness`，benchmark / smoke summary 都会输出 readiness；`348e003d` 又新增 packaging config allowlist 预检，当前 smoke 输出 `packagingConfigVerified=false`、4 个 `missingPackagingConfigPackages` 和 `blockingPackagingConfigExcludes=[\"!node_modules/@codeinsights/**\"]`，并新增 `packaging_config_not_verified` blocker；`6ae1c896` 已新增 optional package 发布状态预检，显式 publication `--check-registry` 当前 no-go 为 4 个 `missingPublishedOptionalPackages`；`dedbfed1` 已新增 `packagedBundledBinarySmokePlan`，当前默认 `packaged-manifest` / `packaged-app-layout` 都为 `status=\"blocked\"`；`9e3e3dff` 已新增 optional package publish-target dry-run，显式 `--check-registry --native-search-package-version 0.0.3` 显示 4 个目标版本当前可发布、无 registry collision，但仍因 source `BINARY_VERSION=0.0.3-dev` 被 `native_search_binary_version_not_release_ready` 阻断。顶层 `realPackagedBinaryVerified` / `bundledBinaryVerified` 必须同时受 optional package publication、optional install-chain、packaging config、packaged evidence、packaged identity 和真实 binary verification 约束，不能由临时 fixture、单项 resolver、publication-only、packaging-only、install-chain-only、ready 状态、publish-target dry-run 或宽泛 builder include 置 true。publication gate 的 `missingPublishedOptionalPackages` 表示真实 package 尚未发布；publish-target dry-run 的 registry 404 只表示目标版本可用。当前 packaging config allowlist 预检仍明确 builder files 缺少 per-package include 并被 `!node_modules/@codeinsights/**` 阻断，因此不能在不发布真实包 / 不获准调整 builder allowlist 的前提下安全声明并安装真实 optionalDependencies。真实 optional package 发布 / optionalDependencies 实际声明与安装执行、真实 packaged app bundled binary smoke、builder allowlist 实际修改和最终 default-enable 风险决策仍未完成，native 仍不能默认启用。尚未完成：default enable、真实 optional package / optionalDependencies、真实 packaged app bundled binary smoke、builder allowlist 实际修改、最终 default-enable 风险决策、Phase 6-9。

启动后先运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`、`a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口`、`33d0990d docs(rust-go): 同步 Phase 5 packaged smoke 执行计划后续状态`、`dedbfed1 feat(rust-go): 补齐 Phase 5 packaged smoke 执行计划预检`、`d2009562 docs(rust-go): 同步 Phase 5 optional package 发布预检后续状态`、`6ae1c896 feat(rust-go): 补齐 Phase 5 optional package 发布状态预检`、`60bdd3a3 docs(rust-go): 回填 Phase 5 packaging config 最新恢复入口`、`f359dadc docs(rust-go): 同步 Phase 5 packaging config allowlist 后续状态`、`348e003d feat(rust-go): 补齐 Phase 5 packaging config allowlist 预检`、`e4d38aff docs(rust-go): 同步 Phase 5 default-enable readiness 后续状态`、`2bf88a5a feat(rust-go): 补齐 Phase 5 default-enable readiness 预检`、`97db0052 docs(rust-go): 同步 Phase 5 Agent nested parity 后续状态`、`665c5db7 feat(rust-go): 补齐 Phase 5 Agent nested native parity`、`c7726a56 docs(rust-go): 回填 Phase 5 Agent facade 最新恢复入口`、`a6f79e14 docs(rust-go): 同步 Phase 5 Agent facade benchmark 后续状态`、`9a06908a feat(rust-go): 补齐 Phase 5 Agent facade benchmark 分析`、`468e5395 docs(rust-go): 同步 Phase 5 native benchmark gate 后续状态`、`f86553ee feat(rust-go): 补齐 Phase 5 native benchmark gate 汇总`、`7891cfc6 docs(rust-go): 回填 Phase 5 optional package 安装链路恢复入口`、`f32d409f docs(rust-go): 同步 Phase 5 optional package 安装链路后续状态`、`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检`、`eb715c46 docs(rust-go): 回填 Phase 5 optionalDependencies 最新恢复入口`、`05df1687 docs(rust-go): 同步 Phase 5 optionalDependencies 预检后续状态`、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate` 和 `16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`。下一步继续 Phase 5“Rust search sidecar 试点”：在 default off 前提下优先推进真实 optional package 发布、真实 `optionalDependencies` 声明与安装执行、builder allowlist 实际修改前置工作、真实 packaged app bundled binary smoke 设计，或先处理 release-ready version / publish-target no-go 的安全前置；仅在真实 optional / packaged gate 可执行通过后再做最终 default-enable 风险决策。真实 packaged smoke、install-chain、publication 与 builder allowlist gate 未完成前不得默认启用 native。不要创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。
