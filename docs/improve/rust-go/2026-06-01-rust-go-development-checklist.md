...
> 最新已确认状态同步 / 恢复入口：`4cf20557 docs(rust-go): 回填 26221722 Phase 5 恢复入口`；若本轮状态回填再次提交后，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
> 当前结论：Rust / Go 优化重构 Phase 0、Phase 1、Phase 2、Phase 3 和 Phase 4 已完成；已建立 shared NativeRuntime DTO / fixtures / benchmark，在 TypeScript fallback 内收敛 Chat / Agent / Pipeline 搜索 facade，完成 Pipeline records cursor tail 与 SearchDialog Pipeline 内容搜索接入，完成 TypeScript workspace 文件索引 cache、watcher invalidation、SearchDialog Workspace 文件分组和安全路径白名单，并补齐 Native Runtime diagnostics 的 IPC / preload / Jotai / Settings UI / SearchDialog 状态可见体验。Phase 5 已完成前置依赖 decision record、sidecar protocol / fallback / packaged smoke 计划、100MB TS fallback benchmark gate、`native/search/` 最小 Rust search-only sidecar 源码切片、Electron main process sidecar manager、Chat search opt-in fallback 边界、Rust vs TS contract parity、100MB native benchmark 对比、search `limit + 1` 早停性能优化、benchmark event-loop 口径增强、native benchmark gate 机器可读汇总、Agent production facade benchmark 分析、Agent nested content native parity、基础 `smoke:native-runtime` 脚本、native-cache manifest schema helper、`protocol-mismatch` / `crash` / `timeout` / `cache-corruption` fake sidecar / isolated cache smoke、optional package manifest schema helper / `packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke 入口、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` 真实 packaged app identity gate、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、install-chain installed manifest metadata 加固、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、native search release-ready source version 前置、optional package source preflight、optional / packaged gate 执行顺序预检、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change plan 证据拆分、packaged bundled binary smoke invocation plan、optional package publication invocation plan、publication invocation 发布证据收紧、release handoff 证据计划、release handoff fail-closed 证据收紧、builder allowlist 实际修改前置证据、release handoff 批准包、release handoff 全 gate 批准队列、release handoff gate 执行证据清单、default-enable gate 网络副作用标记修正、release handoff gate 执行后状态迁移期望、release handoff gate 迁移校验器、release handoff 执行输入证据包、release handoff no-go 边界审计、no-go audit summary 外显和 install-chain installed manifest metadata 加固；尚未实现 packaged native binary、真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策或 Go supervisor。
> 当前补充：`99d76b0c` 已在 `nativeSearchReleaseHandoffPlan` 中新增 `currentGateApprovalPacket`，机器可读记录当前 `optionalPackageExecutionPlan.nextStage` 对应的批准准备包：blocked 时 `approvalRequired=false` 且 `candidateCommands=[]`；publication gate ready 时只要求 `release_approval` 并只授权候选 publish 调用审核，不授权 `package.json` / `bun.lock` / builder / packaged app / default enable。`197569d5` 已新增 `nativeSearchReleaseHandoffPlan.gateApprovalQueue`，按真实 gate 顺序输出 publication、optionalDependencies declaration、install-chain、builder allowlist、packaged smoke 和 default-enable risk review 的批准矩阵：只有当前且 ready 的 gate 暴露候选命令，后续 gate 必须 `blocked_until_prior_gate_verified` 且 `candidateCommands=[]`，已完成 gate 只能标为 `verified`。`cb30b819` 已新增 `nativeSearchReleaseHandoffPlan.gateExecutionEvidenceChecklist`，从 approval queue 只读派生每个 gate 批准后必须产出的证据、文件 / 网络 / 远端写副作用标记、fail-closed criteria 和 does-not-verify 边界；只有当前 ready gate 暴露批准后验证命令，blocked future gate 继续 `postExecutionVerificationCommands=[]`。`72a00b01` 已补齐 default-enable risk review gate 的 `networkRequired=true`，因为其批准后验证命令包含显式 `--check-registry`；blocked future gate 仍不得暴露执行命令。`b874628b` 已为该 execution checklist 增加执行后状态迁移期望：每个 gate 明确 expected next gate、应变 true 的 summary flags、仍必须保持未验证的后续 gate / 子证据字段和 transition fail-closed criteria；这些字段只用于后续真实 gate 执行后的验收，不驱动 `nextGate`、`verified`、candidate command 或 default-enable。`66da7eba` 已新增 `evaluateNativeSearchReleaseHandoffGateTransition()`，把这些 transition expectation 转成 before / after summary 的纯函数校验，并补齐 publication、declaration、install-chain、packaging config 和 packaged smoke 的合规 / fail-closed 用例；它不查询 registry、不发布、不安装、不写 package manifest / lockfile / builder、不创建 packaged app、不读取 binary，也不把 default-enable risk review 标为当前可通过。`461295b0` 已新增 `currentGateExecutionInputPacket`，把当前 gate 批准后的执行输入、命令可见性、副作用、执行后验收和禁止动作收敛为单个只读 packet；blocked / future gate 不暴露命令，install-chain 只有当前 gate ready 且批准后才暴露 `bun install --frozen-lockfile`，packaged smoke 只暴露 `<packaged-app-root>` 占位命令，default-enable 在当前 smoke 中仍 blocked 且不暴露默认启用命令。`8a0bcfd5` 已新增 `evaluateNativeSearchReleaseHandoffNoGoBoundary()`，只读审计 current approval / execution packet、gate approval queue、execution evidence checklist、default readiness、verified flags 和敏感字符串边界；它不执行命令、不联网、不写文件、不读取 binary path，也不证明真实 publication / install-chain / builder / packaged binary / default enable。`63e0fc2b` 已将 no-go audit 外显到 `nativeSearchReleaseHandoffNoGoBoundaryAudit` summary 字段和 `release-handoff-no-go-boundary-audit` smoke case，并要求 installed package manifest 满足 optional package source 的严格 package.json shape；它仍不发布、不安装、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。`ddc818ae` 的 `candidateYamlEditPlan` 仍只提供 builder 实际修改前 review 输入；`3f5a0783` 的 release handoff fail-closed 绑定仍成立，后置 evidence 不能跳过上游 gate。
...
> 本轮新增：`63e0fc2b` 将 release handoff no-go audit 外显到 `nativeSearchReleaseHandoffNoGoBoundaryAudit` summary 字段和 `release-handoff-no-go-boundary-audit` smoke case，audit 失败会让 CLI exit code fail closed；同时把 install-chain 的 installed package manifest 校验加固为 optional package source package.json 严格 shape，拒绝只有 `name/version`、lifecycle scripts 和 runtime dependencies 的 installed package evidence。该新增仍只做只读 evidence / metadata gate，不发布、不安装、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default-enable。`8a0bcfd5` 的 no-go boundary audit、`461295b0` 的 execution input packet、`66da7eba` 的 transition verifier、`197569d5` 的 gate approval queue、`99d76b0c` 的 current gate approval packet、`ddc818ae` 的 builder YAML edit intent、`3f5a0783` 的 release handoff fail-closed 绑定、`cb30b819` 的 execution evidence checklist、`72a00b01` 的 network side-effect 口径和 `b874628b` 的 transition expectation 仍成立。
> 最新状态回填：`4cf20557` 已把 `26221722` 后的最新状态同步 / 恢复入口回填到 Rust / Go 状态文档；`26221722` 已把 `c95c5cf6` 后的最新状态同步 / 恢复入口回填到 Rust / Go 状态文档；`c95c5cf6` 已把 `5bc9ebe8` 后的最新完成 / 未完成边界、3 个核心 gate 和 no-go 验证要求同步到 Rust / Go 状态文档；`5bc9ebe8` 已把 `bcc65b3d` 后的最新恢复入口回填到状态文档；`bcc65b3d` 已把 `e2b620ba` 后的恢复入口回填到状态文档；`e2b620ba` 已把 `baa0ae3d` 后的恢复入口回填到状态文档；`baa0ae3d` 已同步 `63e0fc2b` 后的 3 个核心 gate 计划和最小 optional package gate 发布前只读 readiness。当前 `native/search` Cargo version 与 `BINARY_VERSION` 均为 `0.0.3`；`optional-package-source --native-search-package-version 0.0.3` 和显式 `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 已通过，4 个 planned package 目标版本无 registry collision；`packaged-manifest --check-registry` 仍预期 exit 1，说明 4 个 planned package 尚未真实发布。该状态同步只记录状态，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary，也不改变 default-enable gate。
> 2026-06-07 当前范围裁剪：用户已确认 Phase 6、Phase 7 和 Phase 9 可以不做，后续默认不再进入这些阶段；Phase 8 不再作为全量打包 / CI / 发布收口阶段推进，只把 Phase 5 真实 packaged app bundled binary smoke 所需的最小 packaged 验证设计保留在 Phase 5 收口内。Phase 5 verified 后停止，除非用户明确重新打开后续阶段。
> 2026-06-07 后续计划缩减：从现在开始只保留 3 个核心 gate：最小真实 optional package gate、最小 install-chain gate、最小 packaged smoke gate。default-enable 继续暂缓；即使最小 packaged opt-in 通过，也只记录“packaged opt-in 可用”，不默认启用 native。不再新增 helper-only approval / handoff / dry-run 代码，除非真实 gate 执行暴露明确缺口。
> 2026-06-07 最小 optional package gate 发布前只读 readiness：本轮已确认 `native/search` Cargo version 与 `BINARY_VERSION` 均为 `0.0.3`；`optional-package-source --native-search-package-version 0.0.3` 通过，显式 `optional-package-publish-target --native-search-package-version 0.0.3 --check-registry` 通过，4 个 planned package 的目标版本当前无 registry collision；`packaged-manifest --check-registry` 预期 exit 1，4 个 planned package 仍全部缺失，说明真实 publication 尚未执行。下一步必须等待用户明确批准真实 `npm publish`，并在执行后重新跑 registry evidence。
...
- [~] Phase 5：Rust search sidecar 试点（前置依赖决策、protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar 源码切片、Electron main process sidecar manager、fallback gate、contract parity、native benchmark 对比、search 早停性能优化、2 轮稳定 benchmark 复跑、benchmark event-loop 口径增强、新字段稳定 benchmark 复跑、native benchmark gate 机器可读汇总、Agent production facade benchmark 分析、Agent nested content native parity、基础 smoke script、native-cache schema helper、`protocol-mismatch` / `crash` / `timeout` / `cache-corruption` fake sidecar / isolated cache smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke 入口、packaged app evidence classifier、`packagedAppIdentityVerified` gate、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、install-chain installed manifest metadata 加固、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、release-ready source version 前置、optional package source preflight、optional / packaged gate 执行顺序预检、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change plan 证据拆分、packaged bundled binary smoke invocation plan、optional package publication invocation plan、publication invocation 发布证据收紧、release handoff 证据计划、release handoff fail-closed 证据收紧、builder allowlist 实际修改前置证据、release handoff 批准包、release handoff 全 gate 批准队列、release handoff gate 执行证据清单、default-enable gate 网络副作用标记修正、release handoff gate 执行后状态迁移期望、release handoff gate 迁移校验器、release handoff 执行输入证据包、release handoff no-go 边界审计、no-go audit summary / smoke case 外显和 install-chain installed manifest metadata 加固已完成；真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke、最终 default-enable 风险决策未完成，default enable 与真实 packaged smoke 尚未完成）。
- [x] Phase 5 release handoff fail-closed 证据收紧：`3f5a0783` 已完成 `nativeSearchReleaseHandoffPlan.gateBinding` 与 `packagedBundledBinarySmokeInvocationPlan.executionDesign`，继续保持不发布、不安装、不修改 builder、不创建 packaged app、不证明 bundled binary。
- [x] Phase 5 builder allowlist 实际修改前置证据：`ddc818ae` 已完成 `packagingConfigAllowlistChangePlan.candidateYamlEditPlan`，明确后续真实 builder 修改必须先移除阻断规则、添加 4 个精确 per-package include、拒绝宽泛 include，并在批准后重新跑 packaging config preflight；本轮仍不修改 `apps/electron/electron-builder.yml`。
- [x] Phase 5 builder allowlist 修改意图状态同步：`f02ceae0` 和 `3089b768` 已回填 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 lessons；这些提交只同步状态，不改变真实 publication / install-chain / builder / packaged smoke gate。
- [x] Phase 5 release handoff 批准包：`99d76b0c` 已完成 `nativeSearchReleaseHandoffPlan.currentGateApprovalPacket`，明确当前顺序 gate 的批准项、候选命令、授权边界和禁止动作；blocked gate 不输出可执行候选命令，publication ready 也只表示 release approval 后可审核 publish 调用，不证明 package 已发布。
- [x] Phase 5 release handoff 批准包状态同步：`bd0e61d3` 已回填最新完成 / 未完成状态、恢复入口和下次启动提示词；该提交只更新文档，不改变 native default off / 显式 opt-in 边界。
- [x] Phase 5 release handoff 全 gate 批准队列：`197569d5` 已完成 `nativeSearchReleaseHandoffPlan.gateApprovalQueue`，明确后续真实 gate 的顺序批准矩阵；只有当前 ready gate 可暴露候选命令，未到达 gate 不授权执行，已完成 gate 不授权重复执行。该队列只做批准准备，不证明真实 publication / install-chain / builder / packaged binary / default enable。
- [x] Phase 5 release handoff 批准队列状态同步：`9d8520fa` 已回填最新完成 / 未完成状态、恢复入口和下次启动提示词；该提交只更新文档，不改变真实 optional package 发布 / install-chain / builder / packaged smoke gate。
- [x] Phase 5 release handoff gate 执行证据清单：`cb30b819` 已完成 `nativeSearchReleaseHandoffPlan.gateExecutionEvidenceChecklist`，明确每个真实 gate 批准执行后必须补齐的 evidence、当前 gate 批准后验证命令、未来 gate 不暴露命令、以及远端写 / 文件写 / 网络副作用边界。`72a00b01` 已补齐 default-enable risk review gate 的 future network side-effect 标记。该清单只做执行证据准备，不证明真实 publication / install-chain / builder / packaged binary / default enable。
- [x] Phase 5 release handoff gate 执行后状态迁移期望：`b874628b` 已完成 `gateExecutionEvidenceChecklist` 的 transition expectation，明确每个 gate 执行并通过验证后应到达的 next gate、应变 true 的 summary flags、仍必须保持 false 的后续 gate / 子证据字段和 transition fail-closed criteria。该期望只用于后续真实 gate 执行后的验收，不证明真实 publication / optionalDependencies / install-chain / builder / packaged binary / default enable。
- [x] Phase 5 gate 状态迁移期望状态同步：`da6d79dc` 已回填 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md` 和 `tasks/todo.md` Review，并把 transition expectation 边界、未完成真实 optional / packaged gates 和下次启动提示词同步到文档。
- [x] Phase 5 release handoff gate 迁移校验器：`66da7eba` 已完成 `evaluateNativeSearchReleaseHandoffGateTransition()`，用 before / after summary 校验真实 gate 执行后的 expected next gate、expected summary flags、must-remain-unverified flags 和 no-go boundary violations；同时修正 declaration / install-chain / packaging config 中间 gate 在前置 readiness 满足时可进入 `ready_for_approval`。该校验器只用于验收，不发布、不安装、不写 builder、不创建 packaged app，也不证明 default-enable。
- [x] Phase 5 gate 迁移校验器状态同步：`40366181` 已回填 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`，明确 `66da7eba` 为最新实现基线、`40366181` 为最新已确认恢复入口，并继续标注真实 optional package 发布、optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke 和最终 default-enable 风险决策未完成。
- [x] Phase 5 release handoff 执行输入证据包：`461295b0` 已完成 `nativeSearchReleaseHandoffPlan.currentGateExecutionInputPacket`，把当前 gate 批准后的执行输入、命令可见性、副作用、执行后验收和禁止动作机器可读化。该 packet 只做批准后执行输入，不执行发布 / 安装 / builder 修改 / packaged smoke，不证明真实 publication、install-chain、builder verified、packaged binary verified 或 default-enable。
- [x] Phase 5 release handoff no-go 边界审计：`8a0bcfd5` 已完成 `evaluateNativeSearchReleaseHandoffNoGoBoundary()`，只读校验 release handoff summary 的 default off / explicit opt-in、candidate / post-execution command、side-effect、premature verified flag 和敏感字符串边界。该 audit 不执行发布 / 安装 / builder 修改 / packaged smoke，不联网、不写文件、不读取 binary path，也不证明真实 publication、install-chain、builder verified、packaged binary verified 或 default-enable。
- [x] Phase 5 release handoff no-go 边界审计状态同步：`345dcf01` 已回填 `8a0bcfd5` 后的 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；该提交只更新文档，不改变真实 optional / packaged gate。
- [x] Phase 5 no-go audit summary 与 install-chain metadata 加固：`63e0fc2b` 已完成 `nativeSearchReleaseHandoffNoGoBoundaryAudit` summary 字段和 `release-handoff-no-go-boundary-audit` smoke case 外显，并把 installed package manifest 校验收紧为 optional package source package.json 严格 shape。该提交不发布、不安装、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary，也不证明 default-enable。
- [x] Phase 5 no-go audit summary 与 install-chain metadata 状态同步：本轮回填 `63e0fc2b` 后的 development checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；本轮前最新已确认 docs 恢复入口为 `345dcf01`，提交后以下次 `git log -5 --oneline` 最新 Rust / Go docs 提交为实际恢复入口。
- [x] Phase 5 后续计划缩减状态同步：本轮将后续工作压缩为最小真实 optional package gate、最小 install-chain gate 和最小 packaged smoke gate；default-enable、Go supervisor、Phase 6 / 7 / 9、全平台 CI / 发布矩阵和 helper-only 新增工作均暂缓。该同步只更新文档，不执行发布 / 安装 / builder 修改 / packaged smoke。
- [x] Phase 5 最小 optional package gate 发布前只读 readiness：source preflight 与 publish-target registry dry-run 已通过；publication evidence check 仍显示 4 个 planned package 全部未发布，真实 `npm publish` 尚未执行。
- [x] Phase 5 optional package readiness 状态同步：`baa0ae3d docs(rust-go): 同步 Phase 5 optional package readiness` 已把 readiness 结果、no-go 边界和下一步真实 publication 批准入口同步到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 optional readiness 恢复入口回填：`e2b620ba docs(rust-go): 回填 Phase 5 optional readiness 恢复入口` 已把 `baa0ae3d` 后的最新状态同步 / 恢复入口回填到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 e2b620ba 恢复入口回填：`bcc65b3d docs(rust-go): 回填 e2b620ba Phase 5 恢复入口` 已把 `e2b620ba` 后的最新状态同步 / 恢复入口回填到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 bcc65b3d 恢复入口回填：`5bc9ebe8 docs(rust-go): 回填 bcc65b3d Phase 5 恢复入口` 已把 `bcc65b3d` 后的最新状态同步 / 恢复入口回填到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 5bc9ebe8 最新状态同步：`c95c5cf6 docs(rust-go): 同步 5bc9ebe8 Phase 5 最新状态` 已把 `5bc9ebe8` 后的最新完成 / 未完成边界、3 个核心 gate、只读 readiness/no-go 结果和下次启动提示词同步到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 c95c5cf6 恢复入口回填：`26221722 docs(rust-go): 回填 c95c5cf6 Phase 5 恢复入口` 已把 `c95c5cf6` 后的最新状态同步 / 恢复入口回填到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 26221722 恢复入口回填：`4cf20557 docs(rust-go): 回填 26221722 Phase 5 恢复入口` 已把 `26221722` 后的最新状态同步 / 恢复入口回填到本 checklist、sidecar protocol / smoke plan、`next-session-prompt.md`、`tasks/todo.md` Review 和 `tasks/lessons.md`；它只更新文档，不执行 `npm publish`、不运行 install、不声明真实 optionalDependencies、不修改 builder、不创建 packaged app / binary、不证明 default enable。
- [x] Phase 5 后续范围收窄状态同步：`4518a2ce` 已把当前目标收敛为 Phase 5 verified / release handoff 收口；Phase 6、Phase 7、Phase 9 暂停；Phase 8 全量打包 / CI / 发布收口不再作为当前独立阶段推进。
- [skip] Phase 6：大文件 / 日志 chunk preview 已按当前范围暂停；风险接受人：用户，2026-06-07。
- [skip] Phase 7：PathSafety 与 GitOutputParser 抽象已按当前范围暂停；风险接受人：用户，2026-06-07。
- [min] Phase 8：全量打包、CI、版本与发布收口已最小化；仅保留 Phase 5 packaged smoke 所需的最小 packaged 验证设计。
- [skip] Phase 9：Go supervisor 可选 spike 已按当前范围暂停；风险接受人：用户，2026-06-07。

### 当前未完成的关键能力

- [ ] 最小真实 optional package gate：发布前 source / publish-target readiness 已通过；真实 `npm publish` 尚未执行，必须等待用户明确批准，发布后再用 registry exact-version evidence 证明 package published。
- [ ] 最小 install-chain gate：只在前一 gate 真实通过后声明对应 optionalDependency、执行真实 install，并验证 `bun.lock`、installed package manifest metadata 和 native default off 不变。
- [ ] 最小 packaged smoke gate：只在前两项真实通过后做精确 builder allowlist 修改和一个真实 packaged app bundled binary smoke，证明 app 内 `node_modules` 的 native binary 可解析、可执行且不走系统 `PATH`。
- [defer] default-enable 风险决策继续暂缓；即使最小 packaged opt-in 通过，也只记录“packaged opt-in 可用”，不默认启用 native。
- [skip] Go supervisor、Phase 6 / 7 / 9、全平台 CI / 发布矩阵和 helper-only approval / handoff / dry-run 新增工作均不进入当前后续计划。
...
- [x] Phase 5 optional package install-chain preflight gate 已完成：`1114233b feat(rust-go): 补齐 Phase 5 optional package 安装链路预检` 新增 `validateNativeSearchOptionalPackageInstallChain()`，只读校验 native search optionalDependencies 声明、`bun.lock` resolved package entry 和已安装 package manifest 一致性；`63e0fc2b` 已将 installed package manifest 校验收紧为 optional package source package.json 的完整 metadata shape，要求 exact version、平台 `os` / `cpu`、`bin`、精确 `files` 和 `publishConfig.access=public` 一致，并拒绝 lifecycle scripts 与 runtime dependency 字段。当前仓库仍未声明真实 native search optionalDependencies，因此 summary 保持 `optionalDependenciesInstallChainVerified=false`、`optionalDependenciesLockfileVerified=false`、`optionalDependenciesInstalledPackagesVerified=false`、`realPackagedBinaryVerified=false`。若后续出现声明但 lockfile / installed package manifest 不完整或 metadata 不合规，install-chain preflight 必须 failed，不能 skipped。
...
- [x] Phase 5 builder allowlist 实际修改前置证据已完成：`ddc818ae feat(rust-go): 补齐 Phase 5 builder allowlist 修改意图` 在 `packagingConfigAllowlistChangePlan` 中新增 `candidateYamlEditPlan`，只读输出后续真实 YAML edit intent：`targetFile="apps/electron/electron-builder.yml"`、`modifiesFile=false`、操作顺序、`addIncludes`、`keepIncludes`、`removeRules`、`forbiddenIncludes` 和 `acceptanceEvidence`。当前 plan 仍要求添加 4 个精确 native search package include 并移除 `!node_modules/@codeinsights/**`；该字段不写 builder、不替代批准、不证明 `packagingConfigVerified`、不证明 packaged binary。
...
- [ ] 真实 Rust optional package 发布 / optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、真实 packaged app bundled binary smoke 和 default-enable 判断尚未实现；当前显式 publication registry smoke 仍显示 4 个计划 `@codeinsights/native-search-*` 包均在 `missingPublishedOptionalPackages`，publish-target dry-run 仅证明目标版本当前可尝试发布，source preflight 仅证明 package source blueprint 形状 ready，execution plan 仅证明下一步顺序，builder allowlist dry-run plan 和 `candidateYamlEditPlan` 仅证明人工 review / YAML edit intent 输入已生成，optionalDependencies install-chain dry-run plan 仅证明声明 / 安装执行前 review 输入已生成，publication change plan 仅证明真实发布前 review 输入已生成，optional package publication invocation plan 仅证明真实 `npm publish` 调用形状和验收证据已机器可读化，packaged smoke invocation plan 仅证明真实 packaged app root 调用形状和验收证据已机器可读化，release handoff plan / approval packet / approval queue / gate execution evidence checklist 仅证明当前顺序 gate 可交接人工审核 / 执行及执行后证据清单，且当前 publication、optionalDependencies declaration / install-chain、lockfile、installed package 和 packaging config allowlist 均未真实通过，因此不能把这些 dry-run / invocation / handoff / edit intent / checklist 外推为 published / installed / packaged verified 或 default enable。
- [skip] Phase 6、Phase 7 和 Phase 9 不进入当前目标；Phase 8 全量 scope 不推进，仅保留 Phase 5 packaged smoke 的最小 packaged 验证设计。
- [skip] Go supervisor 不进入当前主线；除非用户明确重新打开 Phase 9，否则不再检查 Phase 9 触发条件。
...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、native benchmark gate 汇总、Agent production facade benchmark 分析、Agent nested native parity、fake sidecar / cache-corruption smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` gate、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、install-chain installed manifest metadata 加固、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、release-ready source version 前置、optional package source preflight、optional / packaged gate 执行顺序预检、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change plan 证据拆分、packaged bundled binary smoke invocation plan、optional package publication invocation plan、publication invocation 发布证据收紧、release handoff 证据计划、release handoff fail-closed 证据收紧、builder allowlist 实际修改前置证据、release handoff 批准包、release handoff 全 gate 批准队列、release handoff gate 执行证据清单、default-enable gate 网络副作用标记、release handoff gate 执行后状态迁移期望、release handoff gate 迁移校验器、release handoff 执行输入证据包、release handoff no-go 边界审计、no-go audit summary 外显和 install-chain installed manifest metadata 加固；native 未达默认启用门槛，packaged native binary / 真实 optional package 发布 / optionalDependencies 实际声明与安装执行 / 真实 packaged app bundled binary smoke / builder allowlist 实际修改 / 最终 default-enable 风险决策尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `4cf20557 docs(rust-go): 回填 26221722 Phase 5 恢复入口`、`26221722 docs(rust-go): 回填 c95c5cf6 Phase 5 恢复入口`、`c95c5cf6 docs(rust-go): 同步 5bc9ebe8 Phase 5 最新状态`、`5bc9ebe8 docs(rust-go): 回填 bcc65b3d Phase 5 恢复入口`、`bcc65b3d docs(rust-go): 回填 e2b620ba Phase 5 恢复入口`、`e2b620ba docs(rust-go): 回填 Phase 5 optional readiness 恢复入口`、`baa0ae3d docs(rust-go): 同步 Phase 5 optional package readiness`、`7c8057e3 docs(rust-go): 收缩 Phase 5 后续核心 gate`、`885e89b5 docs(rust-go): 同步 Phase 5 no-go audit summary 状态`、`63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest`、`345dcf01 docs(rust-go): 同步 Phase 5 no-go 边界审计状态`、`8a0bcfd5 feat(rust-go): 补齐 Phase 5 release handoff no-go 边界审计` 和 `7898dae6 docs(rust-go): 同步 Phase 5 执行输入证据包状态`。
...
5. 下一步不要创建 packaged native binary 或修改打包配置；当前目标只收口 Phase 5 verified / release handoff。不要进入 Phase 6、Phase 7 或 Phase 9；Phase 8 只保留 Phase 5 packaged smoke 所需的最小 packaged 验证设计，不做全量打包 / CI / 发布阶段。在 default off 前提下优先推进真实 optional package 发布、真实 optionalDependencies 声明与安装执行、builder allowlist 实际修改前置工作和真实 packaged app bundled binary smoke 设计，或仅在真实 optional / packaged gate 可执行通过后再做最终 default-enable 风险决策。`optionalPackagePublicationChangePlan`、`optionalDependenciesInstallChainChangePlan` 和 `packagingConfigAllowlistChangePlan` 都只表示实际修改前 review 输入；`publishedPackages` 只表示真实 registry publication 证据，collision / invalid metadata 不能当作已发布。只有 P95 / event loop gate、Agent facade parity、真实 optional install-chain、builder allowlist 和真实 packaged smoke 同时进入可执行通过阶段后，才允许默认启用 native。
...
| M6 | Phase 6 | 大文件 / 日志 chunk preview | [skip] | 当前不做 |
| M7 | Phase 7 | PathSafety 与 GitOutputParser 抽象，可选 Rust helper | [skip] | 当前不做 |
| M8 | Phase 8 | native 打包、CI、发布和回滚收口 | [min] | 仅保留 Phase 5 packaged smoke 最小验证 |
| M9 | Phase 9 | Go supervisor 可选 spike | [skip] | 当前不做 |
...
- 未完成项 / [skip]：Electron 交互 smoke 与 packaged smoke 未在本阶段执行，原计划归入 Phase 8；2026-06-07 范围裁剪后，全量 Phase 8 暂停，只把 Phase 5 packaged smoke 所需的最小 packaged 验证保留在 Phase 5 收口内。Rust sidecar、Go supervisor、native binary、native optional package、大文件 / 日志 chunk preview 当时均未实现。
...
- [x] 阶段开始
- [x] 依赖评估完成
- [~] 测试先行完成（Rust search-only sidecar、sidecar manager、contract parity、benchmark、native benchmark gate 汇总、Agent production facade benchmark、Agent nested native parity、smoke script、native-cache schema、optional package manifest 预检、bundled package resolver fixture、packaged app layout、packaged app evidence classifier、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、install-chain installed manifest metadata 加固、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、release-ready source version 前置、optional package source preflight、optional / packaged gate 执行顺序预检、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change plan 证据拆分、packaged smoke invocation plan、optional package publication invocation plan、publication invocation 发布证据收紧、release handoff 证据计划、release handoff fail-closed 证据收紧、builder allowlist 实际修改前置证据、release handoff 批准包、release handoff 全 gate 批准队列、gate 执行证据清单、default-enable network side-effect 回归测试和 gate 执行后状态迁移期望测试已完成；真实 packaged app bundled binary smoke 测试尚未完成）
- [~] 实现完成（Rust search-only sidecar、Electron main process sidecar manager、Chat opt-in native search、benchmark 增强、native benchmark gate 汇总、Agent production facade benchmark 分析、Agent nested native parity、基础 smoke script、native-cache schema helper、optional package manifest 预检、bundled package resolver fixture、packaged app layout preflight、packaged app evidence classifier、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、install-chain installed manifest metadata 加固、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、release-ready source version 前置、optional package source preflight、optional / packaged gate 执行顺序预检、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change plan 证据拆分、packaged smoke invocation plan、optional package publication invocation plan、publication invocation 发布证据收紧、release handoff 证据计划、release handoff fail-closed 证据收紧、builder allowlist 实际修改前置证据、release handoff 批准包、release handoff 全 gate 批准队列、gate 执行证据清单、default-enable network side-effect 标记和 gate 执行后状态迁移期望已完成；真实 optional package 发布 / optionalDependencies 实际声明与安装执行、builder allowlist 实际修改、packaged binary 和 default enable 尚未完成）
- [~] 验证完成（Rust / native runtime tests / typecheck / build:main / benchmark / native benchmark gate 汇总 / Agent production facade benchmark / Agent nested native parity / packaged-manifest preflight / resolver fixture smoke / packaged app layout smoke / optionalDependencies declaration preflight / optional package install-chain preflight / default-enable readiness 预检 / packaging config allowlist 预检 / optional package 发布状态预检 / packaged bundled binary smoke 执行计划预检 / optional package publish-target dry-run 预检 / release-ready source version 前置 / optional package source preflight / optional packaged execution plan / builder allowlist dry-run plan / optionalDependencies install-chain dry-run plan / optional package publication change plan 证据拆分 / packaged smoke invocation plan / optional package publication invocation plan / publication invocation 发布证据收紧 / release handoff 证据计划 / release handoff fail-closed 证据收紧 / builder allowlist 实际修改前置证据 / release handoff gate 执行证据清单 / gate 执行后状态迁移期望已通过；真实 packaged app bundled binary smoke 尚未完成）
- [~] 阶段 Review 完成（已记录前置、search-only、sidecar manager、性能优化、benchmark gate、native benchmark gate 汇总、Agent production facade benchmark、Agent nested native parity、smoke/cache schema、fake sidecar smoke、optional package manifest 预检、bundled package resolver fixture、packaged app layout preflight、packaged app evidence classifier、optionalDependencies declaration preflight、optional package install-chain preflight、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、release-ready source version 前置、optional package source preflight、optional packaged execution plan、builder allowlist dry-run plan、optionalDependencies install-chain dry-run plan、optional package publication change plan 证据拆分、packaged smoke invocation plan、optional package publication invocation plan、publication invocation 发布证据收紧、release handoff 证据计划、release handoff fail-closed 证据收紧、builder allowlist 实际修改前置证据、release handoff 批准包、release handoff 全 gate 批准队列、gate 执行证据清单和 gate 执行后状态迁移期望 Review；完整 Phase 5 Review 尚未完成）
- [x] release handoff fail-closed 证据收紧测试 / 实现 / 验证 / Review 已完成：`3f5a0783` 已覆盖 `gateBinding` 与 `executionDesign`，真实 optional package 发布、install-chain、builder allowlist 修改和 packaged app bundled binary smoke 仍未完成。
- [x] builder allowlist 实际修改前置证据测试 / 实现 / 验证 / Review 已完成：`ddc818ae` 已覆盖 `candidateYamlEditPlan`，真实 `apps/electron/electron-builder.yml` 修改和 packaged app bundled binary smoke 仍未完成。
- [x] release handoff gate 执行证据清单测试 / 实现 / 验证 / Review 已完成：`cb30b819` 已覆盖 `gateExecutionEvidenceChecklist`，`72a00b01` 已补齐 default-enable risk review 的 `networkRequired` 边界，真实 optional package 发布、install-chain、builder allowlist 修改和 packaged app bundled binary smoke 仍未完成。
- [x] release handoff gate 执行后状态迁移期望测试 / 实现 / 验证 / Review 已完成：`b874628b` 已覆盖 `expectedNextGateAfterVerification`、`expectedSummaryFlagsAfterExecution`、`mustRemainUnverifiedAfterExecution` 和 `transitionFailClosedCriteria`，真实 optional package 发布、install-chain、builder allowlist 修改和 packaged app bundled binary smoke 仍未完成。
- [x] release handoff gate 迁移校验器测试 / 实现 / 验证 / Review 已完成：`66da7eba` 已覆盖 `evaluateNativeSearchReleaseHandoffGateTransition()`，用 before / after summary 验收真实 gate 执行后的 expected next gate、expected summary flags、must-remain-unverified flags 和 no-go boundary violations；它不执行 gate、不发布、不安装、不修改 builder、不创建 packaged app、不证明 packaged verified 或 default-enable。
- [ ] 阶段提交完成
...
> 当前范围：已暂停。用户在 2026-06-07 接受跳过本阶段；Phase 5 verified 收口前不进入该阶段。
...
> 当前范围：已暂停。用户在 2026-06-07 接受跳过本阶段；路径安全和 Git 输出解析不在当前 Rust / Go 收口目标内扩展。
...
> 当前范围：全量 Phase 8 已最小化。当前不做完整打包 / CI / 发布收口，只把 Phase 5 真实 packaged app bundled binary smoke 所需的最小 packaged 验证设计留在 Phase 5 gate 中。
...
> 当前范围：已暂停。用户在 2026-06-07 接受跳过本阶段；除非用户明确重新打开，否则不再检查 Go supervisor 触发条件。
...
当前不可进入 Phase 9。若用户未来明确重新打开 Go supervisor spike，才重新评估以下历史触发条件：
...
```text
请继续 CodeInsights Rust / Go Phase 5。先读取 tasks/lessons.md、tasks/todo.md、docs/improve/rust-go/next-session-prompt.md、docs/improve/rust-go/2026-06-01-rust-go-development-checklist.md、docs/improve/rust-go/2026-06-03-phase-5-sidecar-protocol-and-smoke-plan.md 和 native/search/；启动后运行 git status --short --branch 与 git log -25 --oneline。

当前分支是 rust-go-refactor。最新状态同步 / 恢复入口是 4cf20557 docs(rust-go): 回填 26221722 Phase 5 恢复入口；本次文档状态回填若产生更新提交，则以下次 git log -5 --oneline 最新 Rust / Go docs 提交为实际恢复入口。最新实现基线是 63e0fc2b feat(rust-go): 外显 no-go audit 并加固 install-chain manifest。最近历史应包含 4cf20557、26221722、c95c5cf6、5bc9ebe8、bcc65b3d、e2b620ba、baa0ae3d、7c8057e3、885e89b5、63e0fc2b、345dcf01、8a0bcfd5、7898dae6。

当前真实进度：Phase 0-4 已完成。Phase 5 已完成到 no-go audit summary / smoke case 外显、install-chain installed manifest metadata 加固、后续计划缩减为 3 个核心 gate，以及最小 optional package gate 发布前只读 readiness：native/search Cargo version 与 BINARY_VERSION 均为 0.0.3；optional-package-source --native-search-package-version 0.0.3 通过；optional-package-publish-target --native-search-package-version 0.0.3 --check-registry 通过且 4 个 planned package 目标版本无 registry collision；packaged-manifest --check-registry 仍预期失败，说明真实 publication 尚未执行。

当前后续计划只保留 3 个核心 gate：1. 最小真实 optional package gate：必须先获得用户明确批准后才能执行真实 npm publish，并在发布后用 packaged-manifest --check-registry 取得 exact-version registry evidence。2. 最小 install-chain gate：真实 package evidence 通过后声明对应 optionalDependency，执行真实 install，验证 bun.lock、installed package manifest metadata 和 native default off 不变。3. 最小 packaged smoke gate：前两项通过后做精确 builder allowlist 修改，并验证一个真实 packaged app bundled binary 可从 app 内 node_modules 解析、可执行且不走系统 PATH。

default-enable 继续暂缓；即使最小 packaged opt-in 通过，也只记录 packaged opt-in 可用，不默认启用 native。不要进入 Phase 6、Phase 7 或 Phase 9；Phase 8 不做全量打包 / CI / 发布矩阵；Go supervisor 暂缓；不要新增 helper-only approval / handoff / dry-run 代码，除非真实 gate 执行暴露明确缺口。

当前 no-go：native 必须继续 default off / 显式 opt-in。默认 smoke 不联网。未获明确批准前，不执行真实 npm publish、不运行真实 install、不创建 packaged native binary、不修改 apps/electron/electron-builder.yml、不修改根 README.md / AGENTS.md、不新增真实 @codeinsights/native-search-* optionalDependencies、不 push、不创建 PR。每个阶段完成后自动更新 development checklist、next-session-prompt.md、tasks/todo.md Review 和必要 lessons，运行 no-go 验证并单独提交状态同步文档。
```

下面的长提示词保留为历史上下文；后续启动以本节上方提示词和 `docs/improve/rust-go/next-session-prompt.md` 为准。
...
`3f5a0783` 已新增 `nativeSearchReleaseHandoffPlan.gateBinding` 和 `packagedBundledBinarySmokeInvocationPlan.executionDesign`；这些字段只收紧 release handoff fail-closed 顺序和真实 packaged smoke 设计约束，不等于真实 publication、install-chain、builder allowlist、packaged binary 或 default-enable 已完成。
