...
> 更新时间：2026-06-05
> 最新开发基线：`a3f98673 feat(rust-go): 补齐 Phase 5 builder allowlist dry-run plan`。
...
> 本轮新增：`a3f98673` 新增 `packagingConfigAllowlistChangePlan`，只读输出 builder allowlist 实际修改前的人工 review 输入：4 个精确 required / missing include（`node_modules/@codeinsights/native-search-darwin-arm64/**/*`、`node_modules/@codeinsights/native-search-darwin-x64/**/*`、`node_modules/@codeinsights/native-search-win32-x64/**/*`、`node_modules/@codeinsights/native-search-linux-x64/**/*`）、当前 blocker `!node_modules/@codeinsights/**`、禁止使用的宽泛 include、候选验证命令和禁止动作。当前 plan 为 `status="blocked"`、`approvalRequired=true`；它不写 `apps/electron/electron-builder.yml`，不声明 optionalDependencies，不安装、不发布、不读取 binary，也不把 `packagingConfigVerified`、`bundledBinaryVerified` 或 default-enable 状态置为 true。
...
2. 读取 Rust / Go 优化方案、本文和 `docs/improve/rust-go/next-session-prompt.md`，确认当前状态为“Phase 0-4 已完成；Phase 5 已完成前置依赖决策与 protocol / smoke 计划、100MB TS fallback benchmark gate、最小 Rust search-only sidecar、Electron main process sidecar manager、Chat search fallback gate、Rust vs TS contract parity、100MB native benchmark、native benchmark gate 汇总、Agent production facade benchmark 分析、Agent nested native parity、fake sidecar / cache-corruption smoke、optional package manifest schema helper、`packaged-manifest` 预检、bundled package resolver fixture、`packaged-app-layout` 只读 preflight smoke、`asar: false` packaged app evidence classifier、`packagedAppIdentityVerified` gate、optionalDependencies declaration preflight gate、optional package install-chain preflight gate、default-enable readiness 预检、packaging config allowlist 预检、optional package 发布状态预检、packaged bundled binary smoke 执行计划预检、optional package publish-target dry-run 预检、release-ready source version 前置、optional package source preflight、optional / packaged gate 执行顺序预检和 builder allowlist dry-run plan；native 未达默认启用门槛，packaged native binary / 真实 optional package 发布 / optionalDependencies 实际声明与安装执行 / 真实 packaged app bundled binary smoke / builder allowlist 实际修改 / 最终 default-enable 风险决策尚未完成”。
3. 运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `95c9bf12 docs(rust-go): 同步 Phase 5 builder allowlist dry-run 后续状态`、`a3f98673 feat(rust-go): 补齐 Phase 5 builder allowlist dry-run plan`、`c43b64f2 docs(rust-go): 回填 Phase 5 optional packaged 最新恢复入口`、`3d82125e docs(rust-go): 同步 Phase 5 optional packaged 执行顺序后续状态`、`ce89f490 feat(rust-go): 补齐 Phase 5 optional packaged 执行顺序预检`、`10faa4a3 docs(rust-go): 回填 Phase 5 optional package source 最新恢复入口`、`cf17e7da docs(rust-go): 同步 Phase 5 optional package source 后续状态`、`05c393bb feat(rust-go): 补齐 Phase 5 optional package source 预检`、`e11f4476 docs(rust-go): 回填 Phase 5 release source 最新恢复入口`、`1b496638 docs(rust-go): 同步 Phase 5 release source version 后续状态`、`dc75f380 feat(rust-go): 对齐 Phase 5 native search release source version`、`63507795 docs(rust-go): 回填 Phase 5 publish target 最新恢复入口`、`92a6f1a6 docs(rust-go): 同步 Phase 5 publish target 后续状态` 和 `9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`。
