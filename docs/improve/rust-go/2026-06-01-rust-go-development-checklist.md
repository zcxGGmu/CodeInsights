...
> 最新已确认恢复入口：`63507795 docs(rust-go): 回填 Phase 5 publish target 最新恢复入口`；若本轮状态同步提交后，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
...
启动后先运行 `git status --short --branch` 和 `git log -25 --oneline`，确认最近历史包含 `dc75f380 feat(rust-go): 对齐 Phase 5 native search release source version`、`63507795 docs(rust-go): 回填 Phase 5 publish target 最新恢复入口`、`92a6f1a6 docs(rust-go): 同步 Phase 5 publish target 后续状态`、`9e3e3dff feat(rust-go): 补齐 Phase 5 optional package publish target 预检`、`a05084cd docs(rust-go): 回填 Phase 5 packaged smoke 最新恢复入口`、`33d0990d docs(rust-go): 同步 Phase 5 packaged smoke 执行计划后续状态` 和 `dedbfed1 feat(rust-go): 补齐 Phase 5 packaged smoke 执行计划预检`。下一步继续 Phase 5“Rust search sidecar 试点”：在 default off 前提下优先推进真实 optional package 发布、真实 `optionalDependencies` 声明与安装执行、builder allowlist 实际修改前置工作、真实 packaged app bundled binary smoke 设计；仅在真实 optional / packaged gate 可执行通过后再做最终 default-enable 风险决策。真实 packaged smoke、install-chain、publication 与 builder allowlist gate 未完成前不得默认启用 native。不要创建 packaged native binary，不修改 `apps/electron/electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不新增真实 `@codeinsights/native-search-*` optionalDependencies，不 push，不创建 PR。
