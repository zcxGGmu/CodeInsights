...
> 最新已确认恢复入口：`348e003d feat(rust-go): 补齐 Phase 5 packaging config allowlist 预检`；若本轮状态同步提交后，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
