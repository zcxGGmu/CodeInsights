...
3. 运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含 `05df1687 docs(rust-go): 同步 Phase 5 optionalDependencies 预检后续状态`、`563b804c feat(rust-go): 补齐 Phase 5 optionalDependencies 声明预检 gate`、`16d5af21 docs(rust-go): 同步 Phase 5 packaged app evidence 后续状态`、`31e37cbb fix(rust-go): 收紧 Phase 5 packaged app evidence 真实验证` 和 `800dc885 feat(rust-go): 补齐 Phase 5 packaged app evidence smoke`；再用 `git log -12 --oneline` 确认 `52613780`、`24d1b657`、`e95cd284`、`ce104a59`、`8226c992` 仍在历史中，如需确认更早 optional manifest / fake sidecar / native smoke / benchmark 文档，再用更深日志确认 `e5b92fa7`、`9fe91b03`、`fb7e2d73`、`2cc95b1b`、`9c102c0a`、`c6104eee`、`4f9c4d28`。
