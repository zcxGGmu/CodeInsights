...
> 最新已确认恢复入口：`8ed8c1c5 docs(rust-go): 同步 Phase 5 fail-closed 证据状态`；若本轮状态同步提交后，下次启动以 `git log -5 --oneline` 中最新的 Rust / Go docs 提交为实际恢复入口。
