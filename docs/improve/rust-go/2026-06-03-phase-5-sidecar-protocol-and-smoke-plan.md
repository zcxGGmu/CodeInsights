...
> 最新状态同步：`f78c44d6 docs(rust-go): 回填 Phase 5 最新恢复入口` 是本轮实现前已确认恢复入口；若本轮状态回填再次提交后，下次启动以 `git log -5 --oneline` 中最新 Rust / Go docs 提交为实际恢复入口。approval packet 和 gate approval queue 仍不代表真实 optional package 发布、install-chain、builder allowlist 实际修改或 packaged bundled binary smoke 已完成。
