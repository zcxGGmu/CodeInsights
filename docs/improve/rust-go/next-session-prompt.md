...
> 最新已确认开发基线：`e5b92fa7 feat(rust-go): 补齐 Phase 5 optional package manifest 预检`。
...
- [ ] Phase 5 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进 bundled package resolver / 真实 packaged smoke 设计，或继续分析 Agent native work-delay 小幅回退。只有 P95 / event loop gate 同时达标且真实 packaged smoke 进入可执行阶段后，才允许默认启用 native。
