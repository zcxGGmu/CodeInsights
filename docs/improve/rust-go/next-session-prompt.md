...
> 最新已确认开发基线：`c6104eee feat(rust-go): 补齐 Phase 5 native smoke 与 cache schema 基础`。
...
启动后先运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含 `c6104eee feat(rust-go): 补齐 Phase 5 native smoke 与 cache schema 基础`，以及 `4f9c4d28`、`40f0b06e`、`28e8a504`、`1e851056`。下一步继续 Phase 5“Rust search sidecar 试点”：在 default off 前提下优先补齐 `protocol-mismatch` / `crash` / `timeout` / `cache-corruption` smoke 的 fake sidecar / isolated cache fixture，或继续推进 optional package manifest 与 packaged smoke 设计；未达到 P95 / event loop gate 且 packaged smoke 未完成前不得默认启用 native。不要创建 packaged native binary，不修改 `electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。
