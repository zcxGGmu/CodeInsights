...
> 最新已确认开发基线：`2cc95b1b feat(rust-go): 补齐 Phase 5 fake sidecar smoke 失败路径`。
> 最新已确认恢复入口：`fb7e2d73 docs(rust-go): 同步 Phase 5 fake sidecar smoke 后续状态`；本轮恢复入口回填提交后，以 `git log -5 --oneline` 中最新的 `docs(rust-go): 回填 Phase 5 fake sidecar smoke 最新恢复入口` 为实际恢复入口。
...
- [ ] Phase 5 下一步不要创建 packaged native binary 或修改打包配置；在 default off 前提下优先推进 optional package manifest 与 packaged smoke 设计，或继续分析 Agent native work-delay 小幅回退。只有 P95 / event loop gate 同时达标且 packaged smoke 计划进入可执行阶段后，才允许默认启用 native。
