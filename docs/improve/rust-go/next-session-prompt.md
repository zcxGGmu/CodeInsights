...
> 最新已确认恢复入口：`2cc95b1b feat(rust-go): 补齐 Phase 5 fake sidecar smoke 失败路径`；本轮状态同步提交后，以 `git log -5 --oneline` 中最新的 `docs(rust-go): 同步 Phase 5 fake sidecar smoke 后续状态` 为实际恢复入口。
...
启动后先运行 `git status --short --branch` 和 `git log -5 --oneline`，确认最近历史包含 `2cc95b1b feat(rust-go): 补齐 Phase 5 fake sidecar smoke 失败路径`、`9c102c0a`、`c6104eee`、`4f9c4d28`、`40f0b06e`。下一步继续 Phase 5“Rust search sidecar 试点”：在 default off 前提下优先推进 optional package manifest 与 packaged smoke 设计，或继续分析 Agent native work-delay 小幅回退；未达到 P95 / event loop gate 且 packaged smoke 未完成前不得默认启用 native。不要创建 packaged native binary，不修改 `electron-builder.yml`，不修改根 `README.md` / 根 `AGENTS.md`，不 push，不创建 PR。
